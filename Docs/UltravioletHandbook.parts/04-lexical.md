## 4. Source Text & Lexical Structure
...
- **Universe-protected bindings.** A fixed set of built-in names (`UniverseProtected`) is reserved at the binding level. This includes every primitive type (`i8`…`i128`, `u8`…`u128`, `f16`/`f32`/`f64`, `bool`, `char`, `usize`, `isize`), `string`, `bytes`, `Self`, the built-in predicate names, and the standard capability/runtime types (for example `Region`, `RegionOptions`, `CancelToken`, `Context`, `System`, `IO`, `HeapAllocator`, `Network`, `Reactor`, `Time`, `Duration`, `Future`, `Stream`, and others). In particular `Drop`, `Bitcopy`, `Clone`, and `FfiSafe` are reserved predicate names and MUST NOT be declared as classes or used as user-defined type/value bindings.
