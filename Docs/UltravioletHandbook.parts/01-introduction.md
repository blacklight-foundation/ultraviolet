...
- **No hidden allocation.** Region allocation is written with the `^` allocation expression inside a `region` block; it does not happen silently behind an innocent-looking expression. The reader sees where memory is acquired. See "Statements and Blocks" (region grammar, B.14).
...
procedure accumulateStaged(values: [i32]) -> i32 {
    var total: i32 = 0
    region as staging {
        // Allocation is explicit: the `^` operator inside the named region is the only
        // way region memory is acquired here. Nothing in the surrounding code hides it.
        ...
        let snapshot: Snapshot = staging^Snapshot { sum: total, count: 0 }
        total += snapshot.sum
    }
    return total
}
