...
The paper derives these criteria from four evidence classes: transformer and
autoregressive language-model mechanics; empirical code-generation results such
as Codex/HumanEval, AlphaCode, MultiPL-E, Code Llama, StarCoder, SWE-bench,
CrossCodeEval, ReCode, EvalPlus, API-misuse studies, package-hallucination
studies, license-compliance studies, and security studies of generated code;
type-theoretic results on type soundness, propositions-as-types,
linear/resource types, session/protocol types, and refinement types; and the
Ultraviolet specification mechanisms mapped in section 9. The main conclusion is
not that a language can make LLMs reliable by itself. The defensible conclusion
is narrower: language design can reduce the accepted error surface and improve
feedback density by aligning
...
### 4.5 Known failure areas and root causes

The literature does not support a single flat list of "LLM coding failures",
because different studies measure different tasks: single-function synthesis,
data-science snippets, API use, package selection, repository repair, security,
long-context retrieval, benchmark validity, and agentic tool use. The useful
research inventory is therefore a class taxonomy. The list below is intended to
be complete at the level needed by this paper: every known failure area relevant
to code-writing LLMs should map to one or more of these classes, even though a
new benchmark may introduce a new instance of a class.

K1. Syntax, parse, and formatting invalidity. A model can emit text that is not
valid source, is not valid in the target language version, or is malformed at a
file or patch boundary. Grammar-constrained decoding work such as PICARD exists
because unconstrained autoregressive decoding does not intrinsically respect the
target formal language [Scholak2021].

Root cause. The model generates tokenizer tokens, not parser states or typed AST
nodes. A tokenizer can represent byte sequences that the language frontend later
rejects. Unless decoding is constrained by grammar or repaired by a frontend,
the model's next-token objective only makes syntactic validity likely, not
guaranteed.

K2. Prompt and specification underdetermination. A model can satisfy the most
obvious reading of a prompt while violating unstated edge cases, preconditions,
postconditions, error behavior, performance requirements, security requirements,
or compatibility requirements. HumanEval, MBPP, APPS, and DS-1000 all evaluate
natural-language-to-code tasks, but their need for test harnesses and
problem-specific evaluation shows that natural language is not by itself an
executable specification [Chen2021] [Austin2021] [Lai2022DS1000].

Root cause. The prompt is usually weaker than the intended program
specification. It does not define all input domains, failure modes, authority
constraints, resource obligations, or observational equivalence criteria. The
model therefore optimizes for a plausible completion under an incomplete
description, while the user's intended semantics remain partly implicit.

K3. Functional and algorithmic semantic errors. A generated program can compile
and still compute the wrong result, miss edge cases, use the wrong algorithm,
fail on larger inputs, or confuse intermediate bindings. Codex/HumanEval reports
strong gains over non-code models but also identifies difficulty with long chains
of operations and binding operations [Chen2021]. AlphaCode shows that harder
algorithmic problems require large-scale sampling and behavior filtering, not
just a single natural continuation [AlphaCode2022]. Austin et al. also report
that even strong models were generally unable to predict execution results for
specific inputs in their semantic-grounding experiment [Austin2021].

Root cause. The model's training objective is next-token prediction over text,
not execution, proof search, or complete symbolic reasoning. Generated code may
look locally natural while failing a global semantic relation between inputs,
intermediate state, and outputs. Sampling plus tests can search the space, but
that does not make any single completion semantically grounded.

K4. Execution-state, resource, and protocol errors. A model can use a value
after transfer, duplicate responsibility, omit cleanup, call lifecycle methods in
the wrong order, access state before initialization, or represent protocol state
with unchecked booleans. These failures often appear as ordinary-looking code
because the surface operation is syntactically legal.

Root cause. The deciding fact is non-local and often hidden: whether a resource
is still owned, whether a handle is open, whether a field has been initialized,
or whether the current state permits a method. If the language accepts the call
without a type, permission, or protocol obligation, the LLM has to infer the
state from distant convention and the compiler cannot reject the omission.

K5. API misuse, library hallucination, and version drift. LLMs can call methods
that do not exist, select the wrong overload, pass arguments in the wrong order,
use deprecated behavior, combine incompatible versions, or import nonexistent
packages. Gorilla identifies accurate API calls as difficult for direct
prompting, especially argument generation and API hallucination [Patil2023Gorilla].
RobustAPI found high rates of API misuse in generated Java code, including for
GPT-4 [Zhong2023RobustAPI]. Zhuo et al. identify method-selection and
parameter-usage misuse patterns, with hallucination and intent misalignment as
persistent challenges [Zhuo2025ApiMisuse]. Package hallucination studies show
that generated imports can name packages that do not exist, creating a supply
chain attack surface [Spracklen2024].

Root cause. Library knowledge is external, versioned, and open-ended. The model
compresses patterns from many API versions, tutorials, partial examples, and
similarly named libraries into parametric memory. Without retrieval grounded in
the current dependency graph and without closed-world validation of imports,
names, signatures, and package provenance, plausible API-shaped text can be
accepted into generated source.

K6. Security vulnerability generation. LLM-generated code can omit validation,
use unsafe defaults, mishandle cryptography, leak secrets, trust untrusted input,
construct injections, mishandle paths, or cross authority boundaries
accidentally. Pearce et al. found vulnerable Copilot outputs across
security-relevant scenarios [Pearce2021]. Later work on security weaknesses in
AI-generated code and package hallucinations reinforces that plausible code can
miss security obligations even when it is syntactically and functionally
plausible [Spracklen2024] [Zhuo2025ApiMisuse].

Root cause. Security requirements are often negative, contextual, and absent
from examples: "do not leak", "do not trust this input", "do not use this
authority", "do not accept this race", "do not deserialize this object." Public
training data contains insecure code, and many mainstream APIs make insecure or
over-authorized behavior easy to spell. If the language and API do not make
authority, trust, sanitization, and unsafe boundaries explicit, the model can
select a convenient vulnerable idiom that looks normal.

K7. Repository-scale integration failures. A model can patch the wrong file,
miss a caller, violate a project invariant, duplicate an existing helper, ignore
style or layering rules, or produce a change that passes a local symptom but
does not solve the repository issue. SWE-bench was designed precisely because
real issue resolution requires coordinating changes across functions, classes,
and files, interacting with execution environments, processing long contexts,
and reasoning beyond traditional code generation tasks [Jimenez2023].
CrossCodeEval similarly shows that cross-file code completion remains difficult
when relevant repository context is absent or poorly retrieved [Ding2023CrossCodeEval].

Root cause. Repository correctness depends on project-local APIs, invariants,
layout, build configuration, transitive callers, and conventions that may be
outside the immediate prompt. Retrieval can omit the relevant file or include it
without the decisive symbol. A language can reduce this burden by putting more
authority, effect, ownership, and contract facts in local signatures, but it
cannot remove the need for repository context.

K8. Long-context and context-placement failures. A coding model can receive the
right information and still fail to use it when the information is buried in a
large context. Long-context studies show that models often use information less
reliably when it appears in the middle of the context, even when the total
context length is technically supported [Liu2023Lost].

Root cause. A longer context window is not equivalent to uniform access to every
fact in the window. Relevant facts compete with surrounding tokens and with
position biases induced by the architecture, training, and decoding setup. Code
generation therefore benefits when the language and tooling move discriminating
facts near the use site instead of relying on distant comments or global
conventions.

K9. Test insufficiency and false correctness. A generated program can pass the
provided tests while being wrong. EvalPlus demonstrates that augmenting HumanEval
with many additional tests catches substantial amounts of wrong code that the
original tests missed and can change model rankings [Liu2023EvalPlus]. DS-1000
uses multi-criteria evaluation because ordinary test cases alone are not always
enough for reliable acceptance in data-science code [Lai2022DS1000].

Root cause. Tests sample behavior; they are not a complete specification unless
the input domain is finite and exhaustively covered. A model can overfit to
visible examples, exploit weak oracles, satisfy surface constraints, or produce
a solution that works for common cases but violates untested obligations.
Behavior filtering is valuable, but it is only as strong as the oracle.

K10. Benchmark contamination and memorization. A model can appear to solve code
tasks by having seen the benchmark, near-duplicates, solutions, or synthetic
variants during training or tuning. Matton et al. document direct leakage,
synthetic-data leakage, and overfitting to evaluation sets in code-generation
benchmarks [Matton2024]. Work on SWE-bench Verified raises similar concerns
that some apparent repository-repair performance may be driven by memorization
or benchmark artifacts rather than generalizable problem solving
[Liang2025SWEIllusion].

Root cause. Code benchmark prompts, tests, and solutions are widely replicated
in public repositories, blog posts, notebooks, evaluation harnesses, and
synthetic training data. Code decontamination is harder than natural-text
deduplication because small surface edits can preserve program semantics, while
small token changes can radically change behavior. Benchmark performance
therefore measures a mixture of capability, memorization, retrieval, and
evaluation-specific adaptation unless contamination is actively controlled.

K11. Robustness failures under equivalent prompt or source perturbations. A
model can produce different or worse code when the same task is paraphrased, a
variable is renamed, code formatting changes, examples are reordered, or the
target programming language changes. ReCode shows that code generation models
are brittle under natural semantics-preserving prompt and source transformations
[Wang2022ReCode]. MultiPL-E shows that performance varies across programming
languages [Cassano2022].

Root cause. Equivalent human intent does not necessarily induce equivalent token
distributions. If a language or task has many surface encodings for the same
semantic operation, evidence fragments across forms. If the target language has
little training representation, or if the prompt uses unfamiliar wording, the
model may leave the high-probability region of correct completions.

K12. Repair-loop and diagnostic-locality failures. A model can fail to repair
its own generated code when compiler errors, stack traces, test failures, or
review comments are vague, late, cascading, or causally distant from the true
mistake. It may patch the symptom, introduce unrelated changes, or oscillate
between fixes.

Root cause. The next repair attempt is conditioned on feedback text. If the
feedback does not name the missing capability, type, state, proof obligation,
resource transfer, or failing predicate, the model must infer the root cause
from noisy traces. Precise diagnostics are not just a human usability feature;
they become training and prompt material for the next generation step.

K13. Maintainability, architecture, and reviewability failures. Generated code
can be over-broad, over-defensive, inconsistent with project architecture, too
coupled, duplicative, or hard to review even when it passes tests. This is the
repository-scale analogue of naturalness without correctness: the code has local
plausibility but does not preserve the design pressure of the surrounding
system.

Root cause. Maintainability is usually encoded in conventions, module
boundaries, naming patterns, layering rules, and review norms rather than in
machine-checkable obligations. Unless those obligations are local in APIs,
contracts, modules, capabilities, or tests, the model has only weak statistical
signals about whether a change is architecturally acceptable.

K14. Agentic tool-use, prompt-injection, and authority failures. Modern coding
assistants increasingly read repositories, call tools, edit files, run commands,
and use extension protocols. This creates failures beyond the generated code
itself: malicious repository text can steer the agent, poisoned tool metadata
can affect tool choice, and over-authorized agents can leak data or perform
unintended actions. Recent work on agentic coding assistants treats prompt
injection as a first-class vulnerability class requiring architectural
mitigations [Maloyan2026]. A large-scale public competition found successful
indirect prompt-injection attacks across tool-calling, coding, and computer-use
settings [Dziemian2026].

Root cause. LLM agents often process instructions and untrusted data in the same
token stream while holding filesystem, shell, network, repository, or account
authority. The model is asked to infer which text is command, which text is
data, and which tool action is authorized. This is structurally similar to
ambient authority in programming languages: if authority is broad and instruction
boundaries are not enforced by the system, plausible text can become unintended
action.

K15. License, provenance, and attribution failures. Generated code can be
strikingly similar to existing open-source implementations without carrying the
license, copyright, attribution, or provenance information needed to decide
whether it may be used in a target project. LiCoEval evaluates this compliance
problem and finds that models can generate code with striking similarity to
existing implementations while failing to provide accurate license information,
especially for copyleft-licensed code [Xu2024LiCoEval].

Root cause. Code models are trained over large corpora whose provenance and
licenses are not available as explicit facts at generation time. The generated
token stream normally does not include evidence of where an idiom, algorithmic
expression, or implementation shape came from. Unless the toolchain separately
tracks provenance, performs similarity analysis, or constrains the training and
retrieval corpus by license, the user cannot infer license compliance from the
generated source text alone.

These classes separate symptoms from causes. Syntax failures come from decoding
text rather than ASTs. Semantic failures come from optimizing natural
continuations rather than executing or proving programs. Repository failures
come from missing or poorly localized context. Security failures come from hidden
negative obligations and over-broad authority. API failures come from ungrounded
and versioned external knowledge. Test failures come from weak oracles.
Contamination failures come from measuring memorization as if it were
generalization. Agentic failures come from mixing data, instructions, and
authority. Provenance failures come from generating source without attached
origin and license evidence. A language cannot eliminate every class, but it can
directly address the subset whose root cause is hidden, implicit, non-local,
unchecked, or over-authorized program meaning.
...
not cover every reason an LLM can fail. They are the language-addressable subset
of the broader failure taxonomy in section 4.5: cases where the root cause is
that program obligations are hidden, implicit, non-local, unchecked, or carried
by over-broad authority.
...
4. API and package freshness. Language design cannot keep a model's library
   knowledge current or prove that an external package name is legitimate; that
   requires dependency resolution, package provenance, and documentation
   retrieval.
5. Specification quality. Contracts and refinements only check what they state.
   Weak specifications leave weak obligations.
6. Decidability. Refinement and contract proving must use decidable fragments or
   runtime checks. Full semantic correctness is not generally decidable.
7. Unsafe and foreign trust. Explicit boundaries localize risk; they do not
   eliminate the need to audit boundary implementations and foreign code.
8. Agent tool policy. Ultraviolet can make program authority explicit, but an
   external coding agent can still be vulnerable if its tool host grants broad
   filesystem, shell, network, repository, or account authority to untrusted
   prompt content.
9. License and provenance compliance. Ultraviolet source can be checked for UV
   semantics, but the language cannot by itself prove that generated code has a
   permissible training, retrieval, or authorship origin.
10. Implementation fidelity. This paper maps to the specification. If a compiler
...
[Austin2021]
Jacob Austin et al. "Program Synthesis with Large Language Models." arXiv
2108.07732, 2021. https://arxiv.org/abs/2108.07732
...
Cross-File Code Completion." arXiv 2310.11248, 2023.
https://arxiv.org/abs/2310.11248

[Dziemian2026]
Mateusz Dziemian et al. "How Vulnerable Are AI Agents to Indirect Prompt
Injections? Insights from a Large-Scale Public Competition." arXiv 2603.15714,
2026. https://arxiv.org/abs/2603.15714
...
[Lai2022DS1000]
Yuhang Lai et al. "DS-1000: A Natural and Reliable Benchmark for Data Science
Code Generation." arXiv 2211.11501, 2022. https://arxiv.org/abs/2211.11501
...
[Liang2025SWEIllusion]
Shanchao Liang, Spandan Garg, and Roshanak Zilouchian Moghaddam. "The
SWE-Bench Illusion: When State-of-the-Art LLMs Remember Instead of Reason."
arXiv 2506.12286, 2025. https://arxiv.org/abs/2506.12286
...
Models for Code Generation." arXiv 2305.01210, 2023.
...
[Maloyan2026]
Narek Maloyan and Dmitry Namiot. "Prompt Injection Attacks on Agentic Coding
Assistants: A Systematic Analysis of Vulnerabilities in Skills, Tools, and
Protocol Ecosystems." arXiv 2601.17548, 2026.
https://arxiv.org/abs/2601.17548

[Matton2024]
Alexandre Matton et al. "On Leakage of Code Generation Evaluation Datasets."
Findings of the Association for Computational Linguistics: EMNLP 2024,
13215-13223. https://aclanthology.org/2024.findings-emnlp.772/
...
[Patil2023Gorilla]
Shishir G. Patil, Tianjun Zhang, Xin Wang, and Joseph E. Gonzalez. "Gorilla:
Large Language Model Connected with Massive APIs." arXiv 2305.15334, 2023.
...
Bimal Viswanath, and Murtuza Jadliwala. "We Have a Package for You! A
Comprehensive Analysis of Package Hallucinations by Code Generating LLMs."
arXiv 2406.10279, 2024. https://arxiv.org/abs/2406.10279
...
arXiv 2212.10264, 2022. https://arxiv.org/abs/2212.10264
...
[Xu2024LiCoEval]
Weiwei Xu, Kai Gao, Hao He, and Minghui Zhou. "LiCoEval: Evaluating LLMs on
License Compliance in Code Generation." arXiv 2408.02487, 2024.
https://arxiv.org/abs/2408.02487

[Zhong2023RobustAPI]
Li Zhong and Zilong Wang. "Can ChatGPT replace StackOverflow? A Study on
Robustness and Reliability of Large Language Model Code Generation." arXiv
2308.10335, 2023. https://arxiv.org/abs/2308.10335

[Zhuo2025ApiMisuse]
Terry Yue Zhuo et al. "Identifying and Mitigating API Misuse in Large Language
Models." arXiv 2503.22821, 2025. https://arxiv.org/abs/2503.22821
