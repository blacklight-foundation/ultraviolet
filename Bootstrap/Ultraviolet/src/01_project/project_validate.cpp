...
//   - Path Resolution
//
// SOURCE FILE: ultraviolet-bootstrap/src/01_project/project_validate.cpp
...
//    - PURPOSE: Check if directory contains Ultraviolet.toml
...
//
// Headers required:
//   - "uv/01_project/project.h" (types)
//   - "uv/00_core/assert_spec.h" (SPEC_RULE macro)
//   - "uv/00_core/diagnostic_messages.h" (MakeDiagnostic)
//   - "uv/00_core/diagnostics.h" (DiagnosticStream, Emit)
//   - "uv/00_core/host_primitives.h" (HostPrimFail)
//   - "uv/00_core/path.h" (IsRelative, Resolve, Prefix)
//   - "uv/01_project/ident.h" (IsName, IsIdentifier, IsKeyword)
//   - <string_view>
//   - <system_error>
//   - <unordered_set>
//   - <filesystem>
...
bool IsProjectRoot(const std::filesystem::path& root) {
  ...
  if (ec) {
    core::HostPrimFail(core::HostPrim::IOExists, true);
    return false;
  }
  if (exists) {
