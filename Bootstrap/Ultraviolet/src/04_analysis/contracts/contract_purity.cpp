...
static inline void SpecDefsContractPurity() {
  SPEC_DEF("ContractPure", "UVX.5.W");
  SPEC_DEF("PureExpr", "UVX.5.W");
  SPEC_DEF("CapabilityCall", "UVX.5.W");
  SPEC_DEF("SideEffect", "UVX.5.W");
  SPEC_DEF("PureOp", "UVX.5.W");
  SPEC_DEF("MutationFree", "UVX.5.W");
}
...
bool IsLikelyCapabilityCall(const ast::MethodCallExpr& call) {
  ...
  // Check if receiver is a field access on ctx (e.g., ctx.io, ctx.heap)
  if (const auto* field =
