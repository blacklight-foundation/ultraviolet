...
//   - Type Ordering (Ultraviolet) (lines 14660-14719)
...
//
// SOURCE FILE: ultraviolet-bootstrap/src/04_codegen/layout/layout_unions.cpp
...
//
// DEPENDENCIES:
//   - ultraviolet/include/04_analysis/layout/layout.h (UnionLayout struct)
//   - ultraviolet/include/04_analysis/types/type_equiv.h (SortUnionMembers)
//   - ultraviolet/include/04_analysis/types/types.h (TypeRef, TypePerm, TypePtr)
...
std::uint64_t NicheCount(const ultraviolet::analysis::ScopeContext& ctx,
  ...
  if (const auto* path =
          std::get_if<ultraviolet::analysis::TypePathType>(&type->node)) {
    const auto it = ctx.sigma.types.find(path->path);
    if (it != ctx.sigma.types.end()) {
      if (const auto* alias =
              std::get_if<ultraviolet::ast::TypeAliasDecl>(&it->second)) {
...
std::optional<UnionLayout> UnionLayoutOf(
  ...
  const auto member_list = ultraviolet::analysis::SortUnionMembers(uni.members);
