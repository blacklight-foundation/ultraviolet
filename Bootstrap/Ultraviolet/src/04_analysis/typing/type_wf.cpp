...
//
// SOURCE FILE: ultraviolet-bootstrap/src/03_analysis/types/type_wf.cpp
...
static bool IsTestAuthorityPathType(const TypePath& path) {
  return path.size() == 1 && IdEq(path.front(), "TestAuthority");
}
  ...
  return std::visit(
      [&](const auto& node) -> TypeWfResult {
        ...
        if constexpr (std::is_same_v<T, TypePrim>) {
          ...
          if (IsTestAuthorityPathType(node.path)) {
            SPEC_RULE("WF-Path");
            return {true, std::nullopt};
          }
