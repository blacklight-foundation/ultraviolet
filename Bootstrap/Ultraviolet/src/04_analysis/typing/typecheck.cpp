...
//
// SOURCE: ultraviolet-bootstrap/src/03_analysis/types/typecheck.cpp
//
// =============================================================================
...
ScopeList ModuleScopesForMainCheck(const NameMapTable& name_maps,
                                   const ast::ModulePath& module_path) {
  Scope module_scope;
  ...
  return {Scope{}, std::move(module_scope), UniverseBindings()};
}

// Check if main procedure has valid signature: (move ctx: ContextBundle) -> i32
static bool MainSigOk(const ScopeContext& ctx, const ast::ProcedureDecl& decl) {
  ...
  // Parameter must be move mode, or omit mode to use the entry default.
    ...
    for (const auto& item : module.items) {
      ...
      std::visit(
          [&](const auto& node) {
            using T = std::decay_t<decltype(node)>;
            if constexpr (std::is_same_v<T, ast::ProcedureDecl>) {
              const auto res = ::ultraviolet::analysis::TypeProcedureDecl(
              ...
              const auto res = ::ultraviolet::analysis::TypeClassDecl(
                  ctx, node, module.path, result.diags);
              if (!res.ok) {
                EmitDeclDiag(result.diags, res.diag_id,
                             std::optional<core::Span>(node.span),
              ...
              const auto res = ::ultraviolet::analysis::TypeTypeAliasDecl(
                  ctx, node, module.path, result.diags);
              if (!res.ok) {
                EmitDeclDiag(result.diags, res.diag_id,
                             std::optional<core::Span>(node.span));
              ...
              const auto res = ::ultraviolet::analysis::TypeImportDecl(
              ...
              const auto res = ::ultraviolet::analysis::TypeUsingDecl(
                  ctx, node, module.path);
              if (!res.ok) {
                EmitDeclDiag(result.diags, res.diag_id,
                             std::optional<core::Span>(node.span));
              ...
              const auto res = ::ultraviolet::analysis::TypeExternBlock(
                  ctx, node, module.path, result.diags);
              if (!res.ok) {
                EmitDeclDiag(result.diags, res.diag_id,
                             std::optional<core::Span>(node.span));
...
DeclTypingResult MainCheckProject(ScopeContext& ctx,
    ...
    for (const auto& item : module.items) {
      if (const auto* proc = std::get_if<ast::ProcedureDecl>(&item)) {
        if (IdEq(proc->name, "main")) {
          mains.push_back(MainDeclRef{proc, &module.path});
        }
      }
    }
  }
  ...
  if (mains.size() > 1) {
    ...
    const std::optional<core::Span> duplicate_span =
        mains.front().decl ? std::optional<core::Span>(mains.front().decl->span)
                           : std::nullopt;
    EmitTypecheckDiag(result.diags, "E-MOD-2430", duplicate_span);
    result.ok = false;
    return result;
  }

  const MainDeclRef main_ref = mains.front();
  ...
  ScopedCurrentModuleOverride module_override(ctx, *main_module_path);
  ScopedScopesOverride scopes_override(
      ctx, ModuleScopesForMainCheck(name_maps, *main_module_path));
...
TypecheckResult TypecheckModules(
  ...
  if (!core::HasError(result.diags)) {
    ...
    if (require_main) {
      const auto main_check = MainCheckProject(ctx, modules, *name_maps);
