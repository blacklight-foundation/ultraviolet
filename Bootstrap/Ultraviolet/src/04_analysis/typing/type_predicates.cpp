...
//
// SOURCE: ultraviolet-bootstrap/src/03_analysis/types/type_predicates.cpp
//
// =============================================================================
...
bool UnitEnumType(const ScopeContext& ctx, const TypeRef& type) {
  const TypeRef base = NormalizeFoundationalBuiltinBase(type);
  if (!base) {
    return false;
  }

  const TypePath* path = AppliedTypePath(*base);
  if (!path) {
    return false;
  }

  const ast::EnumDecl* decl = LookupEnumDecl(ctx, *path);
  if (!decl || decl->variants.empty()) {
    return false;
  }
  ...
  return std::all_of(
      ...
      [](const ast::VariantDecl& variant) {
        return !variant.payload_opt.has_value();
      });
}
...
static std::optional<FoundationalBuiltinMethodSig>
LookupFoundationalBuiltinMethodSigImpl(const ScopeContext* ctx,
  ...
  const bool has_intrinsic_eq = ctx ? EqType(*ctx, base) : EqType(base);
  if (IdEq(name, "eq") && has_intrinsic_eq) {
...
std::optional<FoundationalBuiltinMethodSig> LookupFoundationalBuiltinMethodSig(
    const TypeRef& recv_base,
    std::string_view name) {
  return LookupFoundationalBuiltinMethodSigImpl(nullptr, recv_base, name);
}
...
std::optional<FoundationalBuiltinMethodSig> LookupFoundationalBuiltinMethodSig(
    ...
    const TypeRef& recv_base,
    std::string_view name) {
  return LookupFoundationalBuiltinMethodSigImpl(&ctx, recv_base, name);
}
