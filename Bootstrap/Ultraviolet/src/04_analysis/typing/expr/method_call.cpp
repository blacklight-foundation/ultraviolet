...
static std::optional<IOMethodSig> LookupComptimeDiagnosticsMethodSig(
...
static std::optional<IOMethodSig> LookupIntrospectMethodSig(
    std::string_view name) {
  IOMethodSig sig{};
  sig.recv_perm = Permission::Const;
...
ExprTypeResult TypeMethodCallExprImpl(const ScopeContext& ctx,
  ...
  if (const auto builtin_sig =
          LookupFoundationalBuiltinMethodSig(ctx, lookup_base, expr.name)) {
    if (!ReceiverPermissionAdmits(caller_perm, builtin_sig->recv_perm)) {
      SPEC_RULE("MethodCall-RecvPerm-Err");
      result.diag_id = "E-TYP-1605";
      return result;
  ...
  if (const auto* dyn = std::get_if<TypeDynamic>(&lookup_base->node)) {
    if (IsIOClassPath(dyn->path)) {
      const auto sig = LookupIOMethodSig(expr.name);
      if (sig.has_value()) {
  ...
  if (record_method && lookup.record_decl) {
    ...
    record_method_ctx.scopes =
        ultraviolet::analysis::BindTypeParams(ctx, lookup.record_decl->generic_params);
