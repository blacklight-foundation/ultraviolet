...
static bool IsBareTestAuthorityType(const std::shared_ptr<ast::Type>& type) {
  if (!type) {
    return false;
  }
  const auto* path = std::get_if<ast::TypePathType>(&type->node);
  return path && path->generic_args.empty() &&
         path->path == ast::TypePath{"TestAuthority"};
}
...
void LogProcedureTypePerfSummary() {
  ...
  std::fprintf(stderr,
               "[uv] sema perf=procedure decl_calls=%llu "
...
ProcedureDeclResult TypeProcedureDecl(
  ...
  if (perf_on) {
    ...
    if (!perf_banner_printed) {
      ...
      std::cerr << "[uv] sema perf=procedure begin\n";
    }
  }
