...
RecordDeclResult TypeRecordDecl(
  ...
  if (decl.invariant_opt.has_value()) {
    ...
    // Note: In Ultraviolet, fields don't have is_const - all record fields are mutable by default
  ...
  for (const auto* method : methods) {
    ...
    // Note: In Ultraviolet, Receiver = variant<ReceiverShorthand, ReceiverExplicit>
