...
//
// MIGRATED FROM: ultraviolet-bootstrap/src/03_analysis/types/type_expr.cpp
...
bool IsMemoryOrderAttribute(std::string_view name) {
  return name == ::ultraviolet::analysis::attrs::kRelaxed ||
         name == ::ultraviolet::analysis::attrs::kAcquire ||
         name == ::ultraviolet::analysis::attrs::kRelease ||
         name == ::ultraviolet::analysis::attrs::kAcqRel ||
         name == ::ultraviolet::analysis::attrs::kSeqCst;
}
...
static TypeEnv ExtendComptimeEnv(const TypeEnv& env, const ast::AttributeList* attr_list) {
  ...
  if (attr_list && HasAttribute(*attr_list, ::ultraviolet::analysis::attrs::kFiles)) {
    scope[IdKeyOf("files")] = TypeBinding{ast::Mutability::Let, ProjectFilesType()};
  }
  if (attr_list && HasAttribute(*attr_list, ::ultraviolet::analysis::attrs::kEmit)) {
...
static CheckResult TryDynamicRefinementFallback(const ScopeContext& ctx,
  ...
  const auto* refine =
      std::get_if<::ultraviolet::analysis::TypeRefine>(&norm.type->node);
  if (!refine) {
    return result;
  }
...
CheckResult CheckExprAgainst(const ScopeContext& ctx,
  ...
  if (const auto* closure_expr = std::get_if<ast::ClosureExpr>(&e->node)) {
    ...
    if (stripped_expected &&
        std::holds_alternative<TypeClosure>(stripped_expected->node)) {
      const auto typed_closure =
          expr::TypeClosureExpr(*closure_expr, ctx, type_ctx, env,
                                type_expr_fn, type_ident_fn, type_place_fn,
                                &expected);
      if (!typed_closure.ok) {
        result.diag_id = typed_closure.diag_id;
        result.diag_detail = typed_closure.diag_detail;
        result.diag_span = typed_closure.diag_span;
        return result;
      }
      const auto sub = Subtyping(ctx, typed_closure.type, expected);
      if (!sub.ok) {
        result.diag_id = sub.diag_id;
        return result;
      }
