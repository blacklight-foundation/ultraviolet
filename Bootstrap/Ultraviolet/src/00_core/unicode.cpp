...
//
// SOURCE FILE: ultraviolet-bootstrap/src/00_core/unicode.cpp
...
//
// DEPENDENCIES:
//   - ultraviolet/include/00_core/unicode.h (header)
//     - UnicodeScalar type
...
std::string NFC(std::string_view s) {
  ...
  static_assert(kUnicodeVersion == "15.0",
                "ICU must target Unicode 15.0.x to match the Ultraviolet spec.");
  UErrorCode status = U_ZERO_ERROR;
