...
std::string FixtureManifest() {
  ...
  out << "[[assembly]]\n";
  out << "name = \"drop_glue_rawptr_slot\"\n";
  out << "kind = \"executable\"\n";
  out << "root = \"src\"\n";
  out << "out_dir = \"build/drop_glue_rawptr_slot\"\n";
  out << "emit_ir = \"ll\"\n";
  return out.str();
}

std::string FixtureSource() {
  return R"uv(private var _TRACE: i32 = 0

internal record DropLeaf {
...
internal record TraceDropRecord {
    internal amount: i32
    internal payload: DropLeaf

    internal procedure drop(~!) -> () {
        _TRACE = _TRACE + self.amount
    }
}

internal procedure dropAtReturn() -> i32 {
    _TRACE = 0
    let trace_value: TraceDropRecord = TraceDropRecord {
        amount: 7,
        payload: DropLeaf { value: 0 }
    }
    return 5
}
...
int main() {
  ...
  const std::filesystem::path project_root =
      work_root / "drop_glue_rawptr_slot_fixture";
  ...
  const std::string compile_command =
      CommandWithToolPath(
          ...
          Quote(UV_TEST_COMPILER_PATH) + " --target-profile " +
              ...
              " --assembly drop_glue_rawptr_slot --out-dir " +
  ...
  if (compile_result != 0) {
    std::cerr << "drop-glue raw pointer fixture compile failed; see "
              << compile_log << "\n";
    return 1;
  }

  const auto ir_text =
      ReadFile(out_root / "ir" / "drop_x5fglue_x5frawptr_x5fslot.ll");
  if (!ir_text.has_value()) {
    return 1;
  }
  if (!Contains(*ir_text, "rawptr_arg")) {
    std::cerr << "drop glue call did not materialize a raw-pointer slot\n";
    return 1;
  }
  if (Contains(*ir_text, "load ptr, ptr %coerce_bits")) {
    std::cerr << "drop glue call still derives the raw pointer from value bytes\n";
    return 1;
  }

  const std::filesystem::path executable =
      out_root / "bin" / ("drop_glue_rawptr_slot" UV_TEST_EXECUTABLE_SUFFIX);
  const int run_result = RunExecutableWithTimeout(executable, run_log);
  if (run_result != 0) {
    std::cerr << "drop-glue raw pointer fixture exited with " << run_result
              << "; see " << run_log << "\n";
    return 1;
  }
