...
std::string FixtureManifest() {
  ...
  out << "[[assembly]]\n";
  out << "name = \"async_manual_resume\"\n";
  out << "kind = \"executable\"\n";
  out << "root = \"src\"\n";
  out << "out_dir = \"build/async_manual_resume\"\n";
  out << "emit_ir = \"ll\"\n";
  return out.str();
}

std::string FixtureSource() {
  return R"uv(internal type Computation = Async<i32, i32, i32, i32>

internal procedure suspends(value: i32) -> Computation {
    let resumed: i32 = yield value
    return resumed
}

internal procedure manualResumeReference() -> bool {
    let suspended: unique Computation = suspends(18)
...
std::optional<std::string_view> FunctionContaining(
    std::string_view text,
    std::string_view function_name) {
  const std::size_t name_pos = text.find(function_name);
  ...
  std::size_t end = text.find("\ndefine ", name_pos + function_name.size());
...
int main() {
  ...
  const std::filesystem::path project_root =
      work_root / "async_manual_resume_fixture";
  ...
  const std::string compile_command =
      CommandWithToolPath(
          ...
          Quote(UV_TEST_COMPILER_PATH) + " --target-profile " +
              ...
              " --assembly async_manual_resume --out-dir " +
  ...
  if (compile_result != 0) {
    std::cerr << "async manual resume fixture compile failed; see "
              << compile_log << "\n";
    return 1;
  }

  const auto ir_text =
      ReadFile(out_root / "ir" / "async_x5fmanual_x5fresume.ll");
  if (!ir_text.has_value()) {
    return 1;
  }
  const auto function_body =
      FunctionContaining(*ir_text, "manualResumeReference");
  if (!function_body.has_value()) {
    std::cerr << "manualResumeReference was not emitted\n";
    return 1;
  }
  if (!Contains(*function_body, "async_x3a_x3aresume")) {
    std::cerr << "manual resume did not emit the async resume runtime call\n";
    return 1;
  }
  if (Contains(*function_body, "zeroinitializer, ptr %resumed")) {
    std::cerr << "manual resume result storage was zeroed after resume\n";
    return 1;
  }

  const std::filesystem::path executable =
      out_root / "bin" / ("async_manual_resume" UV_TEST_EXECUTABLE_SUFFIX);
  const int run_result = RunExecutableWithTimeout(executable, run_log);
  if (run_result != 0) {
    std::cerr << "async manual resume fixture exited with " << run_result
              << "; see " << run_log << "\n";
    return 1;
  }
