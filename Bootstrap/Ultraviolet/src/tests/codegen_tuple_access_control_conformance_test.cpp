...
std::string FixtureManifest() {
  ...
  out << "[[assembly]]\n";
  out << "name = \"tuple_access_control\"\n";
  out << "kind = \"executable\"\n";
  out << "root = \"src\"\n";
  out << "out_dir = \"build/tuple_access_control\"\n";
  out << "emit_ir = \"ll\"\n";
  return out.str();
}

std::string FixtureSource() {
  return R"uv(internal procedure tupleAccessControlReference(flag: bool) -> i32 {
    return (if flag {
        return 13
    } else {
        (1, 2)
...
public procedure main(move ctx: Context) -> i32 {
    let _ = ctx
    if tupleAccessControlReference(false) != 1 {
        return 1
    }
    if tupleAccessControlReference(true) != 13 {
        return 2
    }
    return 0
}
...
int main() {
  ...
  const std::filesystem::path project_root =
      work_root / "tuple_access_control_fixture";
  ...
  const std::string compile_command =
      CommandWithToolPath(
          ...
          Quote(UV_TEST_COMPILER_PATH) + " --target-profile " +
              ...
              " --assembly tuple_access_control --out-dir " +
  ...
  if (compile_result != 0) {
    std::cerr << "tuple access control fixture compile failed; see "
              << compile_log << "\n";
    return 1;
  }

  const auto ir_text =
      ReadFile(out_root / "ir" / "tuple_x5faccess_x5fcontrol.ll");
  if (!ir_text.has_value()) {
    return 1;
  }
  const auto function_body =
      FunctionContaining(*ir_text, "tupleAccessControlReference");
  if (!function_body.has_value()) {
    std::cerr << "tupleAccessControlReference was not emitted\n";
    return 1;
  }
  if (!Contains(*function_body, "br i1") ||
      !Contains(*function_body, "ret i32 13")) {
    std::cerr << "tuple access control lowering did not preserve the base if\n";
    return 1;
  }

  const std::filesystem::path executable =
      out_root / "bin" / ("tuple_access_control" UV_TEST_EXECUTABLE_SUFFIX);
  const int run_result = RunExecutableWithTimeout(executable, run_log);
  if (run_result != 0) {
    std::cerr << "tuple access control fixture exited with " << run_result
              << "; see " << run_log << "\n";
    return 1;
  }
