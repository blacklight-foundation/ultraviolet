...
std::string FixtureManifest() {
  ...
  out << "[[assembly]]\n";
  out << "name = \"cancel_token_receiver\"\n";
  out << "kind = \"executable\"\n";
  out << "root = \"src\"\n";
  out << "out_dir = \"build/cancel_token_receiver\"\n";
  out << "emit_ir = \"ll\"\n";
  return out.str();
}

std::string FixtureSource() {
  return R"uv(public procedure main(move ctx: Context) -> i32 {
    let _ = ctx
...
int main() {
  ...
  const std::filesystem::path project_root =
      work_root / "cancel_token_receiver_fixture";
  ...
  const std::string compile_command =
      CommandWithToolPath(
          ...
          Quote(UV_TEST_COMPILER_PATH) + " --target-profile " +
              ...
              " --assembly cancel_token_receiver --out-dir " +
  ...
  if (compile_result != 0) {
    std::cerr << "cancel token fixture compile failed; see "
              << compile_log << "\n";
    return 1;
  }

  const auto ir_text =
      ReadFile(out_root / "ir" / "cancel_x5ftoken_x5freceiver.ll");
  if (!ir_text.has_value()) {
    return 1;
  }
  if (!Contains(*ir_text,
                "declare i8 @CancelToken_x3a_x3aActive_x3a_x3ais_x5fcancelled(ptr)") ||
      !Contains(*ir_text,
                "declare i64 @CancelToken_x3a_x3aActive_x3a_x3achild(ptr)")) {
    std::cerr << "cancel token runtime declarations did not use receiver pointers\n";
    return 1;
  }
  if (Contains(*ir_text, "CancelToken_x3a_x3aActive_x3a_x3ais_x5fcancelled(i64") ||
      Contains(*ir_text, "CancelToken_x3a_x3aActive_x3a_x3achild(i64")) {
    std::cerr << "cancel token receiver value was passed as an integer handle\n";
    return 1;
  }

  const std::filesystem::path executable =
      out_root / "bin" / ("cancel_token_receiver" UV_TEST_EXECUTABLE_SUFFIX);
  const int run_result = RunExecutableWithTimeout(executable, run_log);
  if (run_result != 0) {
    std::cerr << "cancel token fixture exited with " << run_result
              << "; see " << run_log << "\n";
    return 1;
  }
