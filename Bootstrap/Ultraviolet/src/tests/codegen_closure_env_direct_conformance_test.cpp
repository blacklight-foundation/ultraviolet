...
std::string FixtureManifest() {
  ...
  out << "[[assembly]]\n";
  out << "name = \"closure_env_direct\"\n";
  out << "kind = \"executable\"\n";
  out << "root = \"src\"\n";
  out << "out_dir = \"build/closure_env_direct\"\n";
  out << "emit_ir = \"ll\"\n";
  return out.str();
}

std::string FixtureSource() {
  return R"uv(internal type IntUnaryClosure = |i32| -> i32

internal procedure applyClosure(closure: IntUnaryClosure, value: i32) -> i32 {
    return closure(value)
}

internal procedure capturedClosureValue() -> i32 {
    let base_value: i32 = 3
    let add_captured: IntUnaryClosure = |value: i32| -> i32 value + base_value
    return add_captured(4)
}

internal procedure capturedClosureViaParameter() -> i32 {
    let base_value: i32 = 5
    let add_captured: IntUnaryClosure = |value: i32| -> i32 value + base_value
    return applyClosure(add_captured, 4)
}

public procedure main(move ctx: Context) -> i32 {
    let _ = ctx
    if capturedClosureValue() != 7 {
        return 1
    }
    if capturedClosureViaParameter() != 9 {
        return 2
    }
    return 0
}
...
int main() {
  ...
  const std::filesystem::path project_root =
      work_root / "closure_env_direct_fixture";
  ...
  const std::string compile_command =
      CommandWithToolPath(
          ...
          Quote(UV_TEST_COMPILER_PATH) + " --target-profile " +
              ...
              " --assembly closure_env_direct --out-dir " +
  ...
  if (compile_result != 0) {
    std::cerr << "closure environment fixture compile failed; see "
              << compile_log << "\n";
    return 1;
  }

  const auto ir_text =
      ReadFile(out_root / "ir" / "closure_x5fenv_x5fdirect.ll");
  if (!ir_text.has_value()) {
    return 1;
  }
  if (Contains(*ir_text, "rawptr_arg")) {
    std::cerr << "closure environment call materialized a raw-pointer slot\n";
    return 1;
  }

  const std::filesystem::path executable =
      out_root / "bin" / ("closure_env_direct" UV_TEST_EXECUTABLE_SUFFIX);
  const int run_result = RunExecutableWithTimeout(executable, run_log);
  if (run_result != 0) {
    std::cerr << "closure environment fixture exited with " << run_result
              << "; see " << run_log << "\n";
    return 1;
  }
