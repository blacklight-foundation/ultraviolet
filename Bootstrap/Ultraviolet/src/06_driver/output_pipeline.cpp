...
using namespace ultraviolet::project;
...
std::filesystem::path HostArtifactPath(const std::filesystem::path& path) {
  return TargetHostFilesystemPath(path);
}
...
void AppendBuildTelemetry(
  ...
  std::error_code ec;
  const std::filesystem::path host_path = HostArtifactPath(path);
  const bool exists = std::filesystem::exists(host_path, ec) && !ec;
  std::ofstream out(host_path, std::ios::binary | std::ios::app);
  if (!out) {
    return;
  }
  if (!exists) {
...
std::filesystem::path TempArtifactPath(const std::filesystem::path& path) {
  ...
  const auto file_hash =
      std::hash<std::string>{}(path.filename().generic_string());
  const std::string temp_name = ".uvtmp." + std::to_string(CurrentProcessId()) +
                                "." + std::to_string(thread_hash) + "." +
                                std::to_string(file_hash);
...
bool RenameArtifactFile(const std::filesystem::path& source,
                        const std::filesystem::path& destination) {
  std::error_code ec;
  std::filesystem::rename(HostArtifactPath(source),
                          HostArtifactPath(destination),
                          ec);
  return !ec;
}

bool CopyArtifactFile(const std::filesystem::path& source,
                      const std::filesystem::path& destination) {
  std::error_code ec;
  std::filesystem::copy_file(HostArtifactPath(source),
                             HostArtifactPath(destination),
...
void RemoveArtifactFile(const std::filesystem::path& path) {
  std::error_code ec;
  std::filesystem::remove(HostArtifactPath(path), ec);
}

bool ReplaceArtifactFile(const std::filesystem::path& temp_path,
                         const std::filesystem::path& path) {
  constexpr int kMaxAttempts = 20;
  constexpr auto kRetryDelay = std::chrono::milliseconds(10);

  for (int attempt = 0; attempt < kMaxAttempts; ++attempt) {
    if (RenameArtifactFile(temp_path, path)) {
      return true;
    }

    RemoveArtifactFile(path);
    if (RenameArtifactFile(temp_path, path)) {
      return true;
    }

    if (CopyArtifactFile(temp_path, path)) {
      RemoveArtifactFile(temp_path);
      return true;
    }
...
bool WriteArtifactFileAtomically(const OutputPipelineDeps& deps,
  ...
  if (ReplaceArtifactFile(temp_path, path)) {
    return true;
  }
...
bool ShouldStageImportedSharedLibraryArtifact(
    const std::filesystem::path& source) {
  if (source.has_parent_path() || source.is_absolute()) {
    return true;
  }
...
std::vector<std::filesystem::path> ImportedSharedLibraryArtifacts(
  ...
  for (const auto& input : inputs) {
    ...
    if (!ShouldStageImportedSharedLibraryArtifact(input)) {
      continue;
    }
...
bool FileExists(const std::filesystem::path& path) {
  std::error_code ec;
  return std::filesystem::exists(HostArtifactPath(path), ec) && !ec;
}
