...
void LogCodegenProgress(const std::string& message) {
  ...
  if (!debug_codegen) {
    std::cerr << "[uv] codegen " << message << "\n";
  } else {
    std::cerr << "[uv] codegen pid=" << CurrentProcessId() << " "
              << message << "\n";
  }
  std::cerr.flush();
}
  ...
  for (const auto& module : project.modules) {
    if (cache.ast_modules.find(module.path) == cache.ast_modules.end()) {
      LogCodegenProgress("cache-build-error stage=module-coverage missing=" +
                         module.path);
      std::cerr << "[uv] BuildCodegenCache: missing AST module entry for path '"
                << module.path << "'\n";
...
std::optional<ModuleCodegen> LowerCodegenModule(codegen::LowerCtx& ctx,
  ...
  if (ctx.resolve_failed || ctx.codegen_failed) {
    ...
    std::cerr << "[uv] EnsureCodegenModule: lowering failed for module '"
              << entry.path_key << "'"
              << " (resolve_failed="
...
std::optional<llvm::Reloc::Model> RelocationModelFor(
    const project::Project& project,
    project::TargetProfile target_profile) {
  if (!project::IsSharedLibrary(project)) {
    return std::nullopt;
  }
  if (project::ObjectFormatOf(target_profile) == project::ObjectFormat::Coff) {
    return std::nullopt;
  }
  return llvm::Reloc::PIC_;
}

std::string_view RelocationModelName(
    std::optional<llvm::Reloc::Model> relocation_model) {
  if (!relocation_model.has_value()) {
    return "default";
  }
  switch (*relocation_model) {
...
llvm::TargetMachine* GetThreadLocalTargetMachine(const llvm::Triple& triple,
  ...
  const std::string triple_key =
      ...
      "|reloc=" + std::string(RelocationModelName(relocation_model));
...
std::optional<CodegenObjectAndIR> EmitObjAndOptionalIRForModule(
  ...
  if (!cached.has_value()) {
    LogCodegenProgress("emit-obj-error module=" + module_name +
                       " stage=lower-ir");
    std::cerr << "[uv] EmitObjForModule: module="
  ...
  if (verify_llvm_module) {
    ...
    if (llvm::verifyModule(*bundle.module, &verify_os)) {
      LogCodegenProgress("emit-obj-error module=" + module_name +
                         " stage=verify");
      std::cerr << "[uv] EmitObjForModule: LLVM module verification failed:\n"
                << verify_os.str() << "\n";
      if (debug_obj) {
  ...
  std::string err;
  const std::optional<llvm::Reloc::Model> relocation_model =
      RelocationModelFor(project, target_profile);
  llvm::TargetMachine* machine =
      GetThreadLocalTargetMachine(triple, opt_level, relocation_model, err);
  if (!machine) {
    LogCodegenProgress("emit-obj-error module=" + module_name +
                       " stage=target-machine");
    std::cerr << "[uv] EmitObjForModule: target machine unavailable for "
                 "triple '"
              << triple.str() << "': " << err << "\n";
    SPEC_RULE("EmitObj-Err");
    return std::nullopt;
  ...
  if (machine->addPassesToEmitFile(pass, dest, nullptr,
    ...
    std::cerr << "[uv] EmitObjForModule: addPassesToEmitFile failed\n";
    SPEC_RULE("EmitObj-Err");
    return std::nullopt;
  }
  ...
  for (std::size_t i = 0; i < sema_ctx.sigma.mods.size(); ++i) {
    ...
    if (!inserted) {
      LogCodegenProgress("cache-build-error stage=duplicate-module path=" +
                         module_key);
      std::cerr << "[uv] BuildCodegenCache: duplicate module path '"
                << module_key << "'\n";
      cache->ok.store(false);
      break;
    }
    ...
    for (const auto& module : sema_ctx.sigma.mods) {
      ...
      if (!codegen::RegisterModuleSignatures(module, cache->ctx)) {
        LogCodegenProgress("cache-build-error stage=register-signatures module=" +
                           module_log_name);
        std::cerr << "[uv] BuildCodegenCache: signature registration failed for module '"
                  << module_key << "'"
                  << " (resolve_failed="
      ...
      for (const auto& module : sema_ctx.sigma.mods) {
        ...
        if (cache->ctx.resolve_failed || cache->ctx.codegen_failed) {
          LogCodegenProgress(
              ...
              module_log_name);
          std::cerr << "[uv] BuildCodegenCache: static metadata registration failed for module '"
                    << module_key << "'"
                    << " (resolve_failed="
                    << (cache->ctx.resolve_failed ? "true" : "false")
                    << ", codegen_failed="
...
std::optional<std::size_t> EnsureCodegenModule(CodegenCache& cache,
  ...
  bool should_lower = false;
  {
    ...
    if (target_it == cache.ast_modules.end() || target_it->second == nullptr) {
      ...
      std::cerr << "[uv] EnsureCodegenModule: module not found for path '"
                << module_key << "'\n";
      cache.ok.store(false);
