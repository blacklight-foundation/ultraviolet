...
bool RenameIncrementalFile(const std::filesystem::path& source,
...
bool CopyIncrementalFile(const std::filesystem::path& source,
                         const std::filesystem::path& destination) {
  std::error_code ec;
...
bool ReplaceIncrementalFile(const std::filesystem::path& temp_path,
                            const std::filesystem::path& path) {
  constexpr int kMaxAttempts = 20;
  constexpr auto kRetryDelay = std::chrono::milliseconds(10);

  for (int attempt = 0; attempt < kMaxAttempts; ++attempt) {
    if (RenameIncrementalFile(temp_path, path)) {
      return true;
    }

    std::error_code existing_remove_ec;
    std::filesystem::remove(path, existing_remove_ec);
    if (RenameIncrementalFile(temp_path, path)) {
      return true;
    }

    if (CopyIncrementalFile(temp_path, path)) {
      std::error_code temp_remove_ec;
      std::filesystem::remove(temp_path, temp_remove_ec);
      return true;
    }
...
bool IncrementalEnabled() {
  const std::optional<bool> override = ultraviolet::core::IncrementalOverride();
  if (override.has_value()) {
    return *override;
  }
  const std::optional<bool> manifest = ultraviolet::core::ManifestIncremental();
...
bool SaveIncrementalManifest(
  ...
  if (ReplaceIncrementalFile(temp_path, manifest_path)) {
    return true;
  }
