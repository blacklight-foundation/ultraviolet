...
//
// SOURCE FILE: ultraviolet-bootstrap/src/02_syntax/tokenize.cpp
...
// Triggered by UV_DEBUG_LEX environment variable.
...
void DebugLexFail(const core::SourceFile& source,
  ...
  std::cerr << "[uv] lex: Max-Munch-Err at scalar=" << index
            << " byte=" << byte_index
            << " codepoint=U+"
  ...
  std::cerr << "[uv] lex: context=\"" << context << "\"\n";
  std::cerr << "[uv] lex: window=[";
  for (std::size_t i = lo; i < hi; ++i) {
    if (i > lo) {
      std::cerr << " ";
    }
...
TokenizeDiagnosticResult TokenizeWithDiagnostics(const core::SourceFile& source) {
  ...
  if (core::IsDebugEnabled("lex") || core::IsDebugEnabled("parse")) {
    std::cerr << "[uv] unicode: token-count=" << lexed.output.tokens.size()
