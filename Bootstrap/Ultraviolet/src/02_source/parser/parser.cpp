...
using ultraviolet::lexer::FilterNewlines;
using ultraviolet::lexer::Token;
using ultraviolet::lexer::TokenKind;
using ultraviolet::lexer::TokenizeWithDiagnostics;
using ultraviolet::lexer::UnsafeSpans;
...
ParseFileResult ParseFile(const core::SourceFile& source) {
  ...
  if (debug_phases) {
    std::cerr << "[uv] parsefile: tokenize " << source.path << "\n";
  }
  ...
  if (debug_phases) {
    std::cerr << "[uv] parsefile: attach-docs " << source.path << "\n";
  }
