...
ParseItemResult ParseItem(Parser parser) {
  ...
  // Handle "use" (not valid as a top-level item in Ultraviolet grammar)
  ...
  if (const Token* tok = Tok(parser); tok) {
    ...
    // "struct" -> Ultraviolet uses "record"
    ...
    // "impl" -> Ultraviolet has no impl blocks
