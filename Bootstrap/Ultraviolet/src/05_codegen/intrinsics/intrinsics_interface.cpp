...
RuntimeCategoryMap BuildRuntimeCategoryMap() {
  ...
  // IO
  AddRuntimeSymbol(categories, RuntimeSymbolCategory::IO, BuiltinSymIOOpenRead());
  AddRuntimeSymbol(categories, RuntimeSymbolCategory::IO, BuiltinSymIOOpenWrite());
  AddRuntimeSymbol(categories, RuntimeSymbolCategory::IO, BuiltinSymIOOpenAppend());
  AddRuntimeSymbol(categories, RuntimeSymbolCategory::IO, BuiltinSymIOCreateWrite());
  AddRuntimeSymbol(categories, RuntimeSymbolCategory::IO, BuiltinSymIOReadFile());
  AddRuntimeSymbol(categories, RuntimeSymbolCategory::IO, BuiltinSymIOReadBytes());
  AddRuntimeSymbol(categories, RuntimeSymbolCategory::IO, BuiltinSymIOWriteFile());
  AddRuntimeSymbol(categories, RuntimeSymbolCategory::IO, BuiltinSymIOWriteStdout());
  AddRuntimeSymbol(categories, RuntimeSymbolCategory::IO, BuiltinSymIOWriteStderr());
  AddRuntimeSymbol(categories, RuntimeSymbolCategory::IO, BuiltinSymIOExists());
  AddRuntimeSymbol(categories, RuntimeSymbolCategory::IO, BuiltinSymIORemove());
  AddRuntimeSymbol(categories, RuntimeSymbolCategory::IO, BuiltinSymIOOpenDir());
  AddRuntimeSymbol(categories, RuntimeSymbolCategory::IO, BuiltinSymIOCreateDir());
  AddRuntimeSymbol(categories, RuntimeSymbolCategory::IO, BuiltinSymIOEnsureDir());
  AddRuntimeSymbol(categories, RuntimeSymbolCategory::IO, BuiltinSymIOKind());
  AddRuntimeSymbol(categories, RuntimeSymbolCategory::IO, BuiltinSymIORestrict());
  AddRuntimeSymbol(categories, RuntimeSymbolCategory::IO, BuiltinSymFileReadReadAll());
  AddRuntimeSymbol(categories, RuntimeSymbolCategory::IO, BuiltinSymFileReadReadAllBytes());
  AddRuntimeSymbol(categories, RuntimeSymbolCategory::IO, BuiltinSymFileWriteWrite());
  AddRuntimeSymbol(categories, RuntimeSymbolCategory::IO, BuiltinSymFileWriteFlush());
  AddRuntimeSymbol(categories, RuntimeSymbolCategory::IO, BuiltinSymFileWriteClose());
  AddRuntimeSymbol(categories, RuntimeSymbolCategory::IO, BuiltinSymFileAppendWrite());
  AddRuntimeSymbol(categories, RuntimeSymbolCategory::IO, BuiltinSymFileAppendFlush());
  AddRuntimeSymbol(categories, RuntimeSymbolCategory::IO, BuiltinSymFileAppendClose());
  AddRuntimeSymbol(categories, RuntimeSymbolCategory::IO, BuiltinSymFileReadClose());
  AddRuntimeSymbol(categories, RuntimeSymbolCategory::IO, BuiltinSymDirIterOpenNext());
  AddRuntimeSymbol(categories, RuntimeSymbolCategory::IO, BuiltinSymDirIterOpenClose());
...
RuntimeSymbolCategory CategorizeRuntimeSymbol(const std::string& symbol) {
  ...
  if (symbol.rfind(language.runtime_symbol_prefix, 0) == 0) {
    ...
    if (suffix.rfind("io", 0) == 0) {
      return RuntimeSymbolCategory::IO;
    }
...
bool RuntimeUsesForeignABI(const std::string& symbol) {
  ...
  switch (CategorizeRuntimeSymbol(symbol)) {
    case RuntimeSymbolCategory::Conformance:
...
std::optional<RuntimeFuncInfo> GetRuntimeFuncInfo(const std::string& symbol) {
  ...
  if (symbol == RuntimePanicSym()) {
    info.noreturn = true;
    info.params.push_back(make_param("code", t_u32));
    info.ret = t_never;
    return info;
  }
  ...
  if (symbol == BuiltinSymIOOpenWrite()) {
    info.params.push_back(make_param("self", t_file_system));
    info.params.push_back(make_param("path", t_string_view));
    info.ret = make_outcome(t_file_write, t_io_error);
    return info;
  }
  if (symbol == BuiltinSymIOOpenAppend()) {
    info.params.push_back(make_param("self", t_file_system));
    info.params.push_back(make_param("path", t_string_view));
    info.ret = make_outcome(t_file_append, t_io_error);
    return info;
  }
  if (symbol == BuiltinSymIOCreateWrite()) {
    info.params.push_back(make_param("self", t_file_system));
    info.params.push_back(make_param("path", t_string_view));
    info.ret = make_outcome(t_file_write, t_io_error);
    return info;
  ...
  if (symbol == BuiltinSymIOWriteFile()) {
    info.params.push_back(make_param("self", t_file_system));
    info.params.push_back(make_param("path", t_string_view));
  ...
  if (symbol == BuiltinSymIOWriteStdout()) {
    info.params.push_back(make_param("self", t_file_system));
    info.params.push_back(make_param("data", t_string_view));
    info.ret = make_outcome(t_unit, t_io_error);
    return info;
  }
  if (symbol == BuiltinSymIOWriteStderr()) {
    info.params.push_back(make_param("self", t_file_system));
    info.params.push_back(make_param("data", t_string_view));
    info.ret = make_outcome(t_unit, t_io_error);
    return info;
  }
  if (symbol == BuiltinSymIOExists()) {
    info.params.push_back(make_param("self", t_file_system));
    info.params.push_back(make_param("path", t_string_view));
    info.ret = t_bool;
    return info;
  }
  if (symbol == BuiltinSymIORemove()) {
    info.params.push_back(make_param("self", t_file_system));
    info.params.push_back(make_param("path", t_string_view));
    info.ret = make_outcome(t_unit, t_io_error);
    return info;
  }
  if (symbol == BuiltinSymIOOpenDir()) {
    info.params.push_back(make_param("self", t_file_system));
    info.params.push_back(make_param("path", t_string_view));
    info.ret = make_outcome(t_dir_iter_open, t_io_error);
    return info;
  }
  if (symbol == BuiltinSymIOCreateDir()) {
    info.params.push_back(make_param("self", t_file_system));
    info.params.push_back(make_param("path", t_string_view));
    info.ret = make_outcome(t_unit, t_io_error);
    return info;
  }
  if (symbol == BuiltinSymIOEnsureDir()) {
    info.params.push_back(make_param("self", t_file_system));
    info.params.push_back(make_param("path", t_string_view));
    info.ret = make_outcome(t_unit, t_io_error);
    return info;
  }
  if (symbol == BuiltinSymIOKind()) {
    info.params.push_back(make_param("self", t_file_system));
    info.params.push_back(make_param("path", t_string_view));
    info.ret = make_outcome(analysis::MakeTypePath({"FileKind"}), t_io_error);
    return info;
  }
  if (symbol == BuiltinSymIORestrict()) {
    info.params.push_back(make_param("self", t_file_system));
    info.params.push_back(make_param("path", t_string_view));
  ...
  if (symbol == BuiltinSymSystemExit()) {
    info.noreturn = true;
    info.params.push_back(make_param("self", t_system));
    info.params.push_back(make_param("code", t_i32));
    info.ret = t_never;
    return info;
  }
  ...
  if (symbol == ConcurrencySymParallelBegin()) {
    // UVDynObject is ABI-lowered by reference on Win64 for this call boundary.
  ...
  if (symbol == ConcurrencySymDispatchRun()) {
    // UVRange and UVStringView are aggregate ABI parameters at this boundary.
