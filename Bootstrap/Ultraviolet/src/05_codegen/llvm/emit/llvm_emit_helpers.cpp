...
namespace ultraviolet::codegen::emit_detail {
    ...
    llvm::Value *EmitContextFieldValue(LLVMEmitter &emitter,
                                       llvm::IRBuilder<> &builder,
                                       ...
                                       std::string_view field_name)
    {
      ...
      auto *context_struct =
          ctx_value ? llvm::dyn_cast<llvm::StructType>(ctx_value->getType()) : nullptr;
      const bool uses_declared_layout =
          context_struct && context_struct->getNumElements() >= fields.size();

      std::size_t declared_index = 0u;
      std::size_t compact_index = 0u;
      for (const std::string_view field : fields)
      {
        const std::optional<analysis::TypeRef> field_type =
            analysis::ContextFieldType(field);
        if (!field_type.has_value() || !*field_type)
        {
          ++declared_index;
          continue;
        }
        ...
        const auto size =
            ::ultraviolet::analysis::layout::SizeOf(
                scope, *field_type).value_or(0u);
        if (field == field_name)
        {
          if (size == 0u)
          {
            llvm::Type *llvm_type = emitter.GetLLVMType(*field_type);
            return llvm_type && !llvm_type->isVoidTy()
                       ? llvm::Constant::getNullValue(llvm_type)
                       : nullptr;
          }
          if (ctx_ptr && ctx_ptr->getType()->isPointerTy())
          {
            const std::optional<std::uint64_t> slot = runtime_field_slot(field);
            llvm::Type *llvm_type = emitter.GetLLVMType(*field_type);
            if (slot.has_value() && llvm_type && !llvm_type->isVoidTy())
            {
              ...
              const std::uint64_t slot_size =
                  ::ultraviolet::analysis::layout::DynLayoutOf(scope).layout.size;
              llvm::Value *offset =
                  llvm::ConstantInt::get(i64_type, *slot * slot_size);
              llvm::Value *field_ptr =
                  builder.CreateGEP(i8_type, ctx_ptr, offset);
              return builder.CreateLoad(llvm_type, field_ptr);
            }
          }
          if (!ctx_value)
          {
          ...
          const std::size_t extract_index =
              uses_declared_layout ? declared_index : compact_index;
          if (context_struct &&
              extract_index >= context_struct->getNumElements())
          {
            return nullptr;
          }
