...
bool TargetUsesSharedLibraryDefinitionVisibility(
...
llvm::Value *MaterializeRuntimeValueRef(llvm::IRBuilder<> *builder,
                                        llvm::Value *value,
                                        llvm::Type *storage_ty,
                                        llvm::StringRef name)
{
  llvm::AllocaInst *slot = CreateRuntimeEntryAlloca(builder, storage_ty, name);
  if (!slot || !value)
  {
    return slot;
  }
...
llvm::Value *MaterializeRuntimeSystemSelfRef(llvm::IRBuilder<> *builder,
                                             llvm::LLVMContext &context,
                                             llvm::StringRef name)
{
  return CreateRuntimeEntryAlloca(builder, llvm::Type::getInt8Ty(context), name);
}
  ...
  void LLVMEmitter::EmitEntryPoint()
  {
    ...
    // Call the Ultraviolet main function
    llvm::Function *uv_main = nullptr;
    ...
    std::uint64_t panic_code_offset = 4;
    {
      ...
      const auto layout = ::ultraviolet::analysis::layout::RecordLayoutOf(scope, {
    ...
    if (!runtime_panic_fn)
    {
      llvm::FunctionType *panic_ty =
          llvm::FunctionType::get(llvm::Type::getVoidTy(context_), {GetOpaquePtr()}, false);
    ...
    llvm::FunctionType *callee_ty = uv_main->getFunctionType();
    ...
    auto entry_context_field_value =
      ...
      return EmitContextFieldValue(
          *this, irb, entry_scope, ctx_ptr, ctx_value, field_name);
    };
    ...
    auto build_entry_context_bundle =
      ...
      if (const auto *dyn = std::get_if<analysis::TypeDynamic>(&cur->node))
      {
        ...
        return entry_context_field_value(
            irb, root_ctx_ptr, root_ctx_loaded, field_name);
      }
      ...
      if (const auto *path = std::get_if<analysis::TypePathType>(&cur->node))
      {
                ...
                analysis::LookupRecordDecl(entry_scope, path->path))
        {
          ...
          auto *target_struct = llvm::dyn_cast<llvm::StructType>(target_ll);
          llvm::Function *current_fn =
              irb.GetInsertBlock() ? irb.GetInsertBlock()->getParent() : nullptr;
          if (!target_struct || !current_fn)
          {
            return nullptr;
          }

          struct ContextBundleRecordField
          {
            std::string name;
            analysis::TypeRef type;
          };

          std::vector<ContextBundleRecordField> context_fields;
          ...
          for (const auto &member : record->members)
          {
            ...
            context_fields.push_back(
                ContextBundleRecordField{std::string(field->name), lowered.type});
            field_types.push_back(lowered.type);
          }
          ...
          const auto layout = ComputeLayoutLLVMRecord(
              *this,
              ...
              cur,
              field_types,
              ::ultraviolet::analysis::layout::ResolveRecordLayoutOptions(
                  record->attrs));
          if (!layout.has_value() ||
              layout->fields.size() < context_fields.size())
          {
            return nullptr;
          }
          ...
          llvm::AllocaInst *aggregate_slot =
              entry_builder.CreateAlloca(target_struct, nullptr, "entry_ctx_bundle");
          irb.CreateStore(llvm::Constant::getNullValue(target_struct), aggregate_slot);
          llvm::Type *i8_type = llvm::Type::getInt8Ty(context_);
          llvm::Type *i64_type = llvm::Type::getInt64Ty(context_);
          for (std::size_t i = 0; i < context_fields.size(); ++i)
          {
            const LayoutLLVMField &layout_field = layout->fields[i];
            if (layout_field.size == 0u)
            {
              continue;
            }
            llvm::Value *field_value = self(
                self,
                irb,
                context_fields[i].type,
                context_fields[i].name,
                root_ctx_ptr,
                root_ctx_loaded);
            llvm::Type *field_ty = layout_field.llvm_type
                                       ? layout_field.llvm_type
                                       : GetLLVMType(context_fields[i].type);
            ...
            if (field_value)
            {
              field_value = CoerceTo(&irb, field_value, field_ty);
            }
            if (!field_value)
            {
              field_value = llvm::Constant::getNullValue(field_ty);
            }
            llvm::Value *field_ptr = irb.CreateGEP(
                i8_type,
                aggregate_slot,
                llvm::ConstantInt::get(i64_type, layout_field.offset));
            llvm::StoreInst *store = irb.CreateStore(field_value, field_ptr);
            store->setAlignment(llvm::Align(std::max<std::uint64_t>(
                1u, layout_field.align)));
          }
          return irb.CreateLoad(target_struct, aggregate_slot);
        }
      }
    ...
    llvm::CallInst *call = builder->CreateCall(callee_ty, uv_main, call_args);
    call->setCallingConv(uv_main->getCallingConv());
    ...
    if (runtime_panic_fn)
    {
      ...
      llvm::Value *panic_arg =
          MaterializeRuntimeValueRef(builder, panic_code, i32_ty, "entry_panic_code_arg");
      panic_arg = CoerceTo(builder, panic_arg, panic_ty->getParamType(0));
      if (!panic_arg)
      {
    ...
    else
    {
      ...
      llvm::Value *system_self =
          MaterializeRuntimeSystemSelfRef(builder, context_, "entry_system_self");
      llvm::Value *panic_code_ref =
          MaterializeRuntimeValueRef(builder, panic_code, i32_ty, "entry_exit_code");
      builder->CreateCall(runtime_exit_fn->getFunctionType(),
                          runtime_exit_fn,
                          {system_self, panic_code_ref});
      builder->CreateUnreachable();
    }
  ...
  void LLVMEmitter::EmitLibraryEntryPoint()
  {
    ...
    llvm::Type *panic_out_ty = GetLLVMType(PanicOutType());
    if (!opaque_ptr_ty || !panic_record_ty || !panic_out_ty)
    {
      if (current_ctx_)
      {
        current_ctx_->ReportCodegenFailure();
    ...
    auto panic_offsets = [&]() {
      ...
      const analysis::ScopeContext &scope = BuildScope(current_ctx_);
      const auto layout = ::ultraviolet::analysis::layout::RecordLayoutOf(
          scope,
    ...
    auto build_panic_out_slot = [&](llvm::IRBuilder<> &irb,
                                    llvm::Value *panic_ptr,
                                    const char *name) -> llvm::Value * {
      llvm::AllocaInst *panic_out_slot =
          irb.CreateAlloca(panic_out_ty, nullptr, name);
      llvm::Value *panic_out_value = CoerceTo(&irb, panic_ptr, panic_out_ty);
      if (!panic_out_value && panic_ptr && panic_ptr->getType() == panic_out_ty)
      {
        panic_out_value = panic_ptr;
      }
      if (!panic_out_value && panic_ptr && panic_ptr->getType()->isPointerTy() &&
          panic_out_ty->isPointerTy())
      {
        panic_out_value = irb.CreatePointerCast(panic_ptr, panic_out_ty);
      }
      if (!panic_out_value)
      {
        if (current_ctx_)
      ...
      irb.CreateStore(panic_out_value, panic_out_slot);
    ...
    auto call_proc_with_panic = [&](llvm::IRBuilder<> &irb,
      ...
      for (unsigned i = 0; i < fn->arg_size(); ++i)
      {
        ...
        {
          arg = CoerceTo(&irb, panic_out_slot, fn->getFunctionType()->getParamType(i));
          if (!arg && panic_out_slot &&
              panic_out_slot->getType() == fn->getFunctionType()->getParamType(i))
          {
            arg = panic_out_slot;
          }
        }
    ...
    llvm::Value *attach_panic_out_slot =
        build_panic_out_slot(*builder, panic_record_ptr, "dll_attach_panic_out");
    if (!attach_panic_out_slot)
    {
      attach_panic_out_slot = llvm::ConstantPointerNull::get(
          llvm::cast<llvm::PointerType>(opaque_ptr_ty));
    }
    ...
    clear_panic_record(*builder, panic_record_ptr);
    llvm::Value *detach_panic_out_slot =
        build_panic_out_slot(*builder, panic_record_ptr, "dll_detach_panic_out");
    if (!detach_panic_out_slot)
    {
      detach_panic_out_slot = llvm::ConstantPointerNull::get(
          llvm::cast<llvm::PointerType>(opaque_ptr_ty));
    }
  ...
  {
    if (!current_ctx_ || !current_ctx_->shared_library_project ||
        !TargetSupportsCtorDtorArrays(target_profile_) ||
    ...
    if (ctor_fn && ctor_fn->empty())
    {
      ...
      llvm::Value *panic_code_ref = MaterializeRuntimeValueRef(
          builder,
          llvm::ConstantInt::get(i32_ty, PanicCode(PanicReason::ForeignPre)),
          i32_ty,
          "library_ctor_panic_code");
    ...
    if (dtor_fn && dtor_fn->empty())
    {
      ...
      llvm::Value *panic_code_ref = MaterializeRuntimeValueRef(
          builder,
          llvm::ConstantInt::get(i32_ty, PanicCode(PanicReason::ForeignPost)),
          i32_ty,
          "library_dtor_panic_code");
      builder->CreateCall(runtime_panic_fn,
                          {panic_code_ref});
      builder->CreateUnreachable();
  ...
  void LLVMEmitter::ApplySharedLibraryDefinitionVisibility()
  {
    if (!current_ctx_ || !current_ctx_->shared_library_project ||
        ...
        !TargetUsesSharedLibraryDefinitionVisibility(target_profile_))
    {
      return;
    }
