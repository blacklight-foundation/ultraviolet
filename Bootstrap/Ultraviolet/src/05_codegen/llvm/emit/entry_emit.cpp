...
using namespace emit_detail;
  ...
  void LLVMEmitter::EmitEntryPoint()
  {
    ...
    auto entry_context_field_value =
      ...
      {
        const auto size = ::ultraviolet::analysis::layout::SizeOf(entry_scope, field.type).value_or(0u);
      ...
      if (const auto *path = std::get_if<analysis::TypePathType>(&cur->node))
      {
                ...
                analysis::LookupRecordDecl(entry_scope, path->path))
        {
          ...
          for (const auto &member : record->members)
          {
            ...
            const auto field_size = ::ultraviolet::analysis::layout::SizeOf(entry_scope, lowered.type).value_or(0u);
            if (field_size == 0u)
            {
              continue;
            }
