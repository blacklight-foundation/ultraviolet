...
llvm::AllocaInst *CreateHostedRuntimeAlloca(llvm::IRBuilder<> &builder,
                                            llvm::Type *type,
                                            llvm::StringRef name)
{
...
llvm::Value *MaterializeHostedRuntimeValueRef(llvm::IRBuilder<> &builder,
                                              llvm::Value *value,
                                              llvm::Type *storage_ty,
                                              llvm::StringRef name)
{
  llvm::AllocaInst *slot = CreateHostedRuntimeAlloca(builder, storage_ty, name);
  if (slot && value)
  {
  ...
  void LLVMEmitter::EmitHostedLifecycleExports()
  {
    ...
    if (!panic_record_ty || !panic_out_ty)
    {
    ...
    auto build_panic_out_slot = [&](llvm::IRBuilder<> &builder,
                                    llvm::Value *panic_ptr,
                                    const char *name) -> llvm::Value * {
      llvm::AllocaInst *panic_out_slot =
          builder.CreateAlloca(panic_out_ty, nullptr, name);
    ...
    auto call_proc_with_panic = [&](llvm::IRBuilder<> &builder,
      ...
      for (unsigned i = 0; i < fn->arg_size(); ++i)
      {
            ...
            *abi.param_indices.back() == i)
        {
          ...
          if (!arg && panic_out_slot && panic_out_slot->getType() == param_ty)
          {
            arg = panic_out_slot;
          }
        }
    ...
    if (create_fn->empty())
    {
      ...
      llvm::Value *panic_out_slot =
          build_panic_out_slot(*builder, panic_ptr, "host_create_panic_out");
      if (!panic_out_slot)
      {
        panic_out_slot = llvm::ConstantPointerNull::get(opaque_ptr_ptr_ty);
      }
           ...
           ++module_index)
      {
        ...
        call_proc_with_panic(*builder, InitFn(module_path), panic_out_slot, env_ptr);
    ...
    if (destroy_fn->empty())
    {
      ...
      llvm::Value *panic_ptr =
          build_env_slot_ptr(*builder, env_ptr, hosted_layout_.panic_offset, panic_record_ty);
      llvm::Value *panic_out_slot =
          build_panic_out_slot(*builder, panic_ptr, "host_destroy_panic_out");
      if (!panic_out_slot)
      {
        panic_out_slot = llvm::ConstantPointerNull::get(opaque_ptr_ptr_ty);
      }
      ...
      for (auto it = current_ctx_->init_order.rbegin();
           ...
           ++it)
      {
        call_proc_with_panic(*builder, DeinitFn(*it), panic_out_slot, env_ptr);
  ...
  void LLVMEmitter::EmitHostedExportThunks()
  {
    ...
    auto runtime_panic_fn = [&]() -> llvm::Function * {
      ...
      {
        llvm::FunctionType *fn_ty =
            llvm::FunctionType::get(llvm::Type::getVoidTy(context_), {opaque_ptr_ty}, false);
        fn = llvm::Function::Create(
            fn_ty,
            llvm::GlobalValue::ExternalLinkage,
            RuntimePanicSym(),
    ...
    auto context_field_value =
      ...
      return EmitContextFieldValue(*this, irb, scope, ctx_ptr, ctx_value, field_name);
    };
    ...
    auto build_context_bundle =
      ...
      if (const auto *dyn = std::get_if<analysis::TypeDynamic>(&cur->node))
      {
        ...
        return context_field_value(irb, root_ctx_ptr, root_ctx_value, field_name);
      }
      ...
      if (const auto *path = std::get_if<analysis::TypePathType>(&cur->node))
      {
                ...
                analysis::LookupRecordDecl(scope, path->path))
        {
          ...
          llvm::AllocaInst *aggregate_slot =
              entry_builder.CreateAlloca(target_struct, nullptr, "host_ctx_bundle");
    ...
    for (const auto &info : current_ctx_->hosted_exports)
    {
      ...
      const bool hosted_win64_aggregate_carrier =
          GetTargetProfile() == project::TargetProfile::X86_64Win64;
      ABICallResult thunk_abi = ComputeCallABI(
          *this,
          thunk_params,
          info.ret,
          /*use_c_abi_aggregate_sret=*/hosted_win64_aggregate_carrier,
      ...
      auto emit_hosted_boundary_failure =
        ...
        llvm::Value *panic_code_ref = MaterializeHostedRuntimeValueRef(
            irb, panic_code_value, i32_ty, "host_panic_code");
        irb.CreateCall(runtime_panic_fn, {panic_code_ref});
