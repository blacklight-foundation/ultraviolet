...
void IRInstructionVisitor::operator()(const IRContextBundleBuild &build) const
{
  ...
  auto context_field_value =
    ...
    return EmitContextFieldValue(emitter, irb, scope, ctx_ptr, ctx_value, field_name);
  };
  ...
  auto build_context_bundle =
    ...
    if (const auto *dyn = std::get_if<analysis::TypeDynamic>(&cur->node))
    {
      ...
      return context_field_value(irb, ctx_ptr, ctx_loaded, field_name);
    }
      ...
      if (const ast::RecordDecl *record = analysis::LookupRecordDecl(scope, path->path))
      {
        ...
        const auto layout = ComputeLayoutLLVMRecord(
            ...
            ::ultraviolet::analysis::layout::ResolveRecordLayoutOptions(
                record->attrs));
        if (!layout.has_value() || layout->fields.size() < context_fields.size())
        {
          return nullptr;
        }
        ...
        llvm::AllocaInst *aggregate_slot =
            entry_builder.CreateAlloca(target_struct, nullptr, "ctx_bundle");
        irb.CreateStore(llvm::Constant::getNullValue(target_struct), aggregate_slot);
        llvm::Type *i8_type = llvm::Type::getInt8Ty(emitter.GetContext());
        llvm::Type *i64_type = llvm::Type::getInt64Ty(emitter.GetContext());
        for (std::size_t i = 0; i < context_fields.size(); ++i)
        {
          ...
          llvm::Type *field_ty = layout_field.llvm_type
                                     ...
                                     : emitter.GetLLVMType(context_fields[i].type);
