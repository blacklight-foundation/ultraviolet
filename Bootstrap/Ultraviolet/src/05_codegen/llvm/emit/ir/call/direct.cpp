...
void IRInstructionVisitor::operator()(const IRCall &call) const
{
  ...
  auto build_call_abi_params =
      [&](const LowerCtx::ProcSigInfo &proc_sig) -> std::vector<IRParam>
  {
    if (!callee_symbol.empty())
    {
      return BuildProcABIParams(emitter, callee_symbol, proc_sig.params);
    }

    std::vector<IRParam> params = proc_sig.params;
    const bool call_supplies_panic_out =
        !call.args.empty() &&
        call.args.back().kind == IRValue::Kind::Local &&
        call.args.back().name == std::string(kPanicOutName);
    if (call_supplies_panic_out &&
        (params.empty() || params.back().name != std::string(kPanicOutName)))
    {
      params.push_back(PanicOutParam());
    }
    return params;
  };
  ...
  if (!callee && call.callee.kind == IRValue::Kind::Symbol)
  {
    ...
    else
    {
      ...
      std::vector<AttrSet> decl_param_attrs;
      if (sig)
      {
        const std::vector<IRParam> call_abi_params =
            build_call_abi_params(*sig);
        ...
        decl_param_attrs = abi.llvm_param_attrs;
      }
