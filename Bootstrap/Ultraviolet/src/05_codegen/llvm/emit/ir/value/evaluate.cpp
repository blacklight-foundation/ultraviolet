  ...
  for (const auto &module : ctx.sigma->mods)
  {
    ...
    for (const auto &item : module.items)
    {
      if (const auto *proc = std::get_if<ast::ProcedureDecl>(&item))
      {
        if (proc->name == name)
        {
          return MangleProcInModule(module, *proc);
        }
      }
    }
    break;
  ...
  llvm::Value *LLVMEmitter::EvaluateIRValue(const IRValue &val)
  {
    ...
    if (track_eval_cycle)
    {
      ...
      if (std::find(eval_stack.begin(), eval_stack.end(), *eval_key) != eval_stack.end())
      {
        ...
        {
          std::cerr << "[uv] recursive EvaluateIRValue cycle for key=" << *eval_key;
          if (!eval_stack.empty())
          {
            std::cerr << " stack=[";
        ...
        if (symbol_type)
        {
          if (llvm::Type *symbol_ll = GetLLVMType(symbol_type))
          {
            ...
            if (const LowerCtx *ctx_for_align = GetCurrentCtx())
            {
              ...
              if (const auto align =
                      ::ultraviolet::analysis::layout::AlignOf(scope, symbol_type))
              {
    ...
    case IRValue::Kind::Opaque:
    {
      ...
      if (!derived)
      {
        if (core::IsDebugEnabled("obj"))
        {
          ...
          std::cerr << "[uv] missing derived value in "
      ...
      case DerivedValueInfo::Kind::AddrLocal:
      {
        ...
        if (!local)
        {
          if (current_ctx_)
          {
            std::cerr << "[uv] missing local address storage"
                      << " name=" << derived->name;
            if (module_)
            {
      ...
      case DerivedValueInfo::Kind::AddrIndex:
      {
        ...
        if (debug_addr_index)
        {
          llvm::Function *fn =
              builder->GetInsertBlock() ? builder->GetInsertBlock()->getParent() : nullptr;
          std::cerr << "[uv] AddrIndex materialize in "
                    << (fn ? fn->getName().str() : std::string("<no-func>"))
                    << " result=" << val.name
                    << " base="
      ...
      case DerivedValueInfo::Kind::Tuple:
      {
        ...
        if (materialized && debug_tuple)
        {
          llvm::Function *fn =
              builder->GetInsertBlock() ? builder->GetInsertBlock()->getParent() : nullptr;
          std::cerr << "[uv] tuple materialized in "
                    << (fn ? fn->getName().str() : std::string("<no-func>"))
        ...
        if (!materialized && core::IsDebugEnabled("obj"))
        {
          ...
          std::cerr << "[uv] tuple materialization failed in "
                    << (fn ? fn->getName().str() : std::string("<no-func>"))
                    << " tuple_index=" << derived->tuple_index
                    << " base_type="
      ...
      case DerivedValueInfo::Kind::EnumPayloadIndex:
      {
        ...
        if (!member.ok)
        {
          ...
          if (enum_decl)
          {
            if (const auto enum_layout = ::ultraviolet::analysis::layout::EnumLayoutOf(
                    scope,
                    *enum_decl,
                    enum_generic_args,
      ...
      case DerivedValueInfo::Kind::ModalField:
      {
        ...
        if (!member.ok)
        {
          ...
          if (modal_decl)
          {
            if (const auto modal_layout = ::ultraviolet::analysis::layout::ModalLayoutOf(scope, *modal_decl, base_modal_args))
            {
      ...
      case DerivedValueInfo::Kind::EnumLit:
      {
        ...
        const auto enum_layout = ::ultraviolet::analysis::layout::EnumLayoutOf(
            scope,
            *enum_decl,
            enum_generic_args,
            ::ultraviolet::analysis::layout::ResolveEnumLayoutOptions(enum_decl->attrs));
        ...
        if (!derived->vtable_sym.empty())
        {
          ...
          if (!vtable_ptr && current_ctx_ && current_ctx_->sigma)
          {
            ...
            if (lazy_vtable_type && !lazy_class_path.empty())
            {
              ...
              {
                GlobalVTable lazy_vtable = ::ultraviolet::codegen::EmitVTable(
