...
void ApplyABIParamAttrs(llvm::LLVMContext& context,
                        llvm::Function* func,
                        const std::vector<AttrSet>& param_attrs) {
  if (!func) {
    return;
  }

  for (std::size_t idx = 0; idx < param_attrs.size(); ++idx) {
    if (idx >= func->arg_size()) {
      continue;
    }
    llvm::AttrBuilder builder(context);
    AddAttrSetToBuilder(builder, param_attrs[idx]);
    if (builder.hasAttributes()) {
      func->addParamAttrs(static_cast<unsigned>(idx), builder);
    }
  }
}
...
void ApplyABIParamAttrs(llvm::LLVMContext& context,
  ...
  for (std::size_t idx = 0; idx < param_attrs.size(); ++idx) {
    ...
    AddAttrSetToBuilder(builder, param_attrs[idx]);
    if (builder.hasAttributes()) {
      call->addParamAttrs(static_cast<unsigned>(idx), builder);
    }
  }
}
