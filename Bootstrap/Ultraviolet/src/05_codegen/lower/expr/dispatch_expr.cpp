...
LowerResult LowerDispatchExpr(
  ...
  if (ctx.expr_type && node.range) {
    const analysis::TypeRef range_type = ctx.expr_type(*node.range);
    if (const auto range_elem_type = analysis::RangeElementType(range_type)) {
      elem_type = *range_elem_type;
    } else {
  ...
  analysis::TypeRef body_type_from_dispatch;
  analysis::TypeRef body_type_from_tail;
  if (ctx.expr_type) {
    if (has_reduce) {
      body_type_from_dispatch = analysis::StripPerm(ctx.expr_type(expr));
      if (!body_type_from_dispatch) {
        body_type_from_dispatch = ctx.expr_type(expr);
      }
    }
    if (node.body && node.body->tail_opt) {
      body_type_from_tail =
          analysis::StripPerm(ctx.expr_type(*node.body->tail_opt));
      if (!body_type_from_tail) {
  ...
  if (has_reduce) {
    if (body_type_from_dispatch) {
      body_type = body_type_from_dispatch;
    } else if (body_type_from_tail) {
      body_type = body_type_from_tail;
    } else {
      ctx.ReportCodegenFailure();
    }
    if (body_type_from_dispatch && body_type_from_tail) {
      const auto equiv =
          analysis::TypeEquiv(body_type_from_dispatch, body_type_from_tail);
...

  {
    ...
    if (has_reduce && !IsUnitType(body_type)) {
      IRWritePtr write;
