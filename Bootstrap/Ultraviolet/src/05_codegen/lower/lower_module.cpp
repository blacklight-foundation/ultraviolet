...
//
// SOURCE FILE: ultraviolet-bootstrap/src/04_codegen/lower/lower_module.cpp
...
//
// DEPENDENCIES:
//   - ultraviolet/src/05_codegen/lower/lower_proc.h (LowerProc)
//   - ultraviolet/src/05_codegen/lower/lower_stmt.h (LowerBlock, LowerStmtList)
//   - ultraviolet/src/05_codegen/abi/abi.h (NeedsPanicOut, PanicOutType)
//   - ultraviolet/src/05_codegen/cleanup.h (ComputeCleanupPlanForCurrentScope, EmitCleanup)
//   - ultraviolet/src/05_codegen/dyn_dispatch.h (EmitVTable)
//   - ultraviolet/src/05_codegen/globals.h (EmitGlobal, EmitModuleInitFn, EmitModuleDeinitFn)
//   - ultraviolet/src/05_codegen/mangle.h (MangleProc, MangleMethod, MangleStateMethod, etc.)
//   - ultraviolet/src/04_analysis/layout/layout.h (LowerTypeForLayout)
//   - ultraviolet/src/04_analysis/types/types.h (TypeRef, MakeTypePrim, etc.)
//   - ultraviolet/src/04_analysis/composite/classes.h (TypeImplementsClass)
//   - ultraviolet/src/04_analysis/composite/record_methods.h (RecvTypeForReceiver, RecvModeOf)
//
// REFACTORING NOTES:
...
analysis::LowerTypeResult LowerTypeForMethod(const analysis::ScopeContext& scope,
  ...
  if (auto lowered =
          ::ultraviolet::analysis::layout::LowerTypeForLayout(scope, resolved_type)) {
...
bool RegisterModuleSignatures(const ast::ASTModule& module, LowerCtx& ctx) {
  ...
  init_sig.params.push_back(::ultraviolet::codegen::PanicOutParam());
  init_sig.ret = analysis::MakeTypePrim("()");
  register_internal_proc(init_sig);

  ProcIR deinit_sig;
  deinit_sig.symbol = DeinitFn(module.path);
  deinit_sig.params.push_back(::ultraviolet::codegen::PanicOutParam());
              ...
              std::visit(
                  [&](const auto& proc) {
                    ...
                    if constexpr (std::is_same_v<PT, ast::ExternProcDecl>) {
                      ...
                      // parameters, not ordinary aliased Ultraviolet parameters.
...
IRDecls ExpandIR(const IRDecls& decls, LowerCtx& ctx) {
  ...
  if (ctx.sigma != nullptr) {
    for (const auto& symbol : CollectVTableRefs(expanded_decls, ctx)) {
      ...
      ProcIR glue = ::ultraviolet::codegen::EmitDropGlue(info->type, ctx);
      if (seen_symbols.insert(glue.symbol).second) {
        expanded_decls.push_back(std::move(glue));
      }

      GlobalVTable vtable =
          ::ultraviolet::codegen::EmitVTable(info->type, info->class_path, class_it->second, ctx);
