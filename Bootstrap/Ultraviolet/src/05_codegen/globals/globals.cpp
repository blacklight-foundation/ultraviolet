...
//
// SOURCE FILE: ultraviolet-bootstrap/src/04_codegen/globals.cpp
//   - EmitGlobal function for static declarations
//   - Global initialization handling
//
// DEPENDENCIES:
//   - ultraviolet/include/05_codegen/globals/globals.h
//   - ultraviolet/include/05_codegen/ir/ir_model.h (GlobalConst, GlobalZero)
//   - ultraviolet/include/04_analysis/layout/layout.h (SizeOf, AlignOf)
//   - ultraviolet/include/05_codegen/symbols/mangle.h (MangleStatic)
//
// REFACTORING NOTES:
...
static analysis::TypeRef StaticAnnotationType(const ast::Binding& binding,
  ...
  if (auto lowered =
          ::ultraviolet::analysis::layout::LowerTypeForLayout(scope, ann_type)) {
    return *lowered;
  }
  return nullptr;
}
...
EmitGlobalResult EmitGlobal(const ast::StaticDecl& item,
    ...
    if (binding.init) {
      if (item.mut == ast::Mutability::Let) {
        if (auto bytes = ConstInit(init_type, *binding.init)) {
          ...
          const auto align =
              ::ultraviolet::analysis::layout::AlignOf(layout_scope, init_type);
          if (!align.has_value()) {
      ...
      const auto size = ::ultraviolet::analysis::layout::SizeOf(layout_scope, init_type);
      if (!size.has_value()) {
        ctx.ReportCodegenFailure();
        return result;
      }
      gz.size = *size;
      const auto align = ::ultraviolet::analysis::layout::AlignOf(layout_scope, init_type);
      if (!align.has_value()) {
        ctx.ReportCodegenFailure();
        return result;
      }
  ...
  for (const auto& name : names) {
    ...
    gz.externally_visible = externally_visible;
    gz.export_from_shared_library = export_from_shared_library;
    const auto size = ::ultraviolet::analysis::layout::SizeOf(layout_scope, type);
    if (!size.has_value()) {
      ctx.ReportCodegenFailure();
      result.decls.clear();
      return result;
    ...
    gz.size = *size;
    const auto align = ::ultraviolet::analysis::layout::AlignOf(layout_scope, type);
    if (!align.has_value()) {
      ctx.ReportCodegenFailure();
      result.decls.clear();
      return result;
