...
//
// DEPENDENCIES:
//   - ultraviolet/include/05_codegen/checks/checks.h
//   - ultraviolet/include/05_codegen/ir/ir_model.h (IRLowerPanic, IRPanicCheck)
//   - ultraviolet/include/04_analysis/layout/layout.h (SizeOf)
...
LowerResult LowerTransmute(analysis::TypeRef from_type,
  ...
  if (ctx.sigma && from_type && to_type) {
    ...
    scope.sigma = *ctx.sigma;
    scope.sigma_source = ctx.sigma;
    from_size = ::ultraviolet::analysis::layout::SizeOf(scope, from_type);
    to_size = ::ultraviolet::analysis::layout::SizeOf(scope, to_type);
  }
