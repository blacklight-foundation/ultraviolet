...
struct ABICallResult {
    ...
    // True when the lowered result is delivered through the leading ABI
    // result pointer. Runtime explicit-out functions use the same storage path
    // but must not receive LLVM's sret attribute.
    bool has_sret = false;
