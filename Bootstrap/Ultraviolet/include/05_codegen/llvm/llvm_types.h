...
// Construct: Ultraviolet Type to LLVM Type Mapping
...
// AddrSpace(T) = 0 for all Ultraviolet pointer types
constexpr unsigned kDefaultAddressSpace = 0;

// LLVMPtrTy(T) = ptr addrspace(AddrSpace(T))
// All pointers in Ultraviolet use opaque pointers in LLVM 21
...
// LLVMPrim(name) - Maps Ultraviolet primitive type names to LLVM types
// i8, u8 → i8
// i16, u16 → i16
// i32, u32 → i32
// i64, u64 → i64
...
// ::ultraviolet::analysis::layout::Layout: [disc_type, padding, [payload_size x i8], tail_padding]
...
// ::ultraviolet::analysis::layout::Layout matches ModalLayout with tagged discriminant + max payload blob
