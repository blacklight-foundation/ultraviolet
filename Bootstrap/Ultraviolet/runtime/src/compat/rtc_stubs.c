...
typedef union __declspec(align(16)) UVRtHalfABIValue {
  __m128 vector;
  uint16_t half[8];
} UVRtHalfABIValue;
#endif
...
static float uv_rt_float_from_bits(uint32_t bits) {
  union {
    uint32_t bits;
    float value;
  } u;
...
static uint32_t uv_rt_float_to_bits(float value) {
  union {
    uint32_t bits;
    float value;
  } u;
...
static float uv_rt_extend_half_bits(uint16_t value) {
  ...
  if (exponent == 0u) {
    if (fraction == 0u) {
      return uv_rt_float_from_bits(sign);
    }
  ...
  if (exponent == 0x1fu) {
    ...
    return uv_rt_float_from_bits(sign | 0x7f800000u | payload);
  }
...
static uint16_t uv_rt_trunc_float_to_half_bits(float value) {
  uint32_t bits = uv_rt_float_to_bits(value);
...
float __extendhfsf2(__m128 value) {
  UVRtHalfABIValue abi_value;
  abi_value.vector = value;
  return uv_rt_extend_half_bits(abi_value.half[0]);
}

__m128 __truncsfhf2(float value) {
  UVRtHalfABIValue abi_value;
  uint16_t bits = uv_rt_trunc_float_to_half_bits(value);
...
float __extendhfsf2(uint16_t value) {
  return uv_rt_extend_half_bits(value);
}

uint16_t __truncsfhf2(float value) {
  return uv_rt_trunc_float_to_half_bits(value);
}
#endif
#endif
