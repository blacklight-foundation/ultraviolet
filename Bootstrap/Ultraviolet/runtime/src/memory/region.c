...
void* ultraviolet_x3a_x3aruntime_x3a_x3aregion_x3a_x3aalloc(
  ...
  const uint64_t handle = self ? self->handle : 0;
  if (handle == 0) {
    return NULL;
  }
  const uint64_t size_value = size ? *size : 0;
  const uint64_t align_value = align ? *align : 0;

  UVRegionState* state = uv_region_state();
  uv_platform_rwlock_lock_exclusive(&state->lock);

  UVRegionArena* arena = uv_region_find(state, handle);
  const UVRegionEntry* entry = uv_region_resolve_entry(state, handle);
  if (!arena || !entry) {
    uv_platform_rwlock_unlock_exclusive(&state->lock);
    return NULL;
  }

  void* ptr = uv_region_alloc_arena(arena, (size_t)size_value, (size_t)align_value);
  if (ptr) {
...
uint64_t ultraviolet_x3a_x3aruntime_x3a_x3aregion_x3a_x3amark(const UVRegion* self) {
  ...
  if (!uv_region_stack_push(state, tag, handle, scope, mark, 1)) {
    uv_platform_rwlock_unlock_exclusive(&state->lock);
    return 0;
  }
...
void ultraviolet_x3a_x3aruntime_x3a_x3aregion_x3a_x3areset_x5fto(
  ...
  const uint64_t mark_value = mark ? *mark : 0;

  UVRegionState* state = uv_region_state();
  uv_platform_rwlock_lock_exclusive(&state->lock);

  UVRegionArena* arena = uv_region_find(state, handle);
  size_t entry_index = uv_region_find_mark_entry_index(state, handle, mark_value);
...
UVRegion ultraviolet_x3a_x3aruntime_x3a_x3aregion_x3a_x3areset_x5funchecked(
  ...
  UVRegionArena* arena = uv_region_find(state, handle);
  if (arena) {
    const size_t tag_count = uv_region_count_target_entries(state, handle);
    ...
    if (tag_count == 0 || fresh_tags) {
      ...
      for (size_t i = 0; i < state->region_count; ++i) {
        if (state->region_stack[i].target != handle) {
          continue;
        }
  ...
  uv_platform_rwlock_unlock_exclusive(&state->lock);
  return uv_region_make(UV_REGION_ACTIVE, handle);
}
...
UVRegion ultraviolet_x3a_x3aruntime_x3a_x3aregion_x3a_x3afree_x5funchecked(
  ...
  if (arena) {
    arena->alloc_count = 0;
    uv_region_stack_pop_target(state, handle);
    uv_region_remove(state, handle);
  ...
  uv_platform_rwlock_unlock_exclusive(&state->lock);
  return uv_region_make(UV_REGION_FREED, handle);
}
