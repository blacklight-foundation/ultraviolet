...
UVStringView ultraviolet_x3a_x3aruntime_x3a_x3astring_x3a_x3aslice(
  ...
  view.data = NULL;
  view.len = 0;
  const uint64_t start_value = start ? *start : 0;
  const uint64_t end_value = end ? *end : 0;

  if (!self || start_value > end_value || end_value > self->len) {
    return view;
  }

  view.data = self->data ? self->data + start_value : NULL;
  view.len = end_value - start_value;
  return view;
}
