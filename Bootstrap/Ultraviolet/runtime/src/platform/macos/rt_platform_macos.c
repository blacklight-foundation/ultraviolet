#include "rt_platform_macos_compat.h"
...
typedef struct UVHeapBlockHeader
{
  ...
  struct UVHeapBlockHeader *prev;
  struct UVHeapBlockHeader *next;
} UVHeapBlockHeader;
...
static struct UVRtHandleBase uv_rt_macos_process_heap_handle = {UV_RT_HANDLE_HEAP, 0, {0}};
static struct UVRtHandleBase uv_rt_macos_pseudo_thread_handle = {UV_RT_HANDLE_PSEUDO_THREAD, 0, {0}};
static struct UVRtHandleBase uv_rt_macos_stdin_handle = {UV_RT_HANDLE_FILE, 0, {.file = {STDIN_FILENO}}};
static struct UVRtHandleBase uv_rt_macos_stdout_handle = {UV_RT_HANDLE_FILE, 0, {.file = {STDOUT_FILENO}}};
static struct UVRtHandleBase uv_rt_macos_stderr_handle = {UV_RT_HANDLE_FILE, 0, {.file = {STDERR_FILENO}}};

static HANDLE uv_rt_macos_std_input = &uv_rt_macos_stdin_handle;
static HANDLE uv_rt_macos_std_output = &uv_rt_macos_stdout_handle;
static HANDLE uv_rt_macos_std_error = &uv_rt_macos_stderr_handle;

static CRITICAL_SECTION uv_rt_macos_std_handle_lock = PTHREAD_MUTEX_INITIALIZER;
static CRITICAL_SECTION uv_rt_macos_tls_slot_lock = PTHREAD_MUTEX_INITIALIZER;
static CRITICAL_SECTION uv_rt_macos_map_view_lock = PTHREAD_MUTEX_INITIALIZER;
static CRITICAL_SECTION uv_rt_macos_heap_lock = PTHREAD_MUTEX_INITIALIZER;
static int uv_rt_macos_tls_slot_used[128] = {0};
static UVMapViewNode *uv_rt_macos_map_views = NULL;
static UVHeapBlockHeader *uv_rt_macos_heap_blocks = NULL;
static atomic_uint uv_rt_macos_next_thread_id = ATOMIC_VAR_INIT(1u);

typedef struct UVRtMacOSPanicScope
{
  jmp_buf jump_env;
  struct UVRtMacOSPanicScope *prev;
  uv_platform_u32_t panic_code;
} UVRtMacOSPanicScope;

typedef struct UVRtMacOSThreadState
{
  uint64_t kernel_tid;
  DWORD logical_id;
  int priority;
  DWORD last_error;
  void *tls_values[128];
  UVRtMacOSPanicScope *panic_scope;
  int active;
} UVRtMacOSThreadState;

static CRITICAL_SECTION uv_rt_macos_thread_state_lock = PTHREAD_MUTEX_INITIALIZER;
static UVRtMacOSThreadState uv_rt_macos_thread_states[256] = {0};
static atomic_int uv_rt_macos_icu_data_configured = ATOMIC_VAR_INIT(0);

static void uv_rt_macos_set_errno_error(int err);

static uint64_t uv_rt_macos_current_kernel_tid(void)
{
...
static UVRtMacOSThreadState *uv_rt_macos_find_thread_state_locked(
    uint64_t kernel_tid)
{
  size_t i;
  for (i = 0u; i < (sizeof(uv_rt_macos_thread_states) /
                    sizeof(uv_rt_macos_thread_states[0]));
       ++i)
  {
    if (uv_rt_macos_thread_states[i].active &&
        uv_rt_macos_thread_states[i].kernel_tid == kernel_tid)
    {
      return &uv_rt_macos_thread_states[i];
    }
  }
  return NULL;
}

static UVRtMacOSThreadState *uv_rt_macos_current_thread_state(
    int create_if_missing)
{
  UVRtMacOSThreadState *state;
  uint64_t kernel_tid;
  size_t i;

  kernel_tid = uv_rt_macos_current_kernel_tid();
  EnterCriticalSection(&uv_rt_macos_thread_state_lock);
  state = uv_rt_macos_find_thread_state_locked(kernel_tid);
  if (state || !create_if_missing)
  {
    LeaveCriticalSection(&uv_rt_macos_thread_state_lock);
    return state;
  }
       ...
       ++i)
  {
    if (!uv_rt_macos_thread_states[i].active)
    {
      state = &uv_rt_macos_thread_states[i];
...
static void uv_rt_macos_release_current_thread_state(void)
{
  UVRtMacOSThreadState *state;

  EnterCriticalSection(&uv_rt_macos_thread_state_lock);
  state = uv_rt_macos_find_thread_state_locked(
      uv_rt_macos_current_kernel_tid());
  if (state)
  {
    memset(state, 0, sizeof(*state));
  }
...
static int uv_rt_macos_file_exists(const char *path)
{
  struct stat st;
  if (!path || path[0] == '\0')
  {
...
static int uv_rt_macos_data_directory_from_module(char *out_dir,
                                                  size_t out_dir_size)
{
  Dl_info info;
  ...
  {
    uint32_t raw_size = (uint32_t)sizeof(module_path);
    if (_NSGetExecutablePath(module_path, &raw_size) != 0)
    {
      return 0;
    }
  ...
  *slash = '\0';
  ...
  const int candidate_path_size = snprintf(candidate_path,
                                           sizeof(candidate_path),
                                           "%s/lib",
                                           module_path);
  if (candidate_path_size < 0 ||
      (size_t)candidate_path_size >= sizeof(candidate_path))
  {
    return 0;
  }
  ...
  for (i = 0u; i < 2u; ++i)
  {
    ...
    const int data_path_size = snprintf(data_path,
                                        sizeof(data_path),
                                        "%s/icudt72l.dat",
                                        candidates[i]);
    if (data_path_size < 0 || (size_t)data_path_size >= sizeof(data_path))
    {
      continue;
    }
    if (!uv_rt_macos_file_exists(data_path))
    {
      continue;
    }
...
static size_t uv_rt_macos_page_size(void)
{
...
static int uv_rt_macos_size_add(size_t lhs, size_t rhs, size_t *out)
{
...
static int uv_rt_macos_size_mul(size_t lhs, size_t rhs, size_t *out)
{
  if (!out)
  {
    return 0;
...
static size_t uv_rt_macos_heap_payload_size(size_t requested)
{
  return requested == 0u ? 1u : requested;
}

static int uv_rt_macos_round_mapping_size(size_t bytes, size_t *out)
{
  const size_t page_size = uv_rt_macos_page_size();
  ...
  if (remainder != 0u)
  {
    if (!uv_rt_macos_size_add(bytes, page_size - remainder, &rounded))
    {
      return 0;
    }
  }
...
static UVHeapBlockHeader *uv_rt_macos_heap_header(const void *memory)
{
  ...
  payload = (uintptr_t)memory;
  if (payload < (uintptr_t)sizeof(UVHeapBlockHeader))
  {
    return NULL;
  }
  return (UVHeapBlockHeader *)(payload - (uintptr_t)sizeof(UVHeapBlockHeader));
}

static int uv_rt_macos_heap_header_valid(const UVHeapBlockHeader *header)
{
  ...
  minimum_payload = uv_rt_macos_heap_payload_size(header->size);
  if (!uv_rt_macos_size_add(sizeof(UVHeapBlockHeader),
                            minimum_payload,
                            &minimum_mapping))
  {
    return 0;
...
static UVHeapBlockHeader *uv_rt_macos_heap_find_locked(const void *memory)
{
  UVHeapBlockHeader *candidate = uv_rt_macos_heap_header(memory);
  UVHeapBlockHeader *block = uv_rt_macos_heap_blocks;
  if (!candidate)
  {
    return NULL;
  }
...
static void uv_rt_macos_heap_link(UVHeapBlockHeader *header)
{
  if (!header)
  {
    return;
  }
  EnterCriticalSection(&uv_rt_macos_heap_lock);
  header->prev = NULL;
  header->next = uv_rt_macos_heap_blocks;
  if (uv_rt_macos_heap_blocks)
  {
    uv_rt_macos_heap_blocks->prev = header;
  }
  uv_rt_macos_heap_blocks = header;
  LeaveCriticalSection(&uv_rt_macos_heap_lock);
}

static int uv_rt_macos_heap_payload_is_live(const void *memory)
{
  int valid = 0;
  EnterCriticalSection(&uv_rt_macos_heap_lock);
  UVHeapBlockHeader *block = uv_rt_macos_heap_find_locked(memory);
  valid = block && uv_rt_macos_heap_header_valid(block);
  LeaveCriticalSection(&uv_rt_macos_heap_lock);
  return valid;
}

static UVHeapBlockHeader *uv_rt_macos_heap_unlink_live(const void *memory)
{
  UVHeapBlockHeader *block = NULL;
  EnterCriticalSection(&uv_rt_macos_heap_lock);
  block = uv_rt_macos_heap_find_locked(memory);
  if (!block || !uv_rt_macos_heap_header_valid(block))
  {
    LeaveCriticalSection(&uv_rt_macos_heap_lock);
    return NULL;
  }
  ...
  {
    uv_rt_macos_heap_blocks = block->next;
  }
  if (block->next)
  {
...
static void *uv_rt_macos_heap_alloc_unchecked(size_t bytes)
{
  const size_t payload_size = uv_rt_macos_heap_payload_size(bytes);
  size_t minimum_mapping = 0u;
  ...
  if (!uv_rt_macos_size_add(sizeof(UVHeapBlockHeader),
                            payload_size,
                            &minimum_mapping) ||
      !uv_rt_macos_round_mapping_size(minimum_mapping, &mapping_size))
  {
    return NULL;
  }
  ...
  header->alignment_padding = 0u;
  header->prev = NULL;
  header->next = NULL;
  uv_rt_macos_heap_link(header);
  return (void *)(header + 1);
}

static void *uv_rt_macos_heap_calloc_unchecked(size_t count, size_t size)
{
  size_t bytes = 0u;
  if (!uv_rt_macos_size_mul(count, size, &bytes))
  {
    return NULL;
  }
  return uv_rt_macos_heap_alloc_unchecked(bytes);
}

static int uv_rt_macos_heap_free_unchecked(void *memory)
{
  UVHeapBlockHeader *header = NULL;
  size_t mapping_size = 0u;
  if (!memory)
  ...
  header = uv_rt_macos_heap_unlink_live(memory);
  if (!header)
  {
    return 0;
  }
...
static int uv_rt_macos_timespec_from_millis(DWORD milliseconds,
...
static void uv_rt_macos_mutex_lock(CRITICAL_SECTION *section)
{
  if (!section)
  {
    return;
  }
  (void)pthread_mutex_lock(section);
}

static void uv_rt_macos_mutex_unlock(CRITICAL_SECTION *section)
{
  if (!section)
  {
    return;
  }
  (void)pthread_mutex_unlock(section);
}

static BOOL uv_rt_macos_condition_wait(CONDITION_VARIABLE *condition,
                                       ...
                                       DWORD milliseconds)
{
  ...
  {
    if (!uv_rt_macos_timespec_from_millis(milliseconds, &timeout))
    {
  ...
  do
  {
    rc = timeout_ptr ? pthread_cond_timedwait(condition, section, timeout_ptr)
                     : pthread_cond_wait(condition, section);
  } while (rc == EINTR);
  if (rc == 0)
  {
  ...
  uv_rt_macos_set_errno_error(rc);
  return FALSE;
}

static void uv_rt_macos_condition_wake(CONDITION_VARIABLE *condition,
                                       int wake_count)
{
  if (!condition)
  {
...
#define malloc(bytes) uv_rt_macos_heap_alloc_unchecked((bytes))
#define calloc(count, size) uv_rt_macos_heap_calloc_unchecked((count), (size))
#define free(memory) uv_rt_macos_heap_free_unchecked((memory))

static void uv_rt_macos_assign_current_thread_id(DWORD logical_id)
{
  UVRtMacOSThreadState *state;
  if (logical_id == 0u)
  {
  ...
  state = uv_rt_macos_current_thread_state(1);
  if (state)
  {
...
static DWORD uv_rt_macos_ensure_current_thread_id(void)
{
  UVRtMacOSThreadState *state = uv_rt_macos_current_thread_state(1);
  if (!state)
  {
    return 0u;
  }
  ...
  {
    state->logical_id = atomic_fetch_add(&uv_rt_macos_next_thread_id, 1u);
    if (state->logical_id == 0u)
    {
      state->logical_id =
          atomic_fetch_add(&uv_rt_macos_next_thread_id, 1u);
    }
  }
  return state->logical_id;
}

static DWORD uv_rt_macos_error_from_errno(int err)
{
  switch (err)
  {
  case 0:
...
static void uv_rt_macos_set_errno_error(int err)
{
  SetLastError(uv_rt_macos_error_from_errno(err));
}
...
static size_t uv_rt_macos_wide_cstr_len(LPCWSTR text)
{
  return text ? wcslen(text) : 0u;
}

static HANDLE uv_rt_macos_alloc_handle(int kind)
{
...
static int uv_rt_macos_handle_fd(HANDLE handle, int *out_fd)
{
...
static int uv_rt_macos_utf8_decode_one(const char *source,
                                       size_t source_len,
                                       size_t *index,
                                       uint32_t *out_cp)
{
...
static size_t uv_rt_macos_utf8_encode_one(uint32_t codepoint, char *dest)
{
  if (codepoint <= 0x7Fu)
  {
    if (dest)
...
static wchar_t *uv_rt_macos_wide_from_utf8_alloc(const char *source,
                                                 size_t source_len,
                                                 int add_nul,
                                                 size_t *out_len)
{
  ...
  while (index < source_len)
  {
    ...
    if (!uv_rt_macos_utf8_decode_one(source, source_len, &index, &codepoint))
    {
      SetLastError(ERROR_INVALID_PARAMETER);
      return NULL;
    }
...
static char *uv_rt_macos_utf8_from_wide_alloc(LPCWSTR source,
                                              size_t source_len,
                                              int add_nul,
                                              size_t *out_len)
{
  ...
  for (i = 0u; i < source_len; ++i)
  {
    ...
    count += uv_rt_macos_utf8_encode_one(codepoint, NULL);
  }
  ...
  for (i = 0u; i < source_len; ++i)
  {
    out_index += uv_rt_macos_utf8_encode_one((uint32_t)source[i], output + out_index);
  }
  if (add_nul)
  {
    output[out_index] = '\0';
...
static char *uv_rt_macos_path_from_wide_alloc(LPCWSTR path)
{
  size_t len = uv_rt_macos_wide_cstr_len(path);
  size_t utf8_len = 0u;
  char *utf8 = uv_rt_macos_utf8_from_wide_alloc(path, len, 1, &utf8_len);
  (void)utf8_len;
  if (!utf8)
  {
    return NULL;
...
static char *uv_rt_macos_strdup_or_null(const char *text)
{
  size_t len;
  char *copy;
  if (!text)
...
static char *uv_rt_macos_join_path_utf8(const char *lhs, const char *rhs)
{
...
{
  UVRtMacOSThreadState *state = uv_rt_macos_current_thread_state(0);
  return state ? state->last_error : ERROR_SUCCESS;
}

HANDLE GetProcessHeap(void)
{
  return &uv_rt_macos_process_heap_handle;
}
...
LPVOID HeapAlloc(HANDLE heap, DWORD flags, size_t bytes)
{
  ...
  (void)heap;
  (void)flags;
  block = (UVHeapBlockHeader *)uv_rt_macos_heap_alloc_unchecked(bytes);
  if (!block)
  {
    SetLastError(ERROR_NOT_ENOUGH_MEMORY);
    return NULL;
...
BOOL HeapFree(HANDLE heap, DWORD flags, LPVOID memory)
{
  ...
  if (!uv_rt_macos_heap_free_unchecked(memory))
  {
    SetLastError(ERROR_INVALID_PARAMETER);
    return FALSE;
  }
...
BOOL HeapValidate(HANDLE heap, DWORD flags, LPCVOID memory)
{
  ...
  if (!uv_rt_macos_heap_payload_is_live(memory))
  {
    SetLastError(ERROR_INVALID_PARAMETER);
    return FALSE;
  }
                        ...
                        int destination_length)
{
  ...
  source_len = (source_length < 0) ? uv_rt_macos_utf8_cstr_len(source) : (size_t)source_length;
  converted = uv_rt_macos_wide_from_utf8_alloc(source, source_len, source_length < 0, &written);
  if (!converted)
  {
    return 0;
  }
                        ...
                        BOOL *used_default_char)
{
  ...
  source_len = (source_length < 0) ? uv_rt_macos_wide_cstr_len(source) : (size_t)source_length;
  converted = uv_rt_macos_utf8_from_wide_alloc(source, source_len, source_length < 0, &written);
  if (!converted)
  {
    return 0;
  }
...
{
  size_t name_len = uv_rt_macos_wide_cstr_len(name);
  char *utf8_name = uv_rt_macos_utf8_from_wide_alloc(name, name_len, 1, NULL);
  const char *value;
  size_t value_len;
  wchar_t *wide_value;
  size_t wide_len;
  ...
  value = uv_rt_macos_process_env_utf8_value(utf8_name);
  free(utf8_name);
  if (!value)
  {
  ...
  wide_value = uv_rt_macos_wide_from_utf8_alloc(value, value_len, 1, &wide_len);
  if (!wide_value)
  {
    return 0u;
  }
...
DWORD GetEnvironmentVariableA(const char *name, char *buffer, DWORD size)
{
  ...
  value = uv_rt_macos_process_env_utf8_value(name);
  if (!value)
  {
    SetLastError(ERROR_ENVVAR_NOT_FOUND);
    return 0u;
...
static void *uv_rt_macos_thread_start_trampoline(void *raw_context)
{
  ...
  DWORD exit_code = 0u;
  uv_rt_macos_assign_current_thread_id(handle->u.thread.logical_id);
  handle->u.thread.tid = uv_rt_macos_current_kernel_tid();
  {
    UVRtMacOSThreadState *state = uv_rt_macos_current_thread_state(1);
    if (state)
    {
  ...
  uv_rt_macos_release_current_thread_state();
  free(context);
  return NULL;
}
                    ...
                    LPPROCESS_INFORMATION process_information)
{
  ...
  if (application_name)
  {
    app_utf8 = uv_rt_macos_path_from_wide_alloc(application_name);
    if (!app_utf8)
    {
      return FALSE;
    }
  ...
  if (command_line)
  {
    cmd_utf8 = uv_rt_macos_path_from_wide_alloc(command_line);
    if (!cmd_utf8)
    {
      free(app_utf8);
      return FALSE;
  ...
  {
    cwd_utf8 = uv_rt_macos_path_from_wide_alloc(current_directory);
    if (!cwd_utf8)
    {
      free(cmd_utf8);
      free(app_utf8);
  ...
  process_handle = uv_rt_macos_alloc_handle(UV_RT_HANDLE_PROCESS);
  if (!process_handle)
  {
    return FALSE;
  }
  ...
  thread_handle = uv_rt_macos_alloc_handle(UV_RT_HANDLE_TOKEN);
  if (!thread_handle)
  {
    free(process_handle);
    return FALSE;
...
static DWORD uv_rt_macos_wait_for_process(struct UVRtHandleBase *handle, DWORD milliseconds)
{
  int status = 0;
  pid_t waited;
  (void)milliseconds;
...
static DWORD uv_rt_macos_wait_for_thread(struct UVRtHandleBase *handle, DWORD milliseconds)
{
  EnterCriticalSection(&handle->u.thread.lock);
...
static DWORD uv_rt_macos_wait_for_event(struct UVRtHandleBase *handle, DWORD milliseconds)
{
...
DWORD WaitForSingleObject(HANDLE handle, DWORD milliseconds)
{
  ...
  case UV_RT_HANDLE_PROCESS:
    return uv_rt_macos_wait_for_process(base, milliseconds);
  case UV_RT_HANDLE_THREAD:
    return uv_rt_macos_wait_for_thread(base, milliseconds);
  case UV_RT_HANDLE_EVENT:
    return uv_rt_macos_wait_for_event(base, milliseconds);
...
{
  int rc = pthread_setspecific((pthread_key_t)index, value);
  if (rc != 0)
  {
    uv_rt_macos_set_errno_error(rc);
    return FALSE;
                         ...
                         PVOID *context)
{
  ...
  pthread_mutex_lock(&init_once->mutex);
  while (init_once->state == 1)
  {
    pthread_cond_wait(&init_once->condition, &init_once->mutex);
  }
  if (init_once->state == 2)
  {
    ok = TRUE;
  }
  else
  {
    ...
    init_once->state = 1;
    pthread_mutex_unlock(&init_once->mutex);
    ...
    init_once->state = ok ? 2 : 0;
    pthread_cond_broadcast(&init_once->condition);
  }
  if (context)
  {
...
{
  uv_rt_macos_mutex_lock(section);
}

void LeaveCriticalSection(CRITICAL_SECTION *section)
{
  uv_rt_macos_mutex_unlock(section);
}
...
BOOL SleepConditionVariableCS(CONDITION_VARIABLE *condition,
                              ...
                              DWORD milliseconds)
{
  return uv_rt_macos_condition_wait(condition, section, milliseconds);
}

void WakeConditionVariable(CONDITION_VARIABLE *condition)
{
  uv_rt_macos_condition_wake(condition, 1);
}

void WakeAllConditionVariable(CONDITION_VARIABLE *condition)
{
  uv_rt_macos_condition_wake(condition, INT_MAX);
}
...
{
  return uv_rt_macos_ensure_current_thread_id();
}

HANDLE GetCurrentThread(void)
{
  uv_rt_macos_ensure_current_thread_id();
  return &uv_rt_macos_pseudo_thread_handle;
}

int GetThreadPriority(HANDLE thread)
{
  struct UVRtHandleBase *base = (struct UVRtHandleBase *)thread;
  if (!thread || thread == &uv_rt_macos_pseudo_thread_handle)
  {
    UVRtMacOSThreadState *state = uv_rt_macos_current_thread_state(1);
...
BOOL SetThreadPriority(HANDLE thread, int priority)
{
  ...
  if (base->u.thread.tid == uv_rt_macos_current_kernel_tid())
  {
    UVRtMacOSThreadState *state = uv_rt_macos_current_thread_state(1);
    if (state)
    {
                    ...
                    DWORD *thread_id)
{
  ...
  (void)attributes;
  (void)creation_flags;
  handle = (struct UVRtHandleBase *)uv_rt_macos_alloc_handle(UV_RT_HANDLE_THREAD);
  if (!handle)
  {
    return NULL;
  }
  ...
  handle->u.thread.logical_id = atomic_fetch_add(&uv_rt_macos_next_thread_id, 1u);
  ...
  {
    int rc = pthread_create(&handle->u.thread.thread, &attr, uv_rt_macos_thread_start_trampoline, context);
    if (rc != 0)
    {
                   ...
                   LPCWSTR name)
{
  ...
  (void)attributes;
  (void)name;
  handle = (struct UVRtHandleBase *)uv_rt_macos_alloc_handle(UV_RT_HANDLE_EVENT);
  if (!handle)
  {
    return NULL;
  }
...
{
  HANDLE handle = NULL;
  EnterCriticalSection(&uv_rt_macos_std_handle_lock);
  switch (std_handle_id)
  {
  case STD_INPUT_HANDLE:
    handle = uv_rt_macos_std_input;
    break;
  case STD_OUTPUT_HANDLE:
    handle = uv_rt_macos_std_output;
    break;
  case STD_ERROR_HANDLE:
    handle = uv_rt_macos_std_error;
    break;
  default:
    handle = NULL;
    break;
  }
  LeaveCriticalSection(&uv_rt_macos_std_handle_lock);
  if (!handle)
  {
    SetLastError(ERROR_INVALID_HANDLE);
  }
...
BOOL SetStdHandle(DWORD std_handle_id, HANDLE handle)
{
  ...
  {
  case STD_INPUT_HANDLE:
    uv_rt_macos_std_input = handle;
    break;
  case STD_OUTPUT_HANDLE:
    uv_rt_macos_std_output = handle;
    break;
  case STD_ERROR_HANDLE:
    uv_rt_macos_std_error = handle;
    break;
  default:
    LeaveCriticalSection(&uv_rt_macos_std_handle_lock);
    SetLastError(ERROR_INVALID_HANDLE);
...
{
  int fd;
  if (!uv_rt_macos_handle_fd(handle, &fd))
  {
    return FALSE;
  }
  if (!isatty(fd))
...
static int uv_rt_macos_open_flags_from_access(DWORD desired_access, DWORD creation_disposition)
{
  int flags = 0;
                   ...
                   HANDLE template_file)
{
  ...
  (void)template_file;
  fd = uv_rt_macos_console_fd_from_name(path);
  if (fd < 0)
  {
    utf8_path = uv_rt_macos_path_from_wide_alloc(path);
    if (!utf8_path)
    {
      return INVALID_HANDLE_VALUE;
    }
    fd = open(utf8_path,
              uv_rt_macos_open_flags_from_access(desired_access, creation_disposition),
              0666);
    free(utf8_path);
  }
  if (fd < 0)
...
BOOL GetFileSizeEx(HANDLE handle, LARGE_INTEGER *size_out)
{
  int fd;
  struct stat st;
  if (!size_out || !uv_rt_macos_handle_fd(handle, &fd))
  {
    SetLastError(ERROR_INVALID_PARAMETER);
    return FALSE;
  }
...
static DWORD uv_rt_macos_attributes_from_stat(const struct stat *st)
{
  if (S_ISDIR(st->st_mode))
  {
...
{
  char *utf8_path = uv_rt_macos_path_from_wide_alloc(path);
  struct stat st;
  if (!utf8_path)
  {
  ...
  free(utf8_path);
  SetLastError(ERROR_SUCCESS);
  return uv_rt_macos_attributes_from_stat(&st);
}
...
static int uv_rt_macos_fill_find_data(const char *dir_path,
                                      const char *name,
                                      WIN32_FIND_DATAW *find_data)
{
  char *full_path = uv_rt_macos_join_path_utf8(dir_path, name);
  struct stat st;
  wchar_t *wide_name;
  size_t wide_len = 0u;
  if (!full_path)
  ...
  free(full_path);
  wide_name = uv_rt_macos_wide_from_utf8_alloc(name, strlen(name), 1, &wide_len);
  if (!wide_name)
  {
    return 0;
  }
  ...
  find_data->dwFileAttributes = uv_rt_macos_attributes_from_stat(&st);
  free(wide_name);
  SetLastError(ERROR_SUCCESS);
  return 1;
}

static int uv_rt_macos_find_next_match(struct UVRtHandleBase *handle,
                                       WIN32_FIND_DATAW *find_data)
{
  struct dirent *entry;
  while ((entry = readdir(handle->u.find.dir)) != NULL)
  {
    ...
    return uv_rt_macos_fill_find_data(handle->u.find.dir_path, entry->d_name, find_data);
  }
...
{
  char *utf8_pattern = uv_rt_macos_path_from_wide_alloc(pattern);
  char *last_sep;
  char *dir_path;
  char *wildcard;
  DIR *dir;
  ...
  {
    *last_sep = '\0';
    dir_path = uv_rt_macos_strdup_or_null(utf8_pattern[0] ? utf8_pattern : "/");
    wildcard = uv_rt_macos_strdup_or_null(last_sep + 1);
  }
  else
  {
    dir_path = uv_rt_macos_strdup_or_null(".");
    wildcard = uv_rt_macos_strdup_or_null(utf8_pattern);
  }
  free(utf8_pattern);
  ...
  handle = (struct UVRtHandleBase *)uv_rt_macos_alloc_handle(UV_RT_HANDLE_FIND);
  if (!handle)
  {
    closedir(dir);
    free(dir_path);
  ...
  if (!uv_rt_macos_find_next_match(handle, find_data))
  {
    closedir(dir);
    free(dir_path);
    free(wildcard);
...
BOOL FindNextFileW(HANDLE handle, WIN32_FIND_DATAW *find_data)
{
  ...
  return uv_rt_macos_find_next_match(base, find_data) ? TRUE : FALSE;
}
...
{
  const char *temp_dir = uv_rt_macos_process_env_utf8_value("TMPDIR");
  size_t wide_len = 0u;
  wchar_t *wide_temp;
  if (!temp_dir || temp_dir[0] == '\0')
  {
    temp_dir = uv_rt_macos_process_env_utf8_value("TEMP");
  }
  if (!temp_dir || temp_dir[0] == '\0')
  {
    temp_dir = uv_rt_macos_process_env_utf8_value("TMP");
  }
  if (!temp_dir || temp_dir[0] == '\0')
  {
    temp_dir = "/tmp";
  }
  wide_temp = uv_rt_macos_wide_from_utf8_alloc(temp_dir, strlen(temp_dir), 1, &wide_len);
  if (!wide_temp)
  {
    return 0u;
  }
...
{
  char *dir_path = uv_rt_macos_path_from_wide_alloc(path_name);
  char *prefix = uv_rt_macos_utf8_from_wide_alloc(prefix_string,
                                                  uv_rt_macos_wide_cstr_len(prefix_string),
                                                  1,
                                                  NULL);
  ...
  close(fd);
  wide_path = uv_rt_macos_wide_from_utf8_alloc(template_path, strlen(template_path), 1, &wide_len);
  if (!wide_path)
  {
                          ...
                          LPCWSTR name)
{
  ...
  {
    char *raw_name = uv_rt_macos_utf8_from_wide_alloc(name, uv_rt_macos_wide_cstr_len(name), 1, NULL);
    size_t i;
    if (!raw_name)
    {
      return NULL;
  ...
  handle = (struct UVRtHandleBase *)uv_rt_macos_alloc_handle(UV_RT_HANDLE_MAPPING);
  if (!handle)
  {
    close(fd);
    return NULL;
                     ...
                     size_t number_of_bytes_to_map)
{
  ...
  node->size = map_size;
  EnterCriticalSection(&uv_rt_macos_map_view_lock);
  node->next = uv_rt_macos_map_views;
  uv_rt_macos_map_views = node;
  LeaveCriticalSection(&uv_rt_macos_map_view_lock);
  SetLastError(ERROR_SUCCESS);
  return address;
}
...
BOOL CloseHandle(HANDLE handle)
{
  ...
  case UV_RT_HANDLE_THREAD:
    ...
    pthread_cond_destroy(&base->u.thread.cond);
    break;
  case UV_RT_HANDLE_EVENT:
    DeleteCriticalSection(&base->u.event.mutex);
    pthread_cond_destroy(&base->u.event.cond);
    break;
...
{
  void *symbol = NULL;
  if (!module || !symbol_name || symbol_name[0] == '\0')
  {
    SetLastError(ERROR_INVALID_PARAMETER);
    return NULL;
  }
...
uv_platform_u32_t uv_platform_backend_executable_path_utf8(
    ...
    uv_platform_u32_t size)
{
  char raw_stack_path[PATH_MAX + 1];
  char resolved_stack_path[PATH_MAX + 1];
  char *raw_path = raw_stack_path;
  char *raw_heap_path = NULL;
  const char *path = raw_stack_path;
  uint32_t raw_size = (uint32_t)sizeof(raw_stack_path);
  size_t len = 0u;
  ...
  if (_NSGetExecutablePath(raw_path, &raw_size) != 0)
  {
    ...
    raw_heap_path = (char *)malloc((size_t)raw_size);
    if (!raw_heap_path)
    {
      return 0u;
    }
    raw_path = raw_heap_path;
    if (_NSGetExecutablePath(raw_path, &raw_size) != 0)
    {
      free(raw_heap_path);
      return 0u;
  ...
  raw_path[raw_size - 1u] = '\0';
  if (realpath(raw_path, resolved_stack_path))
  {
    path = resolved_stack_path;
  }
  len = uv_rt_macos_utf8_cstr_len(path);
    ...
    uv_platform_u32_t size)
{
  ...
  size_t len = uv_rt_macos_utf8_cstr_len(argument);
  if (len >= (size_t)UINT32_MAX)
  {
    return 0u;
  }
    ...
    uv_platform_u32_t size)
{
  ...
  size_t len = uv_rt_macos_utf8_cstr_len(cwd);
  if (len >= (size_t)UINT32_MAX)
  {
    return 0u;
  }
...
void uv_platform_backend_icu_data_configure(void)
{
  ...
  if (atomic_exchange(&uv_rt_macos_icu_data_configured, 1) != 0)
  {
    return;
  }
  ...
  if (!uv_rt_macos_data_directory_from_module(data_dir, sizeof(data_dir)))
  {
    return;
  }
...
{
  return uv_rt_macos_current_kernel_tid();
}
