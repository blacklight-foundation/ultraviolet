...
{
  const UVStringView *rule_id_ref = &rule_id;
  const UVStringView *file_ref = &file;
  const UVStringView *payload_ref = &payload;
  UVTraceRecordInfo record_info;
  UVTraceRecordMeta record_meta;
  uv_trace_compute_record_info(payload_ref, &record_info);
  if (!uv_trace_begin_locked(
          rule_id_ref, file_ref, start_line, start_col, end_line, end_col,
          payload_ref, &record_info, &record_meta))
  {
    return;
  }
  if (payload_ref->data && payload_ref->len != 0)
  {
    uv_trace_write_payload_readable(payload_ref->data, payload_ref->len);
    uv_trace_write_console_pretty_locked(
        rule_id_ref, file_ref, start_line, start_col, payload_ref, &record_meta,
        &record_info, NULL, 0u);
  }
  else if (uv_rule_id_has_log_prefix(rule_id_ref))
  {
    const uint8_t *label_data = rule_id_ref->data;
    uint64_t label_len = rule_id_ref->len;
    ...
    uv_trace_write_console_pretty_locked(
        rule_id_ref, file_ref, start_line, start_col, NULL, &record_meta,
    ...
    uint8_t is_signed)
{
  const UVStringView *rule_id_ref = &rule_id;
  const UVStringView *file_ref = &file;
  const UVStringView *payload_prefix_ref = &payload_prefix;
  UVTraceRecordInfo record_info;
  UVTraceRecordMeta record_meta;
  uv_trace_compute_record_info(payload_prefix_ref, &record_info);
  if (!uv_trace_begin_locked(
          rule_id_ref, file_ref, start_line, start_col, end_line, end_col,
          payload_prefix_ref, &record_info, &record_meta))
  {
    return;
  }
  uv_trace_write_payload_prefix(payload_prefix_ref);
  ...
  uv_trace_write_console_pretty_locked(
      rule_id_ref, file_ref, start_line, start_col, payload_prefix_ref, &record_meta,
      &record_info,
    ...
    UVStringView actual)
{
  ...
  const UVStringView *file_ref = &file;
  const UVStringView *payload_prefix_ref = &payload_prefix;
  const UVStringView *actual_ref = &actual;
  UVTraceRecordInfo record_info;
  UVTraceRecordMeta record_meta;
  uv_trace_compute_record_info(payload_prefix_ref, &record_info);
  if (!uv_trace_begin_locked(
  ...
  if (actual_ref->data && actual_ref->len != 0u)
  {
    preview_len = uv_format_string_preview(
        actual_ref->data, actual_ref->len, preview, (uint64_t)sizeof(preview));
  }
  else
  {
  ...
  if (actual_ref->data && actual_ref->len != 0u)
  {
    uv_trace_write_encoded(actual_ref->data, actual_ref->len);
  }
  uv_trace_write_file_bytes((const uint8_t *)"\"", 1u);
  uv_trace_write_console_pretty_locked(
      rule_id_ref, file_ref, start_line, start_col, payload_prefix_ref, &record_meta,
    ...
    UVBytesView actual)
{
  ...
  const UVStringView *file_ref = &file;
  const UVStringView *payload_prefix_ref = &payload_prefix;
  const UVBytesView *actual_ref = &actual;
  UVTraceRecordInfo record_info;
  UVTraceRecordMeta record_meta;
  uv_trace_compute_record_info(payload_prefix_ref, &record_info);
  if (!uv_trace_begin_locked(
  ...
  preview_len = uv_format_bytes_preview(
      actual_ref->data, actual_ref->len, preview, (uint64_t)sizeof(preview));
  uv_trace_write_hex_bytes(actual_ref->data, actual_ref->len);
  uv_trace_write_console_pretty_locked(
      rule_id_ref, file_ref, start_line, start_col, payload_prefix_ref, &record_meta,
      &record_info,
      preview, preview_len);
