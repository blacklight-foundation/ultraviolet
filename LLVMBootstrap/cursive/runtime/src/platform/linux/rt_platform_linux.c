...
#define C0_HEAP_MAGIC 0xC0DEC0DE51A7EULL

enum C0RtHandleKind {
  C0_RT_HANDLE_HEAP = 1,
  C0_RT_HANDLE_PROCESS = 2,
  C0_RT_HANDLE_THREAD = 3,
  C0_RT_HANDLE_EVENT = 4,
  C0_RT_HANDLE_FILE = 5,
  C0_RT_HANDLE_FIND = 6,
  C0_RT_HANDLE_MAPPING = 7,
  C0_RT_HANDLE_TOKEN = 8,
  C0_RT_HANDLE_PSEUDO_THREAD = 9,
};

typedef struct C0HeapBlockHeader {
  uint64_t magic;
  size_t size;
  size_t mapping_size;
} C0HeapBlockHeader;

typedef struct C0MapViewNode {
  void* address;
  size_t size;
  struct C0MapViewNode* next;
} C0MapViewNode;

struct C0RtHandleBase {
  int kind;
  int owned;
  union {
    struct {
...
static struct C0RtHandleBase cursive_rt_linux_process_heap_handle = { C0_RT_HANDLE_HEAP, 0, { 0 } };
static struct C0RtHandleBase cursive_rt_linux_pseudo_thread_handle = { C0_RT_HANDLE_PSEUDO_THREAD, 0, { 0 } };
static struct C0RtHandleBase cursive_rt_linux_stdin_handle = { C0_RT_HANDLE_FILE, 0, { .file = { STDIN_FILENO } } };
static struct C0RtHandleBase cursive_rt_linux_stdout_handle = { C0_RT_HANDLE_FILE, 0, { .file = { STDOUT_FILENO } } };
static struct C0RtHandleBase cursive_rt_linux_stderr_handle = { C0_RT_HANDLE_FILE, 0, { .file = { STDERR_FILENO } } };

static HANDLE cursive_rt_linux_std_input = &cursive_rt_linux_stdin_handle;
static HANDLE cursive_rt_linux_std_output = &cursive_rt_linux_stdout_handle;
static HANDLE cursive_rt_linux_std_error = &cursive_rt_linux_stderr_handle;

static CRITICAL_SECTION cursive_rt_linux_std_handle_lock = { 0u };
static CRITICAL_SECTION cursive_rt_linux_tls_slot_lock = { 0u };
static CRITICAL_SECTION cursive_rt_linux_map_view_lock = { 0u };
static int cursive_rt_linux_tls_slot_used[128] = { 0 };
static C0MapViewNode* cursive_rt_linux_map_views = NULL;
static atomic_uint cursive_rt_linux_next_thread_id = ATOMIC_VAR_INIT(1u);

typedef struct CursiveRtLinuxPanicScope {
  jmp_buf jump_env;
  struct CursiveRtLinuxPanicScope* prev;
  cursive_platform_u32_t panic_code;
} CursiveRtLinuxPanicScope;

typedef struct CursiveRtLinuxThreadState {
  pid_t kernel_tid;
  DWORD logical_id;
  int priority;
  DWORD last_error;
  void* tls_values[128];
  CursiveRtLinuxPanicScope* panic_scope;
  int active;
} CursiveRtLinuxThreadState;

static CRITICAL_SECTION cursive_rt_linux_thread_state_lock = { 0u };
static CursiveRtLinuxThreadState cursive_rt_linux_thread_states[256] = { 0 };
static atomic_int cursive_rt_linux_icu_data_configured = ATOMIC_VAR_INIT(0);

static void cursive_rt_linux_set_errno_error(int err);

static pid_t cursive_rt_linux_current_kernel_tid(void) {
  return (pid_t)syscall(SYS_gettid);
}

static CursiveRtLinuxThreadState* cursive_rt_linux_find_thread_state_locked(
    pid_t kernel_tid) {
  size_t i;
  for (i = 0u; i < (sizeof(cursive_rt_linux_thread_states) /
                     sizeof(cursive_rt_linux_thread_states[0]));
       ++i) {
    if (cursive_rt_linux_thread_states[i].active &&
        cursive_rt_linux_thread_states[i].kernel_tid == kernel_tid) {
      return &cursive_rt_linux_thread_states[i];
    }
  }
  return NULL;
}

static CursiveRtLinuxThreadState* cursive_rt_linux_current_thread_state(
    int create_if_missing) {
  CursiveRtLinuxThreadState* state;
  pid_t kernel_tid;
  size_t i;

  kernel_tid = cursive_rt_linux_current_kernel_tid();
  EnterCriticalSection(&cursive_rt_linux_thread_state_lock);
  state = cursive_rt_linux_find_thread_state_locked(kernel_tid);
  if (state || !create_if_missing) {
    LeaveCriticalSection(&cursive_rt_linux_thread_state_lock);
    return state;
  }
  ...
  for (i = 0u; i < (sizeof(cursive_rt_linux_thread_states) /
       ...
       ++i) {
    if (!cursive_rt_linux_thread_states[i].active) {
      state = &cursive_rt_linux_thread_states[i];
...
static void cursive_rt_linux_release_current_thread_state(void) {
  CursiveRtLinuxThreadState* state;

  EnterCriticalSection(&cursive_rt_linux_thread_state_lock);
  state = cursive_rt_linux_find_thread_state_locked(
      cursive_rt_linux_current_kernel_tid());
  if (state) {
    memset(state, 0, sizeof(*state));
  }
  LeaveCriticalSection(&cursive_rt_linux_thread_state_lock);
...
static int cursive_rt_linux_file_exists(const char* path) {
  struct stat st;
  if (!path || path[0] == '\0') {
    return 0;
  }
...
static int cursive_rt_linux_data_directory_from_module(char* out_dir,
                                                       size_t out_dir_size) {
  Dl_info info;
  ...
  if (dladdr((void*)&cursive_platform_backend_icu_data_configure, &info) != 0 &&
  ...
  *slash = '\0';
  ...
  for (i = 0u; i < 2u; ++i) {
    ...
    if (!cursive_rt_linux_file_exists(data_path)) {
      continue;
    }
...
static size_t cursive_rt_linux_page_size(void) {
...
static int cursive_rt_linux_size_add(size_t lhs, size_t rhs, size_t* out) {
...
static int cursive_rt_linux_size_mul(size_t lhs, size_t rhs, size_t* out) {
  if (!out) {
    return 0;
  }
...
static size_t cursive_rt_linux_heap_payload_size(size_t requested) {
  return requested == 0u ? 1u : requested;
}

static int cursive_rt_linux_round_mapping_size(size_t bytes, size_t* out) {
  const size_t page_size = cursive_rt_linux_page_size();
  ...
  if (remainder != 0u) {
    if (!cursive_rt_linux_size_add(bytes, page_size - remainder, &rounded)) {
      return 0;
    }
  }
  *out = rounded;
...
static C0HeapBlockHeader* cursive_rt_linux_heap_header(const void* memory) {
  if (!memory) {
    return NULL;
  }
  return ((C0HeapBlockHeader*)memory) - 1;
}

static int cursive_rt_linux_heap_header_valid(const C0HeapBlockHeader* header) {
  size_t minimum_payload = 0u;
  size_t minimum_mapping = 0u;
  if (!header || header->magic != C0_HEAP_MAGIC) {
    return 0;
  }
  minimum_payload = cursive_rt_linux_heap_payload_size(header->size);
  if (!cursive_rt_linux_size_add(sizeof(C0HeapBlockHeader),
                                 minimum_payload,
                                 &minimum_mapping)) {
    return 0;
  }
...
static void* cursive_rt_linux_heap_alloc_unchecked(size_t bytes) {
  const size_t payload_size = cursive_rt_linux_heap_payload_size(bytes);
  size_t minimum_mapping = 0u;
  size_t mapping_size = 0u;
  C0HeapBlockHeader* header = NULL;
  if (!cursive_rt_linux_size_add(sizeof(C0HeapBlockHeader),
                                 payload_size,
                                 &minimum_mapping) ||
      !cursive_rt_linux_round_mapping_size(minimum_mapping, &mapping_size)) {
    return NULL;
  }

  header = (C0HeapBlockHeader*)mmap(NULL,
                                    mapping_size,
  ...
  header->magic = C0_HEAP_MAGIC;
  header->size = bytes;
  header->mapping_size = mapping_size;
  return (void*)(header + 1);
}

static void* cursive_rt_linux_heap_calloc_unchecked(size_t count, size_t size) {
  size_t bytes = 0u;
  if (!cursive_rt_linux_size_mul(count, size, &bytes)) {
    return NULL;
  }
  return cursive_rt_linux_heap_alloc_unchecked(bytes);
}

static void cursive_rt_linux_heap_free_unchecked(void* memory) {
  C0HeapBlockHeader* header = cursive_rt_linux_heap_header(memory);
  if (!header || !cursive_rt_linux_heap_header_valid(header)) {
    return;
  }
  header->magic = 0u;
...
static int cursive_rt_linux_futex_wait(volatile uint32_t* address,
                                       uint32_t expected,
...
static int cursive_rt_linux_futex_wake(volatile uint32_t* address,
                                       int wake_count) {
  return (int)syscall(SYS_futex,
                      (uint32_t*)address,
                      FUTEX_WAKE_PRIVATE,
...
static int cursive_rt_linux_timespec_from_millis(DWORD milliseconds,
...
static void cursive_rt_linux_mutex_lock(CRITICAL_SECTION* section) {
  ...
  for (;;) {
    ...
    while (__atomic_load_n(&section->state, __ATOMIC_RELAXED) == 2u) {
      if (cursive_rt_linux_futex_wait(&section->state, 2u, NULL) < 0 &&
          errno != EAGAIN &&
          errno != EINTR) {
        break;
      }
...
static void cursive_rt_linux_mutex_unlock(CRITICAL_SECTION* section) {
  if (!section) {
    return;
  }
  if (__atomic_fetch_sub(&section->state, 1u, __ATOMIC_RELEASE) != 1u) {
    __atomic_store_n(&section->state, 0u, __ATOMIC_RELEASE);
    cursive_rt_linux_futex_wake(&section->state, 1);
  }
}

static BOOL cursive_rt_linux_condition_wait(CONDITION_VARIABLE* condition,
  ...
  cursive_rt_linux_mutex_unlock(section);
  if (milliseconds != INFINITE) {
    cursive_rt_linux_timespec_from_millis(milliseconds, &timeout);
    timeout_ptr = &timeout;
  }
  rc = cursive_rt_linux_futex_wait(&condition->sequence, sequence, timeout_ptr);
  cursive_rt_linux_mutex_lock(section);
  ...
  cursive_rt_linux_set_errno_error(errno);
  return FALSE;
}

static void cursive_rt_linux_condition_wake(CONDITION_VARIABLE* condition,
                                            int wake_count) {
  if (!condition) {
    return;
  }
  __atomic_add_fetch(&condition->sequence, 1u, __ATOMIC_RELEASE);
  cursive_rt_linux_futex_wake(&condition->sequence, wake_count);
}

#define malloc(bytes) cursive_rt_linux_heap_alloc_unchecked((bytes))
#define calloc(count, size) cursive_rt_linux_heap_calloc_unchecked((count), (size))
#define free(memory) cursive_rt_linux_heap_free_unchecked((memory))

static void cursive_rt_linux_assign_current_thread_id(DWORD logical_id) {
  CursiveRtLinuxThreadState* state;
  if (logical_id == 0u) {
    return;
  }
  state = cursive_rt_linux_current_thread_state(1);
  if (state) {
    state->logical_id = logical_id;
  }
}

static DWORD cursive_rt_linux_ensure_current_thread_id(void) {
  CursiveRtLinuxThreadState* state = cursive_rt_linux_current_thread_state(1);
  if (!state) {
    return 0u;
  }
  if (state->logical_id == 0u) {
    state->logical_id = atomic_fetch_add(&cursive_rt_linux_next_thread_id, 1u);
    if (state->logical_id == 0u) {
      state->logical_id =
          atomic_fetch_add(&cursive_rt_linux_next_thread_id, 1u);
    }
  }
  return state->logical_id;
}

static DWORD cursive_rt_linux_error_from_errno(int err) {
  switch (err) {
    case 0:
...
static void cursive_rt_linux_set_errno_error(int err) {
  SetLastError(cursive_rt_linux_error_from_errno(err));
}

static size_t cursive_rt_linux_utf8_cstr_len(const char* text) {
  return text ? strlen(text) : 0u;
}

static const char* cursive_rt_linux_process_env_utf8_value(const char* name) {
...
static size_t cursive_rt_linux_wide_cstr_len(LPCWSTR text) {
  return text ? wcslen(text) : 0u;
}

static HANDLE cursive_rt_linux_alloc_handle(int kind) {
  struct C0RtHandleBase* handle =
      (struct C0RtHandleBase*)calloc(1u, sizeof(struct C0RtHandleBase));
  if (!handle) {
...
static int cursive_rt_linux_handle_fd(HANDLE handle, int* out_fd) {
  if (!handle || handle == INVALID_HANDLE_VALUE || !out_fd) {
    SetLastError(ERROR_INVALID_HANDLE);
    return 0;
  }
  if (((struct C0RtHandleBase*)handle)->kind != C0_RT_HANDLE_FILE) {
    SetLastError(ERROR_INVALID_HANDLE);
    return 0;
  }
  *out_fd = ((struct C0RtHandleBase*)handle)->u.file.fd;
  return 1;
}

static int cursive_rt_linux_utf8_decode_one(const char* source,
                              size_t source_len,
                              size_t* index,
                              uint32_t* out_cp) {
  size_t i;
...
static size_t cursive_rt_linux_utf8_encode_one(uint32_t codepoint, char* dest) {
  if (codepoint <= 0x7Fu) {
    if (dest) {
...
static wchar_t* cursive_rt_linux_wide_from_utf8_alloc(const char* source,
                                        size_t source_len,
                                        int add_nul,
                                        size_t* out_len) {
  size_t count = 0u;
  ...
  while (index < source_len) {
    ...
    if (!cursive_rt_linux_utf8_decode_one(source, source_len, &index, &codepoint)) {
      SetLastError(ERROR_INVALID_PARAMETER);
      return NULL;
    }
    ++count;
...
static char* cursive_rt_linux_utf8_from_wide_alloc(LPCWSTR source,
                                     size_t source_len,
                                     int add_nul,
                                     size_t* out_len) {
  size_t count = 0u;
  ...
  for (i = 0u; i < source_len; ++i) {
    ...
    count += cursive_rt_linux_utf8_encode_one(codepoint, NULL);
  }
  ...
  for (i = 0u; i < source_len; ++i) {
    out_index += cursive_rt_linux_utf8_encode_one((uint32_t)source[i], output + out_index);
  }
  if (add_nul) {
    output[out_index] = '\0';
  }
...
static char* cursive_rt_linux_path_from_wide_alloc(LPCWSTR path) {
  size_t len = cursive_rt_linux_wide_cstr_len(path);
  size_t utf8_len = 0u;
  char* utf8 = cursive_rt_linux_utf8_from_wide_alloc(path, len, 1, &utf8_len);
  size_t i;
  if (!utf8) {
    return NULL;
  }
...
static char* cursive_rt_linux_strdup_or_null(const char* text) {
  size_t len;
  char* copy;
  if (!text) {
    return NULL;
...
static char* cursive_rt_linux_join_path_utf8(const char* lhs, const char* rhs) {
...
DWORD GetLastError(void) {
  CursiveRtLinuxThreadState* state = cursive_rt_linux_current_thread_state(0);
  return state ? state->last_error : ERROR_SUCCESS;
}

HANDLE GetProcessHeap(void) {
  return &cursive_rt_linux_process_heap_handle;
}

LPVOID HeapAlloc(HANDLE heap, DWORD flags, size_t bytes) {
  C0HeapBlockHeader* block;
  (void)heap;
  (void)flags;
  block = (C0HeapBlockHeader*)malloc(bytes);
  if (!block) {
    SetLastError(ERROR_NOT_ENOUGH_MEMORY);
    return NULL;
  }
...
BOOL HeapFree(HANDLE heap, DWORD flags, LPVOID memory) {
  ...
  block = cursive_rt_linux_heap_header(memory);
  if (!cursive_rt_linux_heap_header_valid(block)) {
    SetLastError(ERROR_INVALID_PARAMETER);
    return FALSE;
  }
  cursive_rt_linux_heap_free_unchecked(memory);
  SetLastError(ERROR_SUCCESS);
  return TRUE;
}
...
int MultiByteToWideChar(UINT code_page,
  ...
  source_len = (source_length < 0) ? cursive_rt_linux_utf8_cstr_len(source) : (size_t)source_length;
  converted = cursive_rt_linux_wide_from_utf8_alloc(source, source_len, source_length < 0, &written);
  if (!converted) {
    return 0;
  }
...
int WideCharToMultiByte(UINT code_page,
  ...
  source_len = (source_length < 0) ? cursive_rt_linux_wide_cstr_len(source) : (size_t)source_length;
  converted = cursive_rt_linux_utf8_from_wide_alloc(source, source_len, source_length < 0, &written);
  if (!converted) {
    return 0;
  }
  required = written + ((source_length < 0) ? 1u : 0u);
...
DWORD GetEnvironmentVariableW(LPCWSTR name, LPWSTR buffer, DWORD size) {
  size_t name_len = cursive_rt_linux_wide_cstr_len(name);
  char* utf8_name = cursive_rt_linux_utf8_from_wide_alloc(name, name_len, 1, NULL);
  const char* value;
  size_t value_len;
  wchar_t* wide_value;
  size_t wide_len;
  ...
  value = cursive_rt_linux_process_env_utf8_value(utf8_name);
  free(utf8_name);
  if (!value) {
  ...
  wide_value = cursive_rt_linux_wide_from_utf8_alloc(value, value_len, 1, &wide_len);
  if (!wide_value) {
    return 0u;
  }
  if (!buffer || size == 0u) {
...
DWORD GetEnvironmentVariableA(const char* name, char* buffer, DWORD size) {
  ...
  value = cursive_rt_linux_process_env_utf8_value(name);
  if (!value) {
    SetLastError(ERROR_ENVVAR_NOT_FOUND);
    return 0u;
  }
...
typedef struct C0ThreadStartContext {
  HANDLE handle;
  LPTHREAD_START_ROUTINE start_routine;
  LPVOID parameter;
} C0ThreadStartContext;

static void* cursive_rt_linux_thread_start_trampoline(void* raw_context) {
  C0ThreadStartContext* context = (C0ThreadStartContext*)raw_context;
  struct C0RtHandleBase* handle = (struct C0RtHandleBase*)context->handle;
  DWORD exit_code = 0u;
  cursive_rt_linux_assign_current_thread_id(handle->u.thread.logical_id);
  ...
  cursive_rt_linux_release_current_thread_state();
  free(context);
  return NULL;
}
BOOL CreateProcessW(LPCWSTR application_name,
  ...
  if (application_name) {
    app_utf8 = cursive_rt_linux_path_from_wide_alloc(application_name);
    if (!app_utf8) {
      return FALSE;
    }
  }
  if (command_line) {
    cmd_utf8 = cursive_rt_linux_path_from_wide_alloc(command_line);
    if (!cmd_utf8) {
      free(app_utf8);
      return FALSE;
    }
  ...
  if (current_directory) {
    cwd_utf8 = cursive_rt_linux_path_from_wide_alloc(current_directory);
    if (!cwd_utf8) {
      free(cmd_utf8);
      free(app_utf8);
      return FALSE;
  ...
  process_handle = cursive_rt_linux_alloc_handle(C0_RT_HANDLE_PROCESS);
  if (!process_handle) {
    return FALSE;
  }
  ((struct C0RtHandleBase*)process_handle)->u.process.pid = pid;
  ((struct C0RtHandleBase*)process_handle)->u.process.waited = 0;
  ((struct C0RtHandleBase*)process_handle)->u.process.exit_code = STILL_ACTIVE;

  thread_handle = cursive_rt_linux_alloc_handle(C0_RT_HANDLE_TOKEN);
  if (!thread_handle) {
    free(process_handle);
    return FALSE;
  }
...
static DWORD cursive_rt_linux_wait_for_process(struct C0RtHandleBase* handle, DWORD milliseconds) {
  int status = 0;
  pid_t waited;
  (void)milliseconds;
...
static DWORD cursive_rt_linux_wait_for_thread(struct C0RtHandleBase* handle, DWORD milliseconds) {
  EnterCriticalSection(&handle->u.thread.lock);
...
static DWORD cursive_rt_linux_wait_for_event(struct C0RtHandleBase* handle, DWORD milliseconds) {
...
DWORD WaitForSingleObject(HANDLE handle, DWORD milliseconds) {
  struct C0RtHandleBase* base = (struct C0RtHandleBase*)handle;
  if (!base || handle == INVALID_HANDLE_VALUE) {
    SetLastError(ERROR_INVALID_HANDLE);
    return WAIT_FAILED;
  }
  switch (base->kind) {
    case C0_RT_HANDLE_PROCESS:
      return cursive_rt_linux_wait_for_process(base, milliseconds);
    case C0_RT_HANDLE_THREAD:
      return cursive_rt_linux_wait_for_thread(base, milliseconds);
    case C0_RT_HANDLE_EVENT:
      return cursive_rt_linux_wait_for_event(base, milliseconds);
    case C0_RT_HANDLE_TOKEN:
      SetLastError(ERROR_SUCCESS);
      return WAIT_OBJECT_0;
    default:
      SetLastError(ERROR_INVALID_HANDLE);
...
DWORD WaitForMultipleObjects(DWORD count,
  ...
  for (;;) {
    for (i = 0u; i < count; ++i) {
      struct C0RtHandleBase* base = (struct C0RtHandleBase*)handles[i];
      if (!base) {
        continue;
      }
      if (base->kind == C0_RT_HANDLE_EVENT && base->u.event.signaled) {
        return WAIT_OBJECT_0 + i;
      }
      if (base->kind == C0_RT_HANDLE_THREAD && base->u.thread.finished) {
        return WAIT_OBJECT_0 + i;
      }
      if (base->kind == C0_RT_HANDLE_PROCESS && base->u.process.waited) {
        return WAIT_OBJECT_0 + i;
      }
    }
    usleep(1000u);
...
BOOL GetExitCodeProcess(HANDLE handle, DWORD* exit_code) {
  ...
  int status = 0;
  pid_t result;
  if (!base || base->kind != C0_RT_HANDLE_PROCESS || !exit_code) {
    SetLastError(ERROR_INVALID_PARAMETER);
    return FALSE;
  }
...
BOOL GetExitCodeThread(HANDLE handle, DWORD* exit_code) {
  struct C0RtHandleBase* base = (struct C0RtHandleBase*)handle;
  if (!base || base->kind != C0_RT_HANDLE_THREAD || !exit_code) {
    SetLastError(ERROR_INVALID_PARAMETER);
    return FALSE;
  }
...
DWORD TlsAlloc(void) {
  DWORD index;
  EnterCriticalSection(&cursive_rt_linux_tls_slot_lock);
  for (index = 0u; index < (DWORD)(sizeof(cursive_rt_linux_tls_slot_used) / sizeof(cursive_rt_linux_tls_slot_used[0])); ++index) {
    if (!cursive_rt_linux_tls_slot_used[index]) {
      cursive_rt_linux_tls_slot_used[index] = 1;
      LeaveCriticalSection(&cursive_rt_linux_tls_slot_lock);
      SetLastError(ERROR_SUCCESS);
      return index;
    }
  }
...
LPVOID TlsGetValue(DWORD index) {
  CursiveRtLinuxThreadState* state;
  if (index >= (DWORD)(sizeof(cursive_rt_linux_tls_slot_used) / sizeof(cursive_rt_linux_tls_slot_used[0])) ||
      !cursive_rt_linux_tls_slot_used[index]) {
    SetLastError(ERROR_INVALID_PARAMETER);
    return NULL;
  }
  state = cursive_rt_linux_current_thread_state(1);
...
BOOL InitOnceExecuteOnce(PINIT_ONCE init_once,
  ...
  for (;;) {
    ...
    if (state == 0 &&
      ...
      cursive_rt_linux_futex_wake((volatile uint32_t*)&init_once->state, INT_MAX);
      break;
    }
    cursive_rt_linux_futex_wait((volatile uint32_t*)&init_once->state, 1u, NULL);
  }
  if (context) {
...
BOOL SleepConditionVariableCS(CONDITION_VARIABLE* condition,
                              CRITICAL_SECTION* section,
                              DWORD milliseconds) {
  return cursive_rt_linux_condition_wait(condition, section, milliseconds);
}

void WakeConditionVariable(CONDITION_VARIABLE* condition) {
  cursive_rt_linux_condition_wake(condition, 1);
}

void WakeAllConditionVariable(CONDITION_VARIABLE* condition) {
  cursive_rt_linux_condition_wake(condition, INT_MAX);
}
...
DWORD GetCurrentThreadId(void) {
  return cursive_rt_linux_ensure_current_thread_id();
}

HANDLE GetCurrentThread(void) {
  cursive_rt_linux_ensure_current_thread_id();
  return &cursive_rt_linux_pseudo_thread_handle;
}

int GetThreadPriority(HANDLE thread) {
  struct C0RtHandleBase* base = (struct C0RtHandleBase*)thread;
  if (!thread || thread == &cursive_rt_linux_pseudo_thread_handle) {
    CursiveRtLinuxThreadState* state = cursive_rt_linux_current_thread_state(1);
    return state ? state->priority : THREAD_PRIORITY_ERROR_RETURN;
  }
  if (!base || base->kind != C0_RT_HANDLE_THREAD) {
...
BOOL SetThreadPriority(HANDLE thread, int priority) {
  ...
  if (base->u.thread.tid == cursive_rt_linux_current_kernel_tid()) {
    CursiveRtLinuxThreadState* state = cursive_rt_linux_current_thread_state(1);
    if (state) {
      state->priority = priority;
    }
...
DWORD_PTR SetThreadAffinityMask(HANDLE thread, DWORD_PTR mask) {
  ...
  if (!thread || thread == &cursive_rt_linux_pseudo_thread_handle) {
    thread_id = 0;
  } else if (((struct C0RtHandleBase*)thread)->kind == C0_RT_HANDLE_THREAD) {
    thread_id = ((struct C0RtHandleBase*)thread)->u.thread.tid;
    if (thread_id == 0) {
...
HANDLE CreateThread(LPSECURITY_ATTRIBUTES attributes,
                    ...
                    DWORD* thread_id) {
  struct C0RtHandleBase* handle;
  C0ThreadStartContext* context;
  pthread_attr_t attr;
  (void)attributes;
  (void)creation_flags;
  handle = (struct C0RtHandleBase*)cursive_rt_linux_alloc_handle(C0_RT_HANDLE_THREAD);
  if (!handle) {
    return NULL;
  }
  InitializeCriticalSection(&handle->u.thread.lock);
  InitializeConditionVariable(&handle->u.thread.cond);
  handle->u.thread.logical_id = atomic_fetch_add(&cursive_rt_linux_next_thread_id, 1u);
  handle->u.thread.exit_code = STILL_ACTIVE;
  handle->u.thread.priority = THREAD_PRIORITY_NORMAL;
  context = (C0ThreadStartContext*)malloc(sizeof(C0ThreadStartContext));
  if (!context) {
    free(handle);
    SetLastError(ERROR_NOT_ENOUGH_MEMORY);
    return NULL;
  ...
  {
    int rc = pthread_create(&handle->u.thread.thread, &attr, cursive_rt_linux_thread_start_trampoline, context);
    if (rc != 0) {
      pthread_attr_destroy(&attr);
      free(context);
      free(handle);
      cursive_rt_linux_set_errno_error(rc);
      return NULL;
    }
  }
  pthread_attr_destroy(&attr);
...
HANDLE CreateEvent(LPSECURITY_ATTRIBUTES attributes,
  ...
  (void)attributes;
  (void)name;
  handle = (struct C0RtHandleBase*)cursive_rt_linux_alloc_handle(C0_RT_HANDLE_EVENT);
  if (!handle) {
    return NULL;
  }
...
BOOL SetEvent(HANDLE handle) {
  struct C0RtHandleBase* base = (struct C0RtHandleBase*)handle;
  if (!base || base->kind != C0_RT_HANDLE_EVENT) {
    SetLastError(ERROR_INVALID_HANDLE);
    return FALSE;
  }
...
HANDLE GetStdHandle(DWORD std_handle_id) {
  HANDLE handle = NULL;
  EnterCriticalSection(&cursive_rt_linux_std_handle_lock);
  switch (std_handle_id) {
    case STD_INPUT_HANDLE:
      handle = cursive_rt_linux_std_input;
      break;
    case STD_OUTPUT_HANDLE:
      handle = cursive_rt_linux_std_output;
      break;
    case STD_ERROR_HANDLE:
      handle = cursive_rt_linux_std_error;
      break;
    default:
      handle = NULL;
      break;
  }
  LeaveCriticalSection(&cursive_rt_linux_std_handle_lock);
  if (!handle) {
    SetLastError(ERROR_INVALID_HANDLE);
  } else {
    SetLastError(ERROR_SUCCESS);
...
BOOL SetStdHandle(DWORD std_handle_id, HANDLE handle) {
  ...
  switch (std_handle_id) {
    case STD_INPUT_HANDLE:
      cursive_rt_linux_std_input = handle;
      break;
    case STD_OUTPUT_HANDLE:
      cursive_rt_linux_std_output = handle;
      break;
    case STD_ERROR_HANDLE:
      cursive_rt_linux_std_error = handle;
      break;
    default:
      LeaveCriticalSection(&cursive_rt_linux_std_handle_lock);
      SetLastError(ERROR_INVALID_HANDLE);
...
BOOL GetConsoleMode(HANDLE handle, DWORD* mode) {
  int fd;
  if (!cursive_rt_linux_handle_fd(handle, &fd)) {
    return FALSE;
  }
  if (!isatty(fd)) {
    SetLastError(ERROR_INVALID_HANDLE);
...
static int cursive_rt_linux_open_flags_from_access(DWORD desired_access, DWORD creation_disposition) {
  int flags = 0;
...
static int cursive_rt_linux_console_fd_from_name(LPCWSTR path) {
  if (!path) {
    return -1;
  }
...
HANDLE CreateFileW(LPCWSTR path,
  ...
  (void)template_file;
  fd = cursive_rt_linux_console_fd_from_name(path);
  if (fd < 0) {
    utf8_path = cursive_rt_linux_path_from_wide_alloc(path);
    if (!utf8_path) {
      return INVALID_HANDLE_VALUE;
    }
    fd = open(utf8_path,
              cursive_rt_linux_open_flags_from_access(desired_access, creation_disposition),
              0666);
    free(utf8_path);
  }
  if (fd < 0) {
  ...
  handle = cursive_rt_linux_alloc_handle(C0_RT_HANDLE_FILE);
  if (!handle) {
    close(fd);
    return INVALID_HANDLE_VALUE;
  }
  ((struct C0RtHandleBase*)handle)->u.file.fd = fd;
  SetLastError(ERROR_SUCCESS);
  return handle;
}
...
BOOL GetFileSizeEx(HANDLE handle, LARGE_INTEGER* size_out) {
  int fd;
  struct stat st;
  if (!size_out || !cursive_rt_linux_handle_fd(handle, &fd)) {
    SetLastError(ERROR_INVALID_PARAMETER);
    return FALSE;
  }
  if (fstat(fd, &st) != 0) {
...
static DWORD cursive_rt_linux_attributes_from_stat(const struct stat* st) {
...
DWORD GetFileAttributesW(LPCWSTR path) {
  char* utf8_path = cursive_rt_linux_path_from_wide_alloc(path);
  struct stat st;
  if (!utf8_path) {
  ...
  free(utf8_path);
  SetLastError(ERROR_SUCCESS);
  return cursive_rt_linux_attributes_from_stat(&st);
}
...
static int cursive_rt_linux_fill_find_data(const char* dir_path,
                             const char* name,
                             WIN32_FIND_DATAW* find_data) {
  char* full_path = cursive_rt_linux_join_path_utf8(dir_path, name);
  struct stat st;
  wchar_t* wide_name;
  size_t wide_len = 0u;
  if (!full_path) {
  ...
  free(full_path);
  wide_name = cursive_rt_linux_wide_from_utf8_alloc(name, strlen(name), 1, &wide_len);
  if (!wide_name) {
    return 0;
  }
  ...
  find_data->dwFileAttributes = cursive_rt_linux_attributes_from_stat(&st);
  free(wide_name);
  SetLastError(ERROR_SUCCESS);
  return 1;
}

static int cursive_rt_linux_find_next_match(struct C0RtHandleBase* handle,
                              WIN32_FIND_DATAW* find_data) {
  struct dirent* entry;
  while ((entry = readdir(handle->u.find.dir)) != NULL) {
    ...
    return cursive_rt_linux_fill_find_data(handle->u.find.dir_path, entry->d_name, find_data);
  }
...
HANDLE FindFirstFileW(LPCWSTR pattern, WIN32_FIND_DATAW* find_data) {
  char* utf8_pattern = cursive_rt_linux_path_from_wide_alloc(pattern);
  char* last_sep;
  char* dir_path;
  char* wildcard;
  DIR* dir;
  ...
  if (last_sep) {
    *last_sep = '\0';
    dir_path = cursive_rt_linux_strdup_or_null(utf8_pattern[0] ? utf8_pattern : "/");
    wildcard = cursive_rt_linux_strdup_or_null(last_sep + 1);
  } else {
    dir_path = cursive_rt_linux_strdup_or_null(".");
    wildcard = cursive_rt_linux_strdup_or_null(utf8_pattern);
  }
  free(utf8_pattern);
  ...
  handle = (struct C0RtHandleBase*)cursive_rt_linux_alloc_handle(C0_RT_HANDLE_FIND);
  if (!handle) {
    closedir(dir);
    free(dir_path);
    free(wildcard);
  ...
  if (!cursive_rt_linux_find_next_match(handle, find_data)) {
    closedir(dir);
    free(dir_path);
    free(wildcard);
    free(handle);
...
BOOL FindNextFileW(HANDLE handle, WIN32_FIND_DATAW* find_data) {
  struct C0RtHandleBase* base = (struct C0RtHandleBase*)handle;
  if (!base || base->kind != C0_RT_HANDLE_FIND || !find_data) {
    SetLastError(ERROR_INVALID_PARAMETER);
    return FALSE;
  }
  return cursive_rt_linux_find_next_match(base, find_data) ? TRUE : FALSE;
}
...
DWORD GetTempPathW(DWORD buffer_length, LPWSTR buffer) {
  const char* temp_dir = cursive_rt_linux_process_env_utf8_value("TMPDIR");
  size_t wide_len = 0u;
  wchar_t* wide_temp;
  if (!temp_dir || temp_dir[0] == '\0') {
    temp_dir = cursive_rt_linux_process_env_utf8_value("TEMP");
  }
  if (!temp_dir || temp_dir[0] == '\0') {
    temp_dir = cursive_rt_linux_process_env_utf8_value("TMP");
  }
  if (!temp_dir || temp_dir[0] == '\0') {
    temp_dir = "/tmp";
  }
  wide_temp = cursive_rt_linux_wide_from_utf8_alloc(temp_dir, strlen(temp_dir), 1, &wide_len);
  if (!wide_temp) {
    return 0u;
  }
...
UINT GetTempFileNameW(LPCWSTR path_name,
  ...
  char* dir_path = cursive_rt_linux_path_from_wide_alloc(path_name);
  char* prefix = cursive_rt_linux_utf8_from_wide_alloc(prefix_string,
                                         cursive_rt_linux_wide_cstr_len(prefix_string),
                                         1,
                                         NULL);
  ...
  close(fd);
  wide_path = cursive_rt_linux_wide_from_utf8_alloc(template_path, strlen(template_path), 1, &wide_len);
  if (!wide_path) {
...
HANDLE CreateFileMappingW(HANDLE file,
  ...
  if (file == INVALID_HANDLE_VALUE) {
    char* raw_name = cursive_rt_linux_utf8_from_wide_alloc(name, cursive_rt_linux_wide_cstr_len(name), 1, NULL);
    size_t i;
    if (!raw_name) {
      return NULL;
    }
    ...
    free(shm_name);
  } else if (file && ((struct C0RtHandleBase*)file)->kind == C0_RT_HANDLE_FILE) {
    fd = dup(((struct C0RtHandleBase*)file)->u.file.fd);
  ...
  handle = (struct C0RtHandleBase*)cursive_rt_linux_alloc_handle(C0_RT_HANDLE_MAPPING);
  if (!handle) {
    close(fd);
    return NULL;
  }
...
LPVOID MapViewOfFile(HANDLE mapping,
  ...
  struct C0RtHandleBase* base = (struct C0RtHandleBase*)mapping;
  off_t offset = (off_t)(((uint64_t)file_offset_high << 32u) | (uint64_t)file_offset_low);
  size_t map_size;
  int prot = PROT_READ;
  void* address;
  C0MapViewNode* node;
  if (!base || base->kind != C0_RT_HANDLE_MAPPING) {
    SetLastError(ERROR_INVALID_HANDLE);
    return NULL;
  }
  ...
  node = (C0MapViewNode*)malloc(sizeof(C0MapViewNode));
  if (!node) {
  ...
  node->size = map_size;
  EnterCriticalSection(&cursive_rt_linux_map_view_lock);
  node->next = cursive_rt_linux_map_views;
  cursive_rt_linux_map_views = node;
  LeaveCriticalSection(&cursive_rt_linux_map_view_lock);
  SetLastError(ERROR_SUCCESS);
  return address;
}
...
BOOL UnmapViewOfFile(LPCVOID base_address) {
  ...
  EnterCriticalSection(&cursive_rt_linux_map_view_lock);
  link = &cursive_rt_linux_map_views;
...
BOOL CloseHandle(HANDLE handle) {
  ...
  switch (base->kind) {
    ...
    case C0_RT_HANDLE_EVENT:
      ...
      break;
    case C0_RT_HANDLE_FILE:
      close(base->u.file.fd);
      break;
    case C0_RT_HANDLE_FIND:
      if (base->u.find.dir) {
...
void cursive_platform_backend_icu_data_configure(void) {
  ...
  if (!cursive_rt_linux_data_directory_from_module(data_dir, sizeof(data_dir))) {
    return;
  }
...
cursive_platform_bool_t cursive_platform_backend_process_create(
  ...
  return CURSIVE_PLATFORM_BACKEND_TRUE;
}
...
cursive_platform_module_t cursive_platform_backend_module_get(const char* name) {
...
void cursive_platform_backend_rwlock_lock_shared(
    cursive_platform_rwlock_t* lock) {
...
cursive_platform_handle_t cursive_platform_backend_find_first_wide(
  ...
  if (handle != CURSIVE_PLATFORM_BACKEND_INVALID_HANDLE && find_data) {
...
cursive_platform_bool_t cursive_platform_backend_panic_boundary_active(void) {
  CursiveRtLinuxThreadState* state = cursive_rt_linux_current_thread_state(0);
  return (state && state->panic_scope != NULL) ? CURSIVE_PLATFORM_TRUE
                                               : CURSIVE_PLATFORM_FALSE;
}
...
cursive_platform_bool_t cursive_platform_backend_panic_boundary_run(
    ...
    cursive_platform_u32_t* panic_code) {
  CursiveRtLinuxThreadState* state;
  CursiveRtLinuxPanicScope scope;
