...
//
// DEPENDENCIES:
//   - cursive/src/05_codegen/ir/ir_model.h (IRSpawn, IRBindVar, IRWritePtr, etc.)
...
LowerResult LowerSpawnExpr(const ast::Expr& expr,
  ...
  if (body_type_from_spawn && body_type_from_tail) {
    ...
    if (!equiv.equiv) {
      ...
      std::fprintf(stderr,
                   "[cursive] codegen failure: spawn result type mismatch; "
                   "spawn=%s tail=%s\n",
  ...
  if (core::IsDebugEnabled("spawn")) {
    ...
    std::fprintf(stderr,
                 "[cursive] spawn lower: tail=%d body_type=%s result_size=%llu\n",
                 has_tail,
