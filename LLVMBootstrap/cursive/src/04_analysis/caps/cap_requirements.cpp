...
//
// SOURCE FILES:
//   - cursive-bootstrap/src/03_analysis/caps/cap_filesystem.cpp
//   - cursive-bootstrap/src/03_analysis/caps/cap_heap.cpp
//   - cursive-bootstrap/src/03_analysis/caps/cap_concurrency.cpp
//
...
std::string_view CapabilityKindName(CapabilityKind kind) {
  ...
  switch (kind) {
    case CapabilityKind::FileSystem:
      return "FileSystem";
...
std::optional<CapabilityKind> CapabilityKindFromPath(const TypePath& path) {
  ...
  if (IsFileSystemClassPath(class_path)) {
    return CapabilityKind::FileSystem;
  }
...
bool CapabilitySet::IsSubsetOf(const CapabilitySet& other) const {
  ...
  if (has_filesystem && !other.has_filesystem) return false;
...
CapabilitySet CapabilitySet::Union(const CapabilitySet& other) const {
  ...
  result.has_filesystem = has_filesystem || other.has_filesystem;
...
bool CapabilitySet::IsEmpty() const {
  return !has_filesystem && !has_network && !has_heap &&
         !has_execution_domain &&
         !has_reactor && !has_system && !has_context;
}
...
std::string CapabilitySet::ToString() const {
  ...
  if (has_context) {
    ...
    if (has_filesystem) append("FileSystem");
...
CapabilityValidationResult ValidateCapabilitySatisfied(
  ...
  if (required.has_filesystem && !provided.has_filesystem) {
    missing.has_filesystem = true;
  }
...
std::optional<CapabilityKind> MethodCallRequiresCapability(
  ...
  if (const auto* dyn = std::get_if<TypeDynamic>(&receiver_type->node)) {
    ...
    // FileSystem methods
    if (IsFileSystemClassPath(class_path)) {
      if (LookupFileSystemMethodSig(method_name)) {
        return CapabilityKind::FileSystem;
      }
    }
