...
namespace cursive::analysis::layout
{
  ...
  namespace
  {
    ...
    cursive::analysis::TypeRef StripPermRefine(
        const cursive::analysis::TypeRef &type)
    {
      cursive::analysis::TypeRef cur = type;
      while (cur)
      {
        if (const auto *perm = std::get_if<cursive::analysis::TypePerm>(&cur->node))
        {
          cur = perm->base;
          continue;
        }
        if (const auto *refine =
                std::get_if<cursive::analysis::TypeRefine>(&cur->node))
        {
          cur = refine->base;
          continue;
        }
    ...
    std::optional<std::vector<cursive::analysis::TypeRef>> RangeFieldTypes(
        const cursive::analysis::TypeRef &type)
    {
      ...
      if (const auto *range =
              std::get_if<cursive::analysis::TypeRangeInclusive>(&stripped->node))
      {
        fields.push_back(range->base);
        fields.push_back(range->base);
        return fields;
      ...
      if (std::holds_alternative<cursive::analysis::TypeRangeFull>(stripped->node))
      {
        return fields;
      }
      return std::nullopt;
        ...
        const cursive::analysis::TypeRef &type)
    {
      ...
      if (std::holds_alternative<cursive::analysis::TypeRange>(stripped->node))
      {
        return ValueRangeKind::Exclusive;
      }
      if (std::holds_alternative<cursive::analysis::TypeRangeInclusive>(
              stripped->node))
      {
        return ValueRangeKind::Inclusive;
      }
      if (std::holds_alternative<cursive::analysis::TypeRangeFrom>(stripped->node))
      {
        return ValueRangeKind::From;
      }
      if (std::holds_alternative<cursive::analysis::TypeRangeTo>(stripped->node))
      {
        return ValueRangeKind::To;
      }
      if (std::holds_alternative<cursive::analysis::TypeRangeToInclusive>(
              stripped->node))
      {
    ...
    std::optional<Value> RangeBoundValueForType(
        const cursive::analysis::TypeRef &bound_type,
        std::uint64_t raw)
    {
      ...
      const auto *prim = std::get_if<cursive::analysis::TypePrim>(&stripped->node);
      if (!prim)
      {
        return std::nullopt;
      }
    ...
    bool IsNicheType(const cursive::analysis::ScopeContext &ctx,
                     const cursive::analysis::TypeRef &type)
    {
      (void)ctx;
      if (!type)
      ...
      {
        return ptr->state == cursive::analysis::PtrState::Valid;
      }
      return false;
    }
    ...
    bool IsNicheBits(const cursive::analysis::ScopeContext &ctx,
                     const cursive::analysis::TypeRef &type,
                     const std::vector<std::uint8_t> &bits)
    {
        ...
        std::string_view name)
    {
      ...
      {
        if (cursive::analysis::IdEq(state.name, name))
        {
          return &state;
        }
      }
    ...
    bool IsEmptyState(const cursive::ast::StateBlock &state)
    {
      for (const auto &member : state.members)
      {
        if (std::holds_alternative<cursive::ast::StateFieldDecl>(member))
        {
          return false;
        }
      }
    ...
    std::optional<std::pair<std::string, cursive::analysis::TypeRef>>
    SingleFieldPayloadType(const cursive::analysis::ScopeContext &ctx,
                           const cursive::ast::StateBlock &state)
    {
      const cursive::ast::StateFieldDecl *field = nullptr;
      for (const auto &member : state.members)
      {
        const auto *payload =
    ...
    std::optional<std::vector<std::pair<std::string, cursive::analysis::TypeRef>>> CollectStateFields(
        const cursive::analysis::ScopeContext &ctx,
        const cursive::ast::StateBlock &state)
    {
      std::vector<std::pair<std::string, cursive::analysis::TypeRef>> fields;
      for (const auto &member : state.members)
      {
        if (const auto *field =
                std::get_if<cursive::ast::StateFieldDecl>(&member))
    ...
    std::optional<std::vector<std::uint8_t>> ValueBitsForRecord(
        const cursive::analysis::ScopeContext &ctx,
        const std::vector<std::pair<std::string, cursive::analysis::TypeRef>> &fields,
        const RecordVal &val);
    ...
    bool ValidStructBits(const cursive::analysis::ScopeContext &ctx,
                         const std::vector<cursive::analysis::TypeRef> &types,
                         const std::vector<std::uint64_t> &offsets,
                         std::uint64_t size,
                         const std::vector<std::uint8_t> &bits)
    {
      ...
      const cursive::ast::Token &lit)
  {
    ...
    const auto *prim = std::get_if<cursive::analysis::TypePrim>(&type->node);
    if (!prim)
    {
      if (std::holds_alternative<cursive::analysis::TypeRawPtr>(type->node) &&
          lit.kind == cursive::ast::TokenKind::NullLiteral)
      {
    ...
    if (prim->name == "bool")
    {
      if (lit.kind != cursive::ast::TokenKind::BoolLiteral)
      {
        return std::nullopt;
      }
    ...
    if (prim->name == "char")
    {
      if (lit.kind != cursive::ast::TokenKind::CharLiteral)
      {
        return std::nullopt;
      }
    ...
    if (lit.kind == cursive::ast::TokenKind::IntLiteral)
    {
      if (!IsIntTypeName(prim->name))
      {
        return std::nullopt;
    ...
    if (lit.kind == cursive::ast::TokenKind::FloatLiteral)
    {
                  ...
                  const std::vector<std::uint8_t> &bits)
  {
    ...
    if (std::holds_alternative<cursive::analysis::TypeBytes>(type->node))
    {
      const auto size = SizeOf(ctx, type);
      return size.has_value() && bits.size() == *size;
    }
  ...
  namespace
  {
        ...
        const RecordVal &val)
    {
      ...
      std::vector<cursive::analysis::TypeRef> types;
      ...
      for (const auto &field : fields)
      {
        const auto it = std::find_if(val.fields.begin(), val.fields.end(),
                                     [&](const auto &entry)
                                     {
                                       return cursive::analysis::IdEq(entry.first,
                                                                      field.first);
                                     });
      ...
      const Value &value)
  {
    ...
    if (const auto *ptr = std::get_if<cursive::analysis::TypePtr>(&type->node))
    {
      if (const auto *v = std::get_if<PtrVal>(&value.node))
      {
        ...
        if (v->state == cursive::analysis::PtrState::Valid && v->addr == 0)
        {
          return std::nullopt;
        }
        if (v->state == cursive::analysis::PtrState::Null && v->addr != 0)
        {
          return std::nullopt;
        }
    ...
    if (const auto *raw = std::get_if<cursive::analysis::TypeRawPtr>(&type->node))
    {
    ...
    if (std::holds_alternative<cursive::analysis::TypeSlice>(type->node))
    {
      ...
      if (v->ptr.qual != cursive::analysis::RawPtrQual::Imm)
      {
        return std::nullopt;
      }
