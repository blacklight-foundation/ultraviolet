...
//
//   CursiveSpecification.md §2 (lines 766-777) - Command-Line Output
//
// =============================================================================
...
ENVIRONMENT
  ...
  CURSIVE_RUNTIME_SINK       Runtime sink override: console|file|both
  CURSIVE_RUNTIME_PATH       Runtime file sink path (used with sink=file|both)
  CURSIVE_RUNTIME_ROOT       Runtime source-root prefix to strip from file paths
  CURSIVE_RUNTIME_FILTER     Runtime category filter: log,diagnostic,runtime,all
  CURSIVE_RUNTIME_LEVEL      Runtime min level: trace|info|warning|error
  CURSIVE_RUNTIME_RULE       Runtime rule-id substring filter
  CURSIVE_RUNTIME_FILE       Runtime file substring filter
  CURSIVE_RUNTIME_LABEL      Runtime label substring filter
  CURSIVE_RUNTIME_FAIL_ONLY  Runtime filter to cmp=fail records only (1/0)
  CURSIVE_RUNTIME_BREAK_ON_FAIL Runtime break on cmp=fail (1/0)
...
void PrintDebugHelp() {
  std::cout << "cursive debug subsystems\n\n";
