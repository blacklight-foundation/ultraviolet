...
//
// SOURCE FILE: ultraviolet-bootstrap/src/02_syntax/lexer_security.cpp
//   Lines 1-142 (entire file)
//
// DEPENDENCIES:
//   - ultraviolet/include/02_source/lexer.h (LexSecureResult, Token)
//   - ultraviolet/src/00_core/unicode.cpp (Utf8Offsets)
//   - ultraviolet/src/00_core/span.cpp (SpanOf, SpanRange)
...
LexSecureResult ConfusableCheck(const core::SourceFile& source,
  ...
  for (const Token& tok : tokens) {
    ...
    if (core::IsDebugEnabled("lex") || core::IsDebugEnabled("parse")) {
      std::cerr << "[uv] unicode: ConfusableCheck token=\"" << tok.lexeme
                << "\"\n";
    }
