...
static TypeEnv ExtendStmtComptimeEnv(const TypeEnv& env,
  ...
  if (HasAttribute(attrs, ::ultraviolet::analysis::attrs::kFiles)) {
    scope[IdKeyOf("files")] =
        TypeBinding{ast::Mutability::Let, StmtProjectFilesType()};
  }
  if (HasAttribute(attrs, ::ultraviolet::analysis::attrs::kEmit)) {
...
class CtProhibitedConstructFinder {
 public:
  std::optional<core::Span> Find(const ast::Block& block) {
...
static StmtTypeResult TypeCtStmt(const ScopeContext& ctx,
  ...
  if (const auto span = CtProhibitedConstructFinder{}.Find(*stmt.body)) {
    return {false, "E-CTE-0020", env, {}, {}, span};
  }
...
struct DirectCallFactView {
  ...
  const ast::CallExpr* call_expr = nullptr;
};
...
static std::optional<DirectCallFactView> DirectCallFactViewOf(
  ...
  if (const auto* call = std::get_if<ast::CallExpr>(&stripped->node)) {
    return DirectCallFactView{&call->callee, &call->args, call};
  }
  if (const auto* call_type_args =
          std::get_if<ast::CallTypeArgsExpr>(&stripped->node)) {
    return DirectCallFactView{&call_type_args->callee, &call_type_args->args,
                              nullptr};
  }
  return std::nullopt;
}

static std::optional<std::pair<ast::ModulePath, std::string>> StaticCalleeName(
  ...
  if (const auto* ident = std::get_if<ast::IdentifierExpr>(&callee->node)) {
    if (const auto ent = ResolveValueName(ctx, ident->name);
        ent && ent->origin_opt.has_value()) {
      return std::make_pair(*ent->origin_opt,
                            ent->target_opt.value_or(std::string(ident->name)));
    }
    return std::make_pair(ctx.current_module, std::string(ident->name));
  }
  if (const auto* qualified =
          std::get_if<ast::QualifiedNameExpr>(&callee->node)) {
    return std::make_pair(qualified->path, std::string(qualified->name));
  }
  if (const auto* path = std::get_if<ast::PathExpr>(&callee->node)) {
    return std::make_pair(path->path.empty() ? ctx.current_module : path->path,
                          std::string(path->name));
  }
  return std::nullopt;
}

static const ast::ASTModule* FindModuleForCallPostcondition(
    const ScopeContext& ctx,
    const ast::ModulePath& path) {
  const auto key = PathKeyOf(path);
  for (const auto& module : ctx.sigma.mods) {
    if (PathKeyOf(module.path) == key) {
      return &module;
    }
  }
  return nullptr;
...
static const ast::ProcedureDecl* FindProcedureForCallPostcondition(
...
static const ast::ProcedureDecl* StaticCalleeProcedure(
    const ScopeContext& ctx,
    const DirectCallFactView& call) {
  if (call.call_expr && ctx.selected_call_targets) {
    const auto selected = ctx.selected_call_targets->find(call.call_expr);
    if (selected != ctx.selected_call_targets->end()) {
      return selected->second.proc;
    }
  }
  if (!call.callee) {
    return nullptr;
...
static ast::ExprPtr IdentifierFactExpr(const std::string& name,
  ...
  ast::IdentifierExpr ident;
  ident.name = name;
  return MakeProofExpr(span, std::move(ident));
}

static bool ExprStableForCallPostcondition(const ast::ExprPtr& expr,
                                           const TypeEnv& env) {
  if (!expr) {
    return false;
  }
  return std::visit(
      [&](const auto& node) -> bool {
        ...
        if constexpr (std::is_same_v<T, ast::LiteralExpr> ||
          ...
          return ExprStableForCallPostcondition(node.base, env);
        } else if constexpr (std::is_same_v<T, ast::TupleAccessExpr>) {
          return ExprStableForCallPostcondition(node.base, env);
        } else if constexpr (std::is_same_v<T, ast::IndexAccessExpr>) {
          return ExprStableForCallPostcondition(node.base, env) &&
                 ExprStableForCallPostcondition(node.index, env);
        } else if constexpr (std::is_same_v<T, ast::UnaryExpr>) {
          return ExprStableForCallPostcondition(node.value, env);
        } else if constexpr (std::is_same_v<T, ast::BinaryExpr>) {
          return ExprStableForCallPostcondition(node.lhs, env) &&
                 ExprStableForCallPostcondition(node.rhs, env);
        } else {
          return false;
        }
      },
...
static ast::ExprPtr SubstituteCallPostconditionExpr(
    const ast::ExprPtr& expr,
    const std::unordered_map<std::string, ast::ExprPtr>& substitutions,
  ...
  return std::visit(
      [&](const auto& node) -> ast::ExprPtr {
        ...
        if constexpr (std::is_same_v<T, ast::LiteralExpr> ||
          ...
          const auto subst = substitutions.find(node.name);
          return subst == substitutions.end() ? expr : subst->second;
        } else if constexpr (std::is_same_v<T, ast::EntryExpr>) {
          auto inner = SubstituteCallPostconditionExpr(
              node.expr, substitutions, result_expr, env, blocked);
          if (blocked || !ExprStableForCallPostcondition(inner, env)) {
            blocked = true;
            return expr;
          }
          return inner;
          ...
          auto out = node;
          out.lhs = SubstituteCallPostconditionExpr(
              node.lhs, substitutions, result_expr, env, blocked);
          out.rhs = SubstituteCallPostconditionExpr(
              node.rhs, substitutions, result_expr, env, blocked);
          return MakeProofExpr(expr->span, std::move(out));
        } else if constexpr (std::is_same_v<T, ast::UnaryExpr>) {
          auto out = node;
          out.value = SubstituteCallPostconditionExpr(
              node.value, substitutions, result_expr, env, blocked);
          return MakeProofExpr(expr->span, std::move(out));
        } else if constexpr (std::is_same_v<T, ast::FieldAccessExpr>) {
          auto out = node;
          out.base = SubstituteCallPostconditionExpr(
              node.base, substitutions, result_expr, env, blocked);
          return MakeProofExpr(expr->span, std::move(out));
        } else if constexpr (std::is_same_v<T, ast::TupleAccessExpr>) {
          auto out = node;
          out.base = SubstituteCallPostconditionExpr(
          ...
          out.base = SubstituteCallPostconditionExpr(
              node.base, substitutions, result_expr, env, blocked);
          out.index = SubstituteCallPostconditionExpr(
              node.index, substitutions, result_expr, env, blocked);
          return MakeProofExpr(expr->span, std::move(out));
        } else if constexpr (std::is_same_v<T, ast::TupleExpr>) {
          auto out = node;
          for (auto& element : out.elements) {
            element = SubstituteCallPostconditionExpr(
                element, substitutions, result_expr, env, blocked);
          }
          return MakeProofExpr(expr->span, std::move(out));
        } else if constexpr (std::is_same_v<T, ast::RecordExpr>) {
          auto out = node;
          for (auto& field : out.fields) {
            field.value = SubstituteCallPostconditionExpr(
                field.value, substitutions, result_expr, env, blocked);
          }
          return MakeProofExpr(expr->span, std::move(out));
        } else if constexpr (std::is_same_v<T, ast::IfExpr>) {
          auto out = node;
          out.cond = SubstituteCallPostconditionExpr(
              node.cond, substitutions, result_expr, env, blocked);
          out.then_expr = SubstituteCallPostconditionExpr(
              node.then_expr, substitutions, result_expr, env, blocked);
          out.else_expr = SubstituteCallPostconditionExpr(
              node.else_expr, substitutions, result_expr, env, blocked);
          return MakeProofExpr(expr->span, std::move(out));
        } else if constexpr (std::is_same_v<T, ast::IfIsExpr>) {
          auto out = node;
          out.scrutinee = SubstituteCallPostconditionExpr(
              node.scrutinee, substitutions, result_expr, env, blocked);
          out.then_expr = SubstituteCallPostconditionExpr(
              node.then_expr, substitutions, result_expr, env, blocked);
          out.else_expr = SubstituteCallPostconditionExpr(
              node.else_expr, substitutions, result_expr, env, blocked);
          ...
          for (auto& arm : out.cases) {
            arm.body = SubstituteCallPostconditionExpr(
                arm.body, substitutions, result_expr, env, blocked);
          }
          out.else_expr = SubstituteCallPostconditionExpr(
              node.else_expr, substitutions, result_expr, env, blocked);
          return MakeProofExpr(expr->span, std::move(out));
          ...
          auto out = node;
          out.callee = SubstituteCallPostconditionExpr(
              node.callee, substitutions, result_expr, env, blocked);
          for (auto& arg : out.args) {
            arg.value = SubstituteCallPostconditionExpr(
                arg.value, substitutions, result_expr, env, blocked);
          }
          return MakeProofExpr(expr->span, std::move(out));
        } else if constexpr (std::is_same_v<T, ast::CallTypeArgsExpr>) {
          auto out = node;
          ...
          auto out = node;
          out.receiver = SubstituteCallPostconditionExpr(
              node.receiver, substitutions, result_expr, env, blocked);
          for (auto& arg : out.args) {
            arg.value = SubstituteCallPostconditionExpr(
                arg.value, substitutions, result_expr, env, blocked);
          }
...
static std::shared_ptr<StaticProofContext> CallPostconditionProofContextForLet(
    const ScopeContext& ctx,
    const TypeEnv& env,
  ...
  const auto* proc = StaticCalleeProcedure(ctx, *call);
  if (!proc || !proc->contract.has_value() || !proc->contract->postcondition) {
    return current_proof_ctx;
  }
  ...
  const auto postcondition = SubstituteCallPostconditionExpr(
      proc->contract->postcondition, substitutions, result_expr, env, blocked);
  if (blocked) {
    return current_proof_ctx;
  }

  SPEC_RULE("Fact-Call-Postcondition");
  return ExtendProofContextWithPredicateAt(current_proof_ctx, postcondition,
                                           let_stmt.span);
}
...
StmtSeqResult TypeStmtSeq(const ScopeContext& ctx,
  ...
  for (const auto& stmt : stmts) {
    ...
    current = typed.env;
    current_proof_ctx =
        FallthroughProofContextForStmt(ctx, current, current_proof_ctx, stmt);
