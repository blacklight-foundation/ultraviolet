...
//
// SOURCE FILE: ultraviolet-bootstrap/src/04_codegen/layout/layout_aggregates.cpp
...
//
// DEPENDENCIES:
//   - ultraviolet/include/04_analysis/layout/layout.h (Layout, RecordLayout, EnumLayout)
...
std::optional<ultraviolet::analysis::TypeSubst> BuildEnumLayoutSubstitution(
  ...
  return ultraviolet::analysis::BuildSubstitution(generic_params->params,
                                              generic_args);
}

std::optional<ultraviolet::analysis::TypeRef> LowerEnumPayloadType(
  ...
  return ultraviolet::analysis::InstantiateType(*lowered, subst);
}
...
std::optional<RecordLayout> RangeLayoutInternal(
  ...
  ultraviolet::analysis::TypeRef stripped = type;
  while (stripped) {
    if (const auto* perm =
            std::get_if<ultraviolet::analysis::TypePerm>(&stripped->node)) {
      stripped = perm->base;
      continue;
    }
    if (const auto* refine =
            std::get_if<ultraviolet::analysis::TypeRefine>(&stripped->node)) {
  ...
  if (const auto* range =
    ...
    fields.push_back(range->base);
  } else if (const auto* range = std::get_if<ultraviolet::analysis::TypeRangeInclusive>(
                 &stripped->node)) {
    ...
    fields.push_back(range->base);
  } else if (std::holds_alternative<ultraviolet::analysis::TypeRangeFull>(
                 stripped->node)) {
...
std::optional<EnumLayout> EnumLayoutOf(
  ...
  const auto discs = ultraviolet::analysis::EnumDiscriminants(decl);
  ...
  for (const auto& variant : decl.variants) {
    ...
    if (!variant.payload_opt.has_value()) {
      layout = RecordLayout{Layout{0, 1}, {}};
                   ...
                   std::get_if<ultraviolet::ast::VariantPayloadTuple>(
                       &*variant.payload_opt)) {
      std::vector<ultraviolet::analysis::TypeRef> elems;
...
std::optional<EnumPayloadMemberLayout> EnumRecordPayloadMemberLayout(
  ...
  for (std::size_t i = 0; i < field_names.size() && i < layout->offsets.size(); ++i) {
    if (!ultraviolet::analysis::IdEq(field_names[i], std::string(field_name))) {
      continue;
    }
    EnumPayloadMemberLayout out;
