...
using namespace emit_detail;
  ...
  void LLVMEmitter::EmitProc(const ProcIR &proc)
  {
    ...
    auto bind_zero_sized_param = [&](const IRParam &param) -> bool
    {
      ...
      const auto size = ::ultraviolet::analysis::layout::SizeOf(param_scope, param.type);
      ...
      if (panic_record_ty && panic_ptr_ty && panic_ptr_ty->isPointerTy())
      {
        ...
        if (!panic_record_ptr)
        {
          llvm::AllocaInst *panic_record_alloca =
              entry_builder.CreateAlloca(panic_record_ty, nullptr, "__uv_panic_record");
          builder->CreateStore(llvm::Constant::getNullValue(panic_record_ty),
                               panic_record_alloca);
          panic_record_ptr =
    ...
    if (async_info)
    {
      ...
      {
        async_state_storage.frame_ptr = LoadLocalValue(*this, builder, "__uv_async_frame");
        async_state_storage.input_ptr = LoadLocalValue(*this, builder, "__uv_async_input");
