...
//
// SOURCE FILE: ultraviolet-bootstrap/src/04_codegen/abi/abi_calls.cpp
...
//
// DEPENDENCIES:
//   - ultraviolet/include/05_codegen/abi/abi.h (ABICallInfo)
...
//      - Capability methods (IO, HeapAllocator)
...
std::optional<std::string> BuiltinMethodSym(const analysis::TypePath& cap_path,
  ...
  // (BuiltinMethodSym-IO)
  // BuiltinSym(IO::name) => sym
  if (analysis::IsIOClassPath(cap_class)) {
    SPEC_RULE("BuiltinMethodSym-IO");
    const std::string sym = BuiltinSym(std::string("IO::") + std::string(name));
    if (sym.empty()) {
      return std::nullopt;
    }
    return sym;
