...
static char* make_temp_file_utf8(const wchar_t* prefix) {
  ...
  if (uv_rt_temp_file_name_wide(temp_root, prefix, 0u, temp_path) == 0u) {
    return NULL;
  }
...
static char* read_file_utf8(const char* path_utf8) {
  wchar_t* wide_path;
  uv_rt_handle_t file;
  uv_rt_large_integer_t size;
  char* bytes;
  size_t total_read = 0u;
  uv_rt_dword_t read = 0u;
  if (!path_utf8) {
    return NULL;
  }
  ...
  file = uv_rt_file_open_wide(wide_path,
                                   UV_RT_GENERIC_READ,
                                   ...
                                   NULL);
  free(wide_path);
  if (file == UV_RT_INVALID_HANDLE) {
    return NULL;
  }
  if (!uv_rt_file_size_get(file, &size) || size.quad_part < 0) {
    uv_rt_close_handle(file);
    return NULL;
  }
  ...
  while (total_read < (size_t)size.quad_part) {
    uv_rt_dword_t remaining = (uv_rt_dword_t)((size_t)size.quad_part - total_read);
    if (!uv_rt_handle_read(file, bytes + total_read, remaining, &read)) {
      free(bytes);
      uv_rt_close_handle(file);
      return NULL;
    }
...
static void delete_file_utf8(const char* path_utf8) {
  ...
  uv_rt_file_delete_wide(wide_path);
  free(wide_path);
}
...
static uv_rt_dword_t task104_emit_thread(void* param) {
  Task104ThreadArgs* args = (Task104ThreadArgs*)param;
  uint32_t i;
  args->thread_id = uv_rt_current_thread_id();
  for (i = 0u; i < args->emit_count; ++i) {
    char payload[128];
    UVStringView rule_id = sv_from_cstr("Log-Thread");
    UVStringView file = sv_from_cstr("task104_runtime_thread");
...
static int test_file_sequence_and_no_reset(void) {
  ...
  UVStringView rule = sv_from_cstr("Log-Test");
  UVStringView file = sv_from_cstr("task104_runtime_file");
  UVStringView payload_a = sv_from_cstr("category=log;level=info;label=file-a");
  UVStringView payload_b = sv_from_cstr("category=log;level=info;label=file-b");
  uv_rt_thread_id_t current_tid = uv_rt_current_thread_id();
  ...
  ultraviolet_x3a_x3aruntime_x3a_x3aconformance_x3a_x3aset_x5fsink(
      1u, (const uint8_t*)path_a, (uint64_t)strlen(path_a));
  ultraviolet_x3a_x3aruntime_x3a_x3aconformance_x3a_x3aemit(
      &rule, &file, 10u, 1u, 10u, 4u, &payload_a);
  ultraviolet_x3a_x3aruntime_x3a_x3aconformance_x3a_x3aemit_x5fint(
...
static int test_multithreaded_sequence_order(void) {
  ...
  uv_rt_handle_t threads[TASK104_THREAD_COUNT];
  Task104ThreadArgs args[TASK104_THREAD_COUNT];
  uv_rt_thread_id_t tids[TASK104_THREAD_COUNT];
  size_t i;
  ...
  for (i = 0u; i < TASK104_THREAD_COUNT; ++i) {
    ...
    threads[i] = uv_rt_thread_create(NULL, 0u, task104_emit_thread, &args[i], 0u, NULL);
    if (!threads[i]) {
      size_t j;
      for (j = 0u; j < i; ++j) {
        if (threads[j]) {
          uv_rt_wait_one(threads[j], UV_RT_INFINITE);
          uv_rt_close_handle(threads[j]);
        }
      }
  ...
  for (i = 0u; i < TASK104_THREAD_COUNT; ++i) {
    uv_rt_dword_t exit_code = 0u;
    if (uv_rt_wait_one(threads[i], UV_RT_INFINITE) != UV_RT_WAIT_OBJECT_0) {
      size_t j;
    ...
    if (!uv_rt_thread_exit_code(threads[i], &exit_code) || exit_code != 0u) {
      size_t j;
...
static int test_console_sink_metadata(void) {
  ...
  uv_rt_handle_t saved_stderr = uv_rt_std_handle(UV_RT_STD_ERROR_HANDLE);
  uv_rt_handle_t capture = UV_RT_INVALID_HANDLE;
  char* text = NULL;
  wchar_t* wide_path = NULL;
  Task104RecordList records;
  UVStringView rule = sv_from_cstr("Log-Console");
  UVStringView file = sv_from_cstr("task104_runtime_console");
  UVStringView payload = sv_from_cstr("category=log;level=info;label=console");
  uv_rt_thread_id_t current_tid = uv_rt_current_thread_id();
  ...
  capture = uv_rt_file_open_wide(wide_path,
                                      UV_RT_GENERIC_WRITE | UV_RT_GENERIC_READ,
                                      UV_RT_FILE_SHARE_READ |
                                          UV_RT_FILE_SHARE_WRITE |
                                          UV_RT_FILE_SHARE_DELETE,
                                      NULL,
                                      ...
                                      NULL);
  free(wide_path);
  if (capture == UV_RT_INVALID_HANDLE) {
    free(capture_path);
    return report_failure("console capture open");
  }

  if (!uv_rt_std_handle_set(UV_RT_STD_ERROR_HANDLE, capture)) {
    uv_rt_close_handle(capture);
    free(capture_path);
    return report_failure("console redirect stderr");
  }

  ultraviolet_x3a_x3aruntime_x3a_x3aconformance_x3a_x3aset_x5fsink(0u, NULL, 0u);
  ultraviolet_x3a_x3aruntime_x3a_x3aconformance_x3a_x3aemit(
      &rule, &file, 20u, 1u, 20u, 4u, &payload);
  uv_rt_handle_flush(capture);

  if (!uv_rt_std_handle_set(UV_RT_STD_ERROR_HANDLE, saved_stderr)) {
    uv_rt_close_handle(capture);
